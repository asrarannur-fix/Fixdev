      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          ...
          <button
            ...
            className="bg-cyan-600 hover:bg-cyan-700 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1"
          >
          ...
          <div className="p-5 border-b border-slate-100 bg-slate-50/50 space-y-3 text-xs">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Contoh: Telepon balik soal servis"
                className="w-full px-3 py-2 border border-slate-200 rounded-lg outline-none"
              />
            </div>
            <div>
        ...
        <div className="px-5 py-3 border-b border-slate-100 flex gap-2">
          {['ALL', 'PENDING', 'COMPLETED', 'OVERDUE'].map((s) => (
            <button
              ...
              className={`px-3 py-1 rounded-full text-[10px] font-bold ${statusFilter === s ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
            >
              {s === 'ALL' ? 'Semua' : s}
            </button>
          ))}
        ...
        <div className="divide-y divide-slate-100">
          {tenantFollowUps.length === 0 ? (
            ...
            tenantFollowUps.map((f) => (
              <div
                key={f.id}
                className={`px-5 py-3 flex items-center gap-4 ${isOverdue(f) ? 'bg-red-50/50' : ''}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-[9px] font-bold font-mono ${statusColor(isOverdue(f) ? 'OVERDUE' : f.status)}`}
                    >
