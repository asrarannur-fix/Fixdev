...
const SettingsDropdown: React.FC<{ onSetTab?: (tab: string, subTab?: string) => void }> = ({
  onSetTab,
}) => {
      ...
      {open && (
        ...
        >
          <div className="px-4 py-2 flex items-center gap-2 border-b border-slate-100 dark:border-zinc-800">
            ...
            <span className="text-[10px] font-black text-slate-600 dark:text-zinc-300 uppercase tracking-widest">
              Pengaturan Sistem
            </span>
          </div>
...
export const Topbar: React.FC<TopbarProps> = ({
  ...
  const cloudStatus =
    currentUser.role === UserRole.SUPER_ADMIN ? platformHealth.status : platformHealth.status;
    ...
    >
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <button
          ...
          aria-label="Search"
          className="hidden sm:flex items-center gap-2 px-3 rounded-xl bg-slate-100/50 dark:bg-slate-900/40 text-slate-500 dark:text-slate-400 border border-slate-200/40 dark:border-slate-800/80 hover:bg-white dark:hover:bg-slate-900 hover:shadow-sm focus:outline-none transition-all duration-200 cursor-pointer shrink-0 text-xs h-8"
          id="top-search-trigger-btn"
          title="Search (⌘K)"
        >
          <Search className="w-4 h-4" />
          <span className="text-slate-400/70 dark:text-slate-500/70">Cari...</span>
          <kbd className="ml-2 text-[9px] bg-slate-200/70 dark:bg-slate-800/70 text-slate-500 dark:text-slate-400 px-1.5 py-0.5 rounded-md font-mono shadow-inner">
            ⌘K
          </kbd>
        </button>
        ...
        {/* Branch Switcher — visible on left for prominence */}
        {currentUser.role !== UserRole.SUPER_ADMIN && tenantBranches.length > 0 && (
          <div className="flex items-center gap-1 bg-slate-100/50 dark:bg-slate-900/40 border border-slate-200/40 dark:border-slate-800/80 rounded-lg px-1.5 sm:px-2 py-1 text-slate-700 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-900 transition-all shrink-0">
            ...
            <select
              ...
              className="text-[10px] sm:text-xs font-bold bg-transparent border-none outline-none text-slate-800 dark:text-slate-100 cursor-pointer pr-1 max-w-[120px] sm:max-w-[180px] truncate [&>option]:bg-white dark:[&>option]:bg-slate-900 [&>option]:text-slate-800 dark:[&>option]:text-slate-100"
              id="branch-switcher-select"
              aria-label="Ganti cabang operasional"
              title="Ganti cabang operasional"
            >
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
        {/* Scrollable icon row - overflow isolated for mobile dropdowns */}
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 overflow-x-auto sm:overflow-visible [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <button
            ...
            title={platformHealth.message || `Status server: ${cloudLabel}`}
            aria-label="Status server"
          ...
          >
            <span className="sr-only">Status server</span>
            <span className="relative flex h-2 w-2 shrink-0">
              {cloudActive && (
                <span className="absolute inline-flex h-full w-full rounded-full opacity-75 animate-ping bg-emerald-400" />
              )}
              <span
                className={`relative inline-flex rounded-full h-2 w-2 ${cloudActive ? 'bg-emerald-500' : cloudChecking ? 'bg-blue-500' : 'bg-amber-500'}`}
              />
            </span>
          ...
          {/* Navigation Mode Switcher - desktop only */}
          {setNavigationMode && (
            <div
              className="flex items-center gap-0.5 rounded-lg border border-slate-200/50 dark:border-slate-800/70 bg-slate-100/50 dark:bg-slate-900/50 p-0.5 shrink-0 hidden md:flex"
              id="topbar-nav-mode-switcher"
            >
              {[
        ...
        {currentUser.role === UserRole.SUPER_ADMIN && (
          <div className="relative">
            <button
              ...
              aria-label="Notifikasi Super Admin"
              className="relative rounded-lg border border-slate-200/40 bg-slate-100/50 p-2 text-slate-500 dark:border-slate-800/80 dark:bg-slate-900/40 dark:text-slate-400"
              id="superadmin-notifications-btn"
            >
              <Bell className="h-4 w-4" />
              {unreadNotifications.length > 0 && (
                <span className="absolute -right-1.5 -top-1.5 grid min-h-4 min-w-4 place-items-center rounded-full bg-rose-600 px-1 text-[9px] font-black text-white">
                  {Math.min(99, unreadNotifications.length)}
                </span>
              )}
            </button>
            {isNotificationsOpen && (
              <div className="absolute right-0 top-full z-50 mt-2 w-80 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-900">
                <div className="flex items-center justify-between border-b border-slate-100 p-3 dark:border-zinc-800">
                  <b className="text-xs text-slate-900 dark:text-white">Notifikasi Operasional</b>
                  ...
                  {notifications.length ? (
                    notifications.map((item) => (
                      <button
                        ...
                        onClick={async () => {
                          if (!item.read_at)
                            await apiFetch(`/api/superadmin/notifications/${item.id}/read`, {
                              method: 'POST',
                            });
                          await loadNotifications();
                        }}
                        className="mb-1 w-full rounded-xl p-3 text-left hover:bg-slate-50 dark:hover:bg-zinc-950"
                      >
                        <span className="flex items-start gap-2">
                          {item.read_at ? (
                            <CheckCircle2 className="mt-0.5 h-4 w-4 text-slate-300" />
                          ) : (
                    ...
                    <p className="py-8 text-center text-xs text-slate-500">Belum ada notifikasi.</p>
                  )}
                </div>
              </div>
            )}
