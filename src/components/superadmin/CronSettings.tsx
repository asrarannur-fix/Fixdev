...
const CRON_JOBS = [
  {
    id: 'notify-due-reminders',
    label: 'Kirim Pengingat Jatuh Tempo',
    desc: 'Kirim notifikasi H-7, H-3, H-1 ke tenant dengan invoice unpaid',
  },
  {
    id: 'notify-overdue-alerts',
    label: 'Kirim Alert Overdue',
    desc: 'Kirim peringatan ke tenant dengan invoice overdue',
  },
  {
    id: 'notify-overdue-deep',
    label: 'Alert Overdue lanjutan (>3 hari)',
    desc: 'Kirim email ke tenant dengan invoice >3 hari overdue',
  },
  {
  ...
  },
  {
    ...
    desc: 'Kirim notifikasi pembayaran dikonfirmasi ke tenant',
  },
  {
    id: 'simulate-recurring-cron',
    label: 'Auto-Renew Subscription',
    desc: 'Buat invoice renewal untuk langganan bulanan/tahunan yang expired',
  },
  {
    id: 'simulate-trial-expiry',
    label: 'Proses Trial Berakhir',
    desc: 'Flip TRIAL→EXPIRED + tier→BASIC untuk trial yang expired',
  },
];
...
export const CronSettings: React.FC = () => {
  ...
  const runCron = async (jobId: string) => {
    try {
      ...
      showToast(`Cron job ${jobId} berhasil dijalankan`, 'success');
    } catch (err) {
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {log.logs && log.logs.length > 0 && (
                      <button
                        onClick={() =>
                          setExpandedLog(expandedLog === log.jobName + i ? null : log.jobName + i)
                        }
