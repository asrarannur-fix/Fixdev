...
export const TenantsManager: React.FC<TenantsManagerProps> = ({
    ...
    try {
      if (action === 'revoke' && invitation) {
        await readJsonResponse(
          await apiFetch(
            `/api/superadmin/tenants/${detailTenant.id}/invitations/${invitation.id}`,
            { method: 'DELETE' }
          ),
          'Cabut undangan'
        );
        ...
        if (!previous.revokedAt && !previous.acceptedAt)
          await readJsonResponse(
            await apiFetch(
              `/api/superadmin/tenants/${detailTenant.id}/invitations/${previous.id}`,
              { method: 'DELETE' }
            ),
            'Cabut undangan lama'
          );
        await readJsonResponse(
          await apiFetch(`/api/superadmin/tenants/${detailTenant.id}/invitations`, {
            method: 'POST',
            body: JSON.stringify({
              name: previous.name,
      ...
      showToast(
        action === 'revoke' ? 'Undangan dicabut.' : 'Undangan baru dimasukkan ke antrean delivery.',
        'success'
      );
    } catch (error: any) {
  ...
  const openTenantDetail = async (tenant: Tenant) => {
    ...
    try {
      const [detailResponse, invitationResponse, operational] = await Promise.all([
  ...
  const [newTier, setNewTier] = useState<SubscriptionTier>(SubscriptionTier.PRO);
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const data = await readJsonResponse<any>(
          await apiFetch(`/api/superadmin/tenants?${params}`),
          'Daftar tenant'
        );
  ...
  const exportTenants = () => {
    const rows = serverTenants.map((tenant) => [
      tenant.name,
      tenant.subdomain,
      tenant.status,
      tenant.tier,
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        `Tenant "${cleanTenantName}" berhasil dibuat. Undangan dikirim ke ${cleanOwnerEmail}.`,
        'success'
      );
      setNewTenantName('');
      setNewOwnerName('');
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="sa-middle-layout">
      {/* Tenants List */}
      <div className="lg:col-span-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-sm flex flex-col h-fit overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between bg-white dark:bg-zinc-900">
          <div>
            <h3 className="font-extrabold text-sm text-slate-800 dark:text-zinc-200 uppercase tracking-wider flex items-center gap-2">
              <Server className="w-4 h-4 text-purple-600" /> Tenant Management Console
            </h3>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
              Kontrol lisensi, alokasi kapasitas storage, impersonate aman, dan suspensi darurat
              real-time.
            </p>
          </div>
        ...
        <div className="px-6 py-3 border-b border-slate-100 dark:border-zinc-800 flex flex-wrap items-center gap-3 bg-white dark:bg-zinc-900">
          ...
          <button
            ...
            className="rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 hover:bg-slate-50 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-950"
          >
            Reset
          </button>
          <button
            ...
            className="inline-flex items-center gap-1 rounded-xl bg-slate-900 px-3 py-1.5 text-xs font-bold text-white dark:bg-zinc-100 dark:text-zinc-900"
          >
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
              {tenantListLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-5 py-10 text-center">
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-zinc-500 mt-1">
                      {tenants.length === 0
                        ? 'Daftarkan tenant baru dari panel registrasi.'
                        : 'Ubah kriteria pencarian atau filter.'}
                    </p>
                  </td>
                </tr>
              ) : (
                visibleTenants.map((t) => {
                  ...
                  const rowUsageSource =
                    (t as any).usage?.source === 'actual' ? 'aktual' : 'estimasi';
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-4">
                        <p className="font-bold text-slate-800 dark:text-slate-200 text-xs">
                          <button
                            ...
                            className="text-left hover:text-accent hover:underline dark:hover:text-indigo-400"
                          >
                            {t.name || 'Tenant tanpa nama'}
                          </button>
                        </p>
                      ...
                      <td className="px-5 py-4 w-44">
                        ...
                        <div
                          className="mt-2 grid grid-cols-2 gap-1 text-[9px] font-mono"
                          id={`sa-tenant-activity-${t.id}`}
                        >
                          <span className="rounded-lg bg-slate-50 dark:bg-zinc-950 px-2 py-1 text-slate-500 dark:text-zinc-400">
                            TX {(t as any).transactionCount || 0}
                          </span>
                          <span className="rounded-lg bg-slate-50 dark:bg-zinc-950 px-2 py-1 text-slate-500 dark:text-zinc-400">
                            Svc {(t as any).serviceCount || 0}
                          </span>
                          <span className="rounded-lg bg-slate-50 dark:bg-zinc-950 px-2 py-1 text-slate-500 dark:text-zinc-400">
                            User {(t as any).userCount || 0}
                          </span>
                          <span className="rounded-lg bg-slate-50 dark:bg-zinc-950 px-2 py-1 text-slate-500 dark:text-zinc-400">
                            Cab {(t as any).branchCount || 0}
                          </span>
                        </div>
                      </td>
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                                t.status === 'SUSPENDED' ? 'bg-rose-500' : 'bg-emerald-500'
                              }`}
                            >
                              <span
                                className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                                  t.status === 'SUSPENDED' ? 'translate-x-5' : 'translate-x-0'
                                }`}
                              />
                            </button>
                            <span
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setConfigStorageLimitMb(t.limits?.storageMb || 1024);
                          ...
                          >
                            ...
                            {isDeletingTenant ? 'Menghapus...' : 'Hapus Permanen'}
                          </button>
                        </div>
                      </td>
                    </tr>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 px-6 py-3 text-xs text-slate-500 dark:border-zinc-800 dark:text-zinc-400">
          <span>
            {serverTotal} tenant · Halaman {page} dari {totalPages}
          </span>
          <div className="flex gap-2">
            <button
              ...
              className="rounded-lg border border-slate-200 p-2 disabled:opacity-40 dark:border-zinc-700"
            >
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-sm p-6 h-fit">
        <h3 className="font-bold text-xs uppercase text-slate-800 dark:text-zinc-200 tracking-wider mb-4 flex items-center gap-2">
          <Settings2 className="w-4 h-4 text-slate-400 dark:text-zinc-500" /> Registrasi Tenant Baru
        </h3>
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Owner akan menerima undangan untuk membuat sandi sendiri. Undangan kedaluwarsa dalam
              48 jam.
            </p>
          </div>
          <div>
            ...
            >
              <option value={SubscriptionTier.BASIC}>BASIC (15 user, Rp 100k/bln)</option>
              <option value={SubscriptionTier.PRO}>PRO (15 user, Rp 250k/bln)</option>
          ...
          >
            ...
            {isRegistering ? 'Mendaftarkan...' : 'Daftarkan Tenant'}
          </button>
        </form>
      </div>
      {detailTenant && (
        <div
          className="fixed inset-0 z-[9999] flex justify-end bg-slate-950/40"
          role="dialog"
          aria-modal="true"
        ...
        >
          <section className="h-full w-full max-w-3xl overflow-y-auto bg-white shadow-2xl dark:bg-zinc-900">
            <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
              <div>
                <h3
                ...
                <p className="text-xs text-slate-500">
                  {detailTenant.subdomain} · {detailTenant.status} · {detailTenant.tier}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setDetailTenant(null)}
                className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold dark:border-zinc-700"
              >
                Tutup
              </button>
            </header>
            <nav
              className="flex gap-1 overflow-x-auto border-b border-slate-200 p-3 dark:border-zinc-800"
            ...
            >
              {[
                ...
                <button
                  ...
                  className={`whitespace-nowrap rounded-xl px-3 py-2 text-xs font-bold ${detailTab === id ? 'bg-accent text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-zinc-300 dark:hover:bg-zinc-800'}`}
                >
                  {label}
                </button>
              ))}
            ...
            <div className="p-5">
              {detailLoading ? (
                <p className="py-12 text-center text-sm text-slate-500">Memuat detail tenant…</p>
              ) : !detailData ? (
                <p className="py-12 text-center text-sm text-slate-500">Detail belum tersedia.</p>
              ) : (
                <>
                  {detailTab === 'summary' && (
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {[
                        ['Status', detailData.tenant.status],
                        ['Paket', detailData.tenant.tier],
                        ['Pengguna', detailData.users.length],
                        ['Invoice', detailData.invoices.length],
                      ].map(([label, value]) => (
                        <div
                          key={String(label)}
                          className="rounded-xl bg-slate-50 p-4 dark:bg-zinc-950"
                        >
                  ...
                  {detailTab === 'users' && (
                    <div className="space-y-3">
                      {detailData.users.length ? (
                        detailData.users.map((user: any) => (
                          <div
                            key={user.id}
                            className="flex justify-between rounded-xl border border-slate-200 p-3 text-xs dark:border-zinc-800"
                          >
                            <span>
                              <b>{user.name}</b>
                              <br />
                        ...
                        <p className="text-sm text-slate-500">Belum ada user aktif.</p>
                      )}
                      <div className="border-t border-slate-200 pt-3 dark:border-zinc-800">
                        <h4 className="text-xs font-black">Undangan Owner</h4>
                        {detailData.invitations.length ? (
                          detailData.invitations.map((invite: any) => (
                            <div
                              key={invite.id}
                              className="mt-2 flex items-center justify-between rounded-xl bg-slate-50 p-3 text-xs dark:bg-zinc-950"
                            >
                              <span>
                                <b>{invite.name}</b>
                                <br />
                                ...
                                <span className="text-slate-400">
                                  {invite.acceptedAt
                                    ...
                                    : invite.revokedAt
                                      ? 'Dicabut'
                                      : `Aktif sampai ${new Date(invite.expiresAt).toLocaleString('id-ID')}`}
                                </span>
                              </span>
                              <span className="flex gap-1">
                                {!invite.acceptedAt && !invite.revokedAt && (
                                  <button
                                    type="button"
                                    disabled={readOnlyMode}
                                    onClick={() => manageInvitation('revoke', invite)}
                                    className="rounded-lg border border-rose-200 px-2 py-1 text-rose-700 disabled:opacity-40"
                                  >
                                    Cabut
                                  </button>
                                )}
                                <button
                                  type="button"
                                  disabled={readOnlyMode || Boolean(invite.acceptedAt)}
                                  onClick={() => manageInvitation('resend', invite)}
                          ...
                          <p className="mt-2 text-xs text-slate-500">Belum ada undangan.</p>
                        )}
                      </div>
                    </div>
                  )}
                  {detailTab === 'billing' && (
                    <div className="space-y-2">
                      {detailData.invoices.length ? (
                        detailData.invoices.map((invoice: any) => (
                          <div
                            key={invoice.id}
                            className="flex justify-between rounded-xl border border-slate-200 p-3 text-xs dark:border-zinc-800"
                          >
                            <span>
                              <b>{invoice.id}</b>
                              <br />
                              {invoice.tier} · {invoice.billingCycle}
                            </span>
                  ...
                  {detailTab === 'infrastructure' && (
                    <dl className="grid gap-3 text-sm">
                      <div>
                        ...
                        <dd className="font-bold">{detailData.tenant.subdomain}</dd>
                      </div>
                      <div>
                        <dt className="text-slate-500">Storage aktual</dt>
                        <dd className="font-bold">
                          {detailData.tenant.storageUsedBytes == null
                            ? 'Belum diukur'
                            : `${Math.round(Number(detailData.tenant.storageUsedBytes) / 1048576)} MB`}
                        </dd>
                      </div>
                    </dl>
                  )}
                  {detailTab === 'features' && (
                    <div className="flex flex-wrap gap-2">
                      {(
                        detailData.tenant.settings?.limits?.features ||
                        detailTenant.limits?.features ||
                        []
                      ).map((feature: string) => (
                        <span
                          key={feature}
                          className="rounded-full bg-accent-lighter px-3 py-1 text-xs font-bold text-accent dark:bg-indigo-950/30 dark:text-indigo-300"
                        >
                          {feature}
                        </span>
                      ))}
                  ...
                  {detailTab === 'operational' && (
                    <div className="grid grid-cols-1 gap-4">
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`text-[10px] font-black px-2 py-1 rounded-lg ${operationalSummary?.health.status === 'ok' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300'}`}
                        >
                          {operationalSummary
                            ? operationalSummary.health.status.toUpperCase()
                            : '-'}{' '}
                          · {operationalSummary ? operationalSummary.health.alertCount : 0} alert
                        </span>
                      </div>
                      {operationalLoading ? (
                        ...
                        <>
                          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                            {(Object.entries(operationalSummary.modules || {}) as any[]).map(
                              ([module, data]) => (
                                <div
                                  key={module}
                                  className="rounded-xl border border-slate-200 p-3 text-xs dark:border-zinc-800"
                                >
                                  ...
                                  <p className="mt-1 font-black text-slate-900 dark:text-white">
                                    {(data as any).open_tickets != null
                                      ? `Tiket ${(data as any).open_tickets}`
                                      : JSON.stringify(data)}
                                  </p>
                                </div>
                              )
                          ...
                          {operationalSummary.alerts?.length ? (
                            <div className="space-y-2">
                              {operationalSummary.alerts.map((alert: any, idx: number) => (
                                <div
                                  key={idx}
                                  className="flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-300"
                                >
                                  <span className="font-bold">
                                    {alert.module}: {alert.label}
                                  </span>
                            ...
                            <p className="text-xs text-slate-500">
                              Tidak ada alert operasional untuk tenant ini.
                            </p>
                          )}
                        </>
                      ) : (
        ...
        >
          <form
            ...
            onSubmit={async (event) => {
              ...
              const nextStatus =
                statusDialogTenant.status === TenantStatus.SUSPENDED
                  ? TenantStatus.ACTIVE
                  : TenantStatus.SUSPENDED;
              try {
                const response = await apiFetch(
                  `/api/superadmin/tenants/${statusDialogTenant.id}/status`,
                  {
                    method: 'POST',
                    headers: { 'X-SuperAdmin-Mode': readOnlyMode ? 'read-only' : 'edit' },
                    body: JSON.stringify({
                      ...
                      scheduledReactivationAt: reactivateAt || null,
                      notifyOwner: true,
                      expectedVersion: Number((statusDialogTenant as any).version || 1),
                    }),
                  }
                );
                await readJsonResponse(response, 'Perubahan status tenant');
                showToast(
                  `Tenant berhasil ${nextStatus === TenantStatus.SUSPENDED ? 'ditangguhkan' : 'diaktifkan'}.`,
                  'success'
                );
          ...
          >
            ...
            <p className="mt-1 text-xs text-slate-500">
              {statusDialogTenant.name} · tindakan ini tercatat di audit.
            </p>
            <div className="mt-5 grid gap-4">
              <label className="text-xs font-bold text-slate-600 dark:text-zinc-300">
                ...
                <select
                  ...
                  className="mt-1 w-full rounded-xl border border-slate-200 bg-white p-2.5 dark:border-zinc-700 dark:bg-zinc-950"
                >
                  <option value="BILLING">Pembayaran</option>
                  <option value="SECURITY">Keamanan</option>
                  <option value="ABUSE">Penyalahgunaan</option>
                  <option value="TENANT_REQUEST">Permintaan tenant</option>
                </select>
              </label>
              <label className="text-xs font-bold text-slate-600 dark:text-zinc-300">
                Alasan
              ...
              <label className="text-xs font-bold text-slate-600 dark:text-zinc-300">
                ...
                <textarea
                  ...
                  className="mt-1 min-h-20 w-full rounded-xl border border-slate-200 bg-white p-2.5 dark:border-zinc-700 dark:bg-zinc-950"
                />
              </label>
              {statusDialogTenant.status !== TenantStatus.SUSPENDED && (
                <label className="text-xs font-bold text-slate-600 dark:text-zinc-300">
                  Aktif kembali otomatis (opsional)
                  <input
                    type="datetime-local"
                    value={reactivateAt}
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                ...
                className="rounded-xl border border-slate-200 px-4 py-2 text-xs font-bold dark:border-zinc-700"
              >
                Batal
              </button>
              <button
      ...
      {impersonationTenant && (
        ...
        >
          <form
            ...
            onSubmit={async (event) => {
              ...
              if (!impersonationReason.trim())
                return showToast('Alasan akses wajib diisi.', 'error');
              try {
                const response = await apiFetch(
                  `/api/superadmin/tenants/${impersonationTenant.id}/impersonation`,
                  {
                    method: 'POST',
                    headers: { 'X-SuperAdmin-Mode': readOnlyMode ? 'read-only' : 'edit' },
                    body: JSON.stringify({
                      reason: impersonationReason,
                      ticketId: impersonationTicket || null,
                      accessMode: impersonationMode,
                      durationMinutes: 30,
                    }),
                  }
                );
          ...
          >
            <h3
              id="impersonation-title"
              className="text-lg font-black text-slate-900 dark:text-white"
            >
              Akses tenant secara aman
            </h3>
            <p className="mt-1 text-xs text-slate-500">
              {impersonationTenant.name} · sesi otomatis berakhir dalam 30 menit.
            </p>
            <div className="mt-5 grid gap-4">
              <label className="text-xs font-bold text-slate-600 dark:text-zinc-300">
                Alasan
              ...
              <label className="text-xs font-bold text-slate-600 dark:text-zinc-300">
                ...
                <input
                  ...
                  onChange={(event) => setImpersonationTicket(event.target.value)}
                  className="mt-1 w-full rounded-xl border border-slate-200 p-2.5 dark:border-zinc-700 dark:bg-zinc-950"
                />
              </label>
              <label className="text-xs font-bold text-slate-600 dark:text-zinc-300">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setImpersonationMode(event.target.value as 'READ_ONLY' | 'FULL')
                  }
                  className="mt-1 w-full rounded-xl border border-slate-200 p-2.5 dark:border-zinc-700 dark:bg-zinc-950"
                >
