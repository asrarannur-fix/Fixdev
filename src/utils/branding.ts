...
export const PLATFORM_FAVICON =
  "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Crect width='32' height='32' rx='6' fill='%236366f1'/%3E%3Ctext x='16' y='22' text-anchor='middle' fill='white' font-family='system-ui' font-weight='900' font-size='16'%3EFD%3C/text%3E%3C/svg%3E";
...
export function applyTenantBranding(
  ...
  const primary = validColor(branding.primaryColor)
    ? normalizeHex(branding.primaryColor!)
    : DEFAULT_PRIMARY;
  const secondary = validColor(branding.secondaryColor)
    ? normalizeHex(branding.secondaryColor!)
    : undefined;

  root.style.setProperty('--accent', primary);
  root.style.setProperty(
    '--accent-hover',
    secondary || blendColor(primary, isDark ? 'white' : 'black', 15)
  );
  root.style.setProperty(
    '--accent-light',
    isDark ? blendColor(primary, 'white', 18) : blendColor(primary, 'white', 92)
  );
  root.style.setProperty(
    '--accent-lighter',
    isDark ? blendColor(primary, 'white', 10) : blendColor(primary, 'white', 96)
  );
