...
declare global {
  interface Window {
    __splitPaymentDetails?: string;
  }
}
import { Barcode, Lock, CheckCircle2, History, X, Printer, Store, DollarSign } from 'lucide-react';
import {
  InventoryProduct,
  PaymentMethod,
  Customer,
...
export const POSTab: React.FC<POSTabProps> = ({
  ...
  const canViewInvoice =
    ...
    currentUserPermissions?.includes('action-pos-invoice-view');
  const canVoidTransaction =
    currentUser?.role === UserRole.OWNER ||
    currentUserPermissions?.includes('action-pos-void-approve');
  ...
  const [internalCart, setInternalCart] = useState<
    { product: InventoryProduct; qty: number; discount: number }[]
  >([]);
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && barcodeScan.trim()) {
                        const found = effectiveProducts.find(
                          (p: any) =>
                            p.sku === barcodeScan.trim() || p.barcode === barcodeScan.trim()
                        );
                        if (found) {
                          ...
                          showToast(
                            `Produk dengan SKU/barcode "${barcodeScan}" tidak ditemukan.`,
                            'error'
                          );
                        }
                      }
                  ...
                  <div className="space-y-1.5 max-h-24 overflow-y-auto pr-1">
                    {heldCarts.map((hc) => {
                      ...
                      return (
                        <div
                          key={hc.id}
                          className="flex justify-between items-center text-[10px] bg-white p-2 border border-slate-200 rounded-lg shadow-2xs"
                        >
                          <div>
              ...
              <div className="space-y-3 max-h-48 overflow-y-auto pr-1 mb-4">
                {effectiveCart.length === 0 ? (
                  ...
                  effectiveCart.map((item, idx) => (
                    ...
                    >
                      <div>
                        <p className="font-bold text-slate-700 leading-snug">{item.product.name}</p>
                        <p className="text-slate-500 text-[11px] mt-0.5 font-mono">
                          {item.qty} x Rp {(item.product.sellPrice ?? 0).toLocaleString()}
                        </p>
                      </div>
                      <span className="font-semibold text-slate-800 font-mono shrink-0 pl-2">
                        Rp {((item.product.sellPrice ?? 0) * item.qty).toLocaleString()}
                      </span>
                    </div>
                  ))
                )}
                ...
                <div className="space-y-2 border-t border-slate-100 pt-3 text-xs">
                  <div className="flex justify-between text-slate-500">
                    ...
                    <span className="font-mono">
                      Rp{' '}
                      {effectiveCart
                        .reduce((sum, item) => sum + (item.product.sellPrice ?? 0) * item.qty, 0)
                        .toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between text-slate-500">
                  ...
                  <div className="space-y-3 pt-3">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value={PaymentMethod.QRIS}>QRIS (AUTO SETTLEMENT)</option>
                        <option value={PaymentMethod.DEPOSIT}>DEPOSIT / STORE CREDIT</option>
                      </select>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <label
                        ...
                        className="text-[10px] font-mono text-slate-500 uppercase cursor-pointer"
                      >
                        Bayar Ganda (Split)
                      </label>
                    </div>
                      ...
                      <div className="space-y-2 bg-slate-50 rounded-xl p-2.5 border border-slate-200">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={splitAmount}
                            onChange={(e) =>
                              setSplitAmount(String(Math.max(0, Number(e.target.value) || 0)))
                            }
                        ...
                        <p className="text-[9px] text-emerald-600 font-bold">
                          Total: Rp{' '}
                          {(
                            (Number(posAmountPaid) || 0) + (Number(splitAmount) || 0)
                          ).toLocaleString()}
                        </p>
                      </div>
                    )}
            ...
            {activeShift ? (
              <div className="space-y-4">
                <div className="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg text-xs leading-relaxed">
                  ...
                  <strong>Rp {(activeShift?.startingCash ?? 0).toLocaleString()}</strong>
                </div>
                <div>
                  <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">
          ...
          <div className="space-y-6">
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-3">
                ...
                <span className="text-xs text-slate-400 font-mono">
                  {transactions.filter((tx) => tx.tenantId === currentTenantId).length} Transaksi
                  terdaftar
                </span>
              </div>
              ...
              <div className="responsive-table-container max-h-[500px]">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {transactions.filter((tx) => tx.tenantId === currentTenantId).length === 0 ? (
                      <tr>
            ...
            {selectedTxId &&
              (() => {
                ...
                if (!tx) return null;
                const custName = customers.find((c) => c.id === tx.customerId)?.name || 'Umum';
                return (
                  <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm max-w-xl animate-fadeIn space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs">
                      <div>
                        <p className="text-slate-400 font-mono text-[10px] uppercase">Pelanggan</p>
                        <p className="font-bold text-slate-700 mt-1">{custName}</p>
                      </div>
                      <div>
                        <p className="text-slate-400 font-mono text-[10px] uppercase">
                          Metode Pembayaran
                        </p>
                        <p className="font-bold text-slate-700 mt-1">{tx.paymentMethod}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5 text-xs text-slate-500 pt-1">
                      <div className="flex justify-between">
                        <span>Subtotal:</span>
                        <span className="font-mono">Rp {(tx.subtotal ?? 0).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Diskon:</span>
                      ...
                      <div className="flex justify-between">
                        <span>PPN (11%):</span>
                        <span className="font-mono">Rp {(tx.taxAmount ?? 0).toLocaleString()}</span>
                      </div>
                      {(tx.depositUsed ?? 0) > 0 && (
                        <div className="flex justify-between">
                      ...
                      <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-3">
                        ...
                        {canVoidTransaction ? (
                            ...
                            <button
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  showToast(
                                    'Nota transaksi berhasil di-void! Jurnal pembalik otomatis dipos ke ledger.',
                                    'success'
                                  );
                          ...
                          <div className="bg-amber-50 border border-amber-200 p-3 rounded-lg text-[11px] text-amber-800 leading-normal space-y-1">
                            ...
                            <p>
                              Peran Anda (
                              <strong className="font-mono uppercase">{currentUser.role}</strong>)
                              tidak diberikan otorisasi{' '}
                              <strong className="font-mono">
                                Approve Refund / Void Transaction
                              </strong>
                              . Silakan minta Owner atau Admin untuk memberikan izin pada menu
                              Matriks RBAC.
                            </p>
                          </div>
                        )}
