...
const modules = [
  { icon: Wrench, title: "Servis terpadu", desc: "Penerimaan, diagnosa, persetujuan, QC, sampai serah-terima.", tone: "bg-accent-lighter text-accent" },
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-slate-50 text-slate-900" id="landing-page">
      ...
      <nav className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
          <button onClick={() => scrollTo("landing-page")} className="flex items-center gap-2.5 font-black tracking-tight" aria-label="FixDev beranda">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-accent text-white shadow-lg shadow-accent/20"><Wrench className="h-4 w-4" /></span>
            <span>FixDev</span>
          </button>
          <div className="hidden items-center gap-6 md:flex">
            <button onClick={() => scrollTo("fitur")} className="text-xs font-bold text-slate-600 hover:text-accent">Fitur</button>
            <button onClick={() => scrollTo("cara-kerja")} className="text-xs font-bold text-slate-600 hover:text-accent">Cara kerja</button>
            <button onClick={() => scrollTo("paket")} className="text-xs font-bold text-slate-600 hover:text-accent">Paket</button>
            <button onClick={() => setShowLogin(true)} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-black text-slate-700 shadow-sm transition hover:border-indigo-200 hover:bg-accent-lighter">{isAuthenticated ? "Buka dashboard" : "Masuk"}</button>
            <button onClick={() => setShowLogin(true)} className="rounded-xl bg-accent px-4 py-2 text-xs font-black text-white shadow-sm shadow-accent/20 transition hover:bg-accent-hover">Mulai gratis <ArrowRight className="ml-1 inline h-3.5 w-3.5" /></button>
          </div>
      ...
      <section className="relative z-10 mx-auto grid max-w-6xl items-center gap-8 px-4 pb-12 pt-12 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:pb-20 lg:pt-20">
        <div>
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-indigo-100 bg-white px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-accent shadow-sm"><Sparkles className="h-3.5 w-3.5" /> Platform untuk bengkel modern</div>
          <h1 className="max-w-2xl text-4xl font-black leading-[1.03] tracking-tight text-slate-950 sm:text-5xl lg:text-6xl">Operasional bengkel. <span className="text-accent">Satu alur yang jelas.</span></h1>
          ...
          <div className="mt-6 flex flex-col gap-2 sm:flex-row"><button onClick={() => setShowLogin(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-accent px-5 py-3 text-xs font-black text-white shadow-lg shadow-accent/20 transition hover:-translate-y-0.5 hover:bg-accent-hover">Daftarkan toko <ArrowRight className="h-4 w-4" /></button><button onClick={() => scrollTo("fitur")} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-xs font-black text-slate-700 shadow-sm hover:bg-slate-50">Lihat semua fitur <ChevronDown className="h-4 w-4" /></button></div>
        ...
        <div className="relative mx-auto w-full max-w-xl">
          ...
          <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white p-3 shadow-2xl shadow-accent/50">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-[.9fr_1.1fr]">
              ...
              <div className="space-y-3"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold text-slate-500">Selamat pagi, Admin</p><p className="text-sm font-black">Ringkasan hari ini</p></div><span className="grid h-8 w-8 place-items-center rounded-xl bg-accent-lighter text-accent"><Zap className="h-4 w-4" /></span></div><div className="grid grid-cols-2 gap-2"><div className="rounded-xl border border-slate-100 p-2.5"><p className="text-[9px] font-bold text-slate-500">Servis aktif</p><p className="mt-1 text-lg font-black">24</p><p className="text-[9px] font-bold text-emerald-600">+5 hari ini</p></div><div className="rounded-xl border border-slate-100 p-2.5"><p className="text-[9px] font-bold text-slate-500">Penjualan</p><p className="mt-1 text-lg font-black">Rp8,4jt</p><p className="text-[9px] font-bold text-emerald-600">+12,4%</p></div></div><div className="rounded-xl bg-slate-50 p-3"><div className="flex items-center justify-between"><p className="text-[10px] font-black">Tiket terbaru</p><span className="text-[9px] font-bold text-accent">Lihat semua</span></div><div className="mt-2 flex items-center gap-2 rounded-lg bg-white p-2"><span className="grid h-6 w-6 place-items-center rounded-lg bg-amber-50 text-amber-600"><MonitorSmartphone className="h-3 w-3" /></span><div className="min-w-0 flex-1"><p className="truncate text-[9px] font-black">MacBook Pro M1</p><p className="text-[8px] font-bold text-slate-500">Menunggu diagnosa</p></div><span className="h-1.5 w-12 rounded-full bg-amber-300" /></div></div></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="fitur" className="mx-auto max-w-6xl px-4 py-14 sm:px-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[10px] font-black uppercase tracking-widest text-accent">Semua yang dibutuhkan</p><h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Kerja lebih cepat. Data tetap nyambung.</h2></div><p className="max-w-sm text-xs leading-5 text-slate-600">Fitur inti berhubungan langsung, bukan kumpulan aplikasi yang terpisah.</p></div><div className="mt-7 grid gap-3 md:grid-cols-2 lg:grid-cols-3">{modules.map((module) => <article key={module.title} className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-md"><span className={`grid h-10 w-10 place-items-center rounded-xl ${module.tone}`}><module.icon className="h-5 w-5" /></span><h3 className="mt-4 text-sm font-black">{module.title}</h3><p className="mt-1.5 text-xs leading-5 text-slate-600">{module.desc}</p><span className="mt-4 inline-flex items-center gap-1 text-[10px] font-black text-accent opacity-0 transition group-hover:opacity-100">Pelajari modul <ArrowRight className="h-3 w-3" /></span></article>)}</div></section>
      ...
      <section id="paket" className="mx-auto max-w-6xl px-4 py-14 sm:px-6"><div className="mx-auto mb-8 max-w-2xl text-center"><p className="text-[10px] font-black uppercase tracking-widest text-accent">Paket langganan</p><h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Pilih paket yang sesuai toko Anda.</h2><p className="mt-3 text-xs leading-5 text-slate-600">Semua paket sudah termasuk isolasi data per cabang, hak akses per peran, dan dukungan migrasi.</p></div><div className="grid gap-4 lg:grid-cols-3">{plans.map((plan) => <article key={plan.name} className={`relative rounded-3xl border bg-white p-5 shadow-sm transition ${plan.highlight ? "border-accent/50 shadow-lg shadow-accent/10 ring-1 ring-indigo-200" : "border-slate-200"}`}><div className="flex items-center justify-between"><h3 className="text-sm font-black">{plan.name}</h3>{plan.highlight && <span className="rounded-full bg-accent px-2 py-0.5 text-[9px] font-black text-white">Terpopuler</span>}</div><p className="mt-3"><span className="text-2xl font-black">{plan.price}</span><span className="text-[10px] font-bold text-slate-500">{plan.period}</span></p><button onClick={() => setShowLogin(true)} className={`mt-4 w-full rounded-xl py-2.5 text-xs font-black ${plan.highlight ? "bg-accent text-white hover:bg-accent-hover" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}>{plan.highlight ? "Mulai sekarang" : "Pilih paket"}</button><ul className="mt-4 space-y-2 text-[11px] font-bold text-slate-700">{plan.points.map((point) => <li key={point} className="flex items-center gap-2"><CircleCheck className="h-3.5 w-3.5 text-emerald-600" />{point}</li>)}</ul></article>)}</div></section>

      <section className="mx-auto max-w-6xl px-4 pb-14 sm:px-6"><div className="rounded-3xl border border-indigo-100 bg-gradient-to-br from-indigo-50 via-white to-cyan-50 p-5 sm:p-7"><div className="grid items-center gap-6 lg:grid-cols-[1fr_.9fr]"><div><p className="text-[10px] font-black uppercase tracking-widest text-accent">Siap mulai?</p><h2 className="mt-2 text-3xl font-black tracking-tight">Satu tempat untuk mengelola bengkel Anda.</h2><p className="mt-3 max-w-xl text-xs leading-5 text-slate-600">Buat toko, atur cabang, undang tim, lalu mulai terima pekerjaan. Data cabang terisolasi dari awal.</p><div className="mt-5 grid grid-cols-2 gap-2 text-[11px] font-bold text-slate-700">{capabilities.map((item) => <p key={item} className="flex items-center gap-1.5"><CircleCheck className="h-3.5 w-3.5 text-emerald-600" />{item}</p>)}</div></div><div className="rounded-2xl bg-white p-4 shadow-lg shadow-accent/10"><p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Paket awal</p><p className="mt-2 text-2xl font-black">Mulai sesuai kebutuhan</p><p className="mt-2 text-xs leading-5 text-slate-600">Daftar toko baru dan konfigurasi dasar langsung tersedia.</p><button onClick={() => setShowLogin(true)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-accent px-4 py-3 text-xs font-black text-white hover:bg-accent-hover">Daftarkan toko <ArrowRight className="h-4 w-4" /></button><p className="mt-3 text-center text-[10px] font-bold text-slate-500"><Check className="mr-1 inline h-3 w-3 text-emerald-600" />Tanpa input kartu di awal</p></div></div></div></section>

      <footer className="border-t border-slate-200 bg-white"><div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-6 text-[10px] font-bold text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:px-6"><span>© 2026 FixDev</span><span className="flex items-center gap-1.5"><MessageSquare className="h-3.5 w-3.5 text-accent" />Operasional lebih rapi, satu alur.</span></div></footer>
    </main>
  );
};
