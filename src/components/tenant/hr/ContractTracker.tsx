...
export const ContractTracker: React.FC<ContractTrackerProps> = ({ activeSubTab }) => {
  ...
  const handleExtend = (empId: string) => {
    ...
    showToast(
      `Kontrak ${emp.name} berhasil diperpanjang hingga ${fmtDate(extendEndDate)}.`,
      'success'
    );
    setExtendEmpId(null);
    setExtendEndDate('');
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Monitor masa berlaku kontrak dan masa kerja seluruh karyawan aktif.
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200">
                {['Nama Karyawan', 'Status', 'Mulai', 'Berakhir'].map((h) => (
                  <th
                    key={h}
                    className="py-3 px-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {contractHistory.map((row, idx) => {
                const badge =
                  STATUS_BADGE[row.status as Employee['contractStatus']] || STATUS_BADGE.CONTRACT;
                return (
