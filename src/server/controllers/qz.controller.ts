...
const refreshSigningConfig = () => {
  ...
  cachedEnabled = Boolean(
    cachedCert?.startsWith('-----BEGIN') && cachedKey?.startsWith('-----BEGIN')
  );
};
...
export const qzSignHandler = async (req: Request, res: Response): Promise<void> => {
  if (!cachedEnabled) {
    await recordAuditEvent(
      ...
      'QZ_SIGN_FAILED',
      'QZ signing failed: not configured.',
      { dataPreview: req.body?.data?.slice(0, 100) }
    );
    res.status(503).json({ error: 'QZ signing belum dikonfigurasi' });
    return;
  }
  ...
  if (!data || typeof data !== 'string' || data.length > 100_000) {
    await recordAuditEvent(
      ...
      req.authActor?.userId || null,
      'QZ_SIGN_FAILED',
      'QZ signing failed: invalid payload.',
  ...
  try {
    ...
    await recordAuditEvent(
      ...
      req.authActor?.userId || null,
      'QZ_SIGN_FAILED',
      `QZ signing failed: ${err.message}`,
      { dataPreview: data.slice(0, 100), error: err.message }
    );
