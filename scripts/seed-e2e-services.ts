...
if (process.env.NODE_ENV !== 'test' || process.env.E2E_SEED_CONFIRM !== 'devtes') {
  throw new Error('E2E seed requires NODE_ENV=test and E2E_SEED_CONFIRM=devtes.');
}
if (!databaseUrl || !password) throw new Error('DATABASE_URL and TEST_OWNER_PASSWORD are required.');

const parsedUrl = new URL(databaseUrl);
const databaseName = parsedUrl.pathname.replace(/^\//, '');
if (!['fixdev_e2e', 'fixdev_test'].includes(databaseName)) {
  throw new Error('E2E seed only permits dedicated fixdev_e2e or fixdev_test database.');
}
...
try {
  ...
  await client.query('SELECT pg_advisory_xact_lock(hashtext($1))', ['fixdev-e2e-devtes']);
  ...
  await client.query(
    `INSERT INTO users (id,tenant_id,email,name,role,permissions,password_hash)
     VALUES ($1,$2,'devtes1@mail.com','E2E Owner','OWNER','{}',$3)
    ...
    [ids.user, ids.tenant, passwordHash]
  );
  await client.query(
