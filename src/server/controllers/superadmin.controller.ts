...
async function sendInvitationEmail(
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject: `Undangan Owner ${tenantName}`,
    html: `<p>Halo ${ownerName},</p><p>Anda diundang sebagai owner <strong>${tenantName}</strong>.</p><p><a href="${inviteUrl}">Terima undangan dan buat password</a></p><p>Berlaku sampai ${expiresAt}.</p>`,
  });
  return true;
}
...
export async function getOverview(req: Request, res: Response) {
  try {
    const [billing, tenants, trials, manual, incidents, notifications, analytics] =
      await Promise.all([
        dbQuery(`SELECT
        ...
        FROM tenants`),
        dbQuery(
          `SELECT COUNT(*) FILTER (WHERE status='SUBMITTED')::int AS pending FROM manual_payment_requests`
        ).catch(() => ({ rows: [{ pending: 0 }] }) as any),
        dbQuery(`SELECT id, component, severity, title, details, opened_at AS "openedAt"
        FROM platform_incidents WHERE status='OPEN' ORDER BY opened_at DESC LIMIT 10`).catch(
          () => ({ rows: [] }) as any
        ),
        dbQuery(`SELECT COUNT(*) FILTER (WHERE read_at IS NULL)::int AS unread
        FROM billing_internal_notifications WHERE audience_role='SUPER_ADMIN'`).catch(
          () => ({ rows: [{ unread: 0 }] }) as any
        ),
        dbQuery(`SELECT 
    ...
    const actions = [
      Number(manual.rows[0]?.pending) > 0 && {
        id: 'manual-payments',
        severity: 'critical',
        count: Number(manual.rows[0].pending),
        label: 'Pembayaran manual menunggu verifikasi',
        targetTab: 'saas-billing',
        targetFilter: 'manual-pending',
      },
      Number(b.overdue_count) > 0 && {
        id: 'overdue',
        severity: 'critical',
        count: Number(b.overdue_count),
        label: 'Invoice melewati jatuh tempo',
        targetTab: 'saas-billing',
        targetFilter: 'overdue',
      },
      Number(trial.warning) > 0 && {
        id: 'trial-warning',
        severity: 'warning',
      ...
      ...incidents.rows.map((incident: any) => ({
        id: incident.id,
        severity: incident.severity === 'CRITICAL' ? 'critical' : 'warning',
        count: 1,
        label: incident.title,
        targetTab: 'saas-operations',
        targetFilter: 'incidents',
      })),
    ].filter(Boolean);
    ...
    res.json({
      metrics: {
        mrr,
        arr: mrr * 12,
        receivedThisMonth: Number(b.received_month || 0),
        outstanding: Number(b.outstanding || 0),
        overdueInvoices: Number(b.overdue_count || 0),
        totalTenants: Number(t.total || 0),
        activeTenants: Number(t.active || 0),
        trialTenants: Number(t.trial || 0),
        suspendedTenants: Number(t.suspended || 0),
        pendingManualPayments: Number(manual.rows[0]?.pending || 0),
        trialExpiring: Number(trial.warning || 0),
      ...
      await dbTransaction(async (client) => {
        ...
        await client.query(
          ...
          [
            ...
            tenant.id,
            clientIp(req),
            JSON.stringify({ storageUsedBytes: total }),
          ]
        );
      });
      measured++;
  ...
  try {
    ...
    const items = result.rows.map((row: any) => {
      ...
      const limitMb = Number(
        sanitizedSettings?.limits?.storageMb || sanitizedSettings?.storageMb || 1024
      );
      const estimatedMb = Math.round(
        15.4 +
          row.transactionCount * 0.4 +
          row.serviceCount * 2.5 +
          row.userCount * 1.2 +
          row.branchCount * 5
      );
      const actualMb =
        row.storageUsedBytes == null ? null : Math.round(Number(row.storageUsedBytes) / 1048576);
      return {
        ...row,
        settings: sanitizedSettings,
        branding: row.branding,
        usage: {
          usedMb: actualMb ?? estimatedMb,
          limitMb,
          percent: Math.min(
            100,
            Number((((actualMb ?? estimatedMb) / Math.max(limitMb, 1)) * 100).toFixed(1))
          ),
          source: actualMb == null ? 'estimated' : 'actual',
          measuredAt: row.storageMeasuredAt,
        },
      };
    });
    res.json({ items, page, pageSize, total: Number(count.rows[0]?.count || 0) });
  } catch (err: any) {
    logger.error({ err: err.message }, 'listTenants failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}

export async function listTenantBranches(req: Request, res: Response) {
  const tenantId = req.params.id;
  try {
    const result = await dbQuery(
      `SELECT id, name, address, phone, "isActive", tenant_id AS "tenantId", created_at AS "createdAt"
       FROM branches WHERE tenant_id = $1 ORDER BY name ASC`,
      [tenantId]
    );
    res.json({ branches: result.rows });
  } catch (err: any) {
    logger.error({ err: err.message, tenantId }, 'List tenant branches failed');
    res.status(500).json({ error: 'Gagal memuat data cabang.' });
  }
}
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const locked = await client.query(
        `SELECT id,name,subdomain,status,tier,settings,branding,version FROM tenants WHERE id=$1 FOR UPDATE`,
        [req.params.id]
      );
      ...
      await adminAudit(
        ...
        current.id,
        {
          ...
          storageSettings: current.settings?.storageSettings,
          version: current.version,
        },
        {
          ...updated.rows[0],
          settings: undefined,
          storageSettings: updated.rows[0].settings?.storageSettings,
        },
...
export async function changeTenantStatus(req: Request, res: Response) {
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const locked = await client.query(`SELECT * FROM tenants WHERE id=$1 FOR UPDATE`, [
        req.params.id,
      ]);
...
export async function startImpersonation(req: Request, res: Response) {
  ...
  if (
    !reason?.trim() ||
    !['READ_ONLY', 'FULL'].includes(accessMode) ||
    durationMinutes < 5 ||
    durationMinutes > 120
  ) {
    return res
      .status(422)
      .json({ error: 'Alasan, mode akses, dan durasi 5-120 menit wajib valid.' });
  }
  if (accessMode === 'FULL') {
    const rolePermissions = req.authActor?.superadminRole
      ? await dbQuery(`SELECT permission FROM superadmin_role_permissions WHERE role=$1`, [
          req.authActor.superadminRole,
        ]).catch(() => ({ rows: [] }) as any)
      : { rows: [] };
    const permissions = [
      ...(req.authActor?.permissions || []),
      ...rolePermissions.rows.map((row: any) => row.permission),
    ];
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const ended = await client.query(
        `UPDATE impersonation_sessions SET ended_at=now(),end_reason=$3
...
export async function listAudit(req: Request, res: Response) {
  ...
  if (req.query.search) {
    ...
    clauses.push(
      `(action ILIKE $${params.length} OR resource_id ILIKE $${params.length} OR metadata::text ILIKE $${params.length})`
    );
  }
  const where = clauses.length ? `WHERE ${clauses.join(' AND ')}` : '';
  const sort = validSorts.includes(String(req.query.sort || 'createdAt'))
  ...
  try {
    ...
    const rows = await dbQuery(
      `SELECT a.*,u.name AS actor_name,t.name AS tenant_name
    ...
    logger.error({ err: err.message }, 'listAudit failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function createBackupJob(req: Request, res: Response) {
  ...
  if (
    ...
    return res.status(422).json({ error: 'Metadata snapshot tidak valid untuk dry-run.' });
  try {
    ...
    const row = await dbQuery(
      ...
      [
        ...
        JSON.stringify({ valid: true, dryRun: mode === 'RESTORE_DRY_RUN' }),
        req.authActor?.userId,
      ]
    );
    res.status(201).json({ success: true, job: row.rows[0] });
  } catch (err: any) {
    logger.error({ err: err.message }, 'createBackupJob failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function listBackupJobs(_req: Request, res: Response) {
  try {
    const result = await dbQuery(`SELECT * FROM backup_jobs ORDER BY created_at DESC LIMIT 50`);
    res.json({ jobs: result.rows });
  } catch (err: any) {
    logger.error({ err: err.message }, 'listBackupJobs failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function listNotifications(req: Request, res: Response) {
  try {
    ...
    logger.error({ err: err.message }, 'listNotifications failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function markNotificationRead(req: Request, res: Response) {
  try {
    await dbQuery(`UPDATE billing_internal_notifications SET read_at=now() WHERE id=$1`, [
      req.params.id,
    ]);
    res.json({ success: true });
  } catch (err: any) {
    logger.error({ err: err.message }, 'markNotificationRead failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function listOutbox(_req: Request, res: Response) {
  try {
    const result = await dbQuery(
      `SELECT id,event_key,tenant_id AS "tenantId",channel,recipient,status,attempts,next_attempt_at AS "nextAttemptAt",sent_at AS "sentAt",last_error AS "lastError",created_at AS "createdAt" FROM billing_notification_outbox ORDER BY created_at DESC LIMIT 100`
    );
    res.json({ items: result.rows });
  } catch (err: any) {
    logger.error({ err: err.message }, 'listOutbox failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const row = await client.query(
        `UPDATE billing_notification_outbox SET status='PENDING',attempts=0,next_attempt_at=now(),last_error=NULL WHERE id=$1 AND status='FAILED' RETURNING id,tenant_id`,
        [req.params.id]
      );
    ...
    logger.error({ err: err.message }, 'retryNotification failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function listIncidents(_req: Request, res: Response) {
  try {
    const incidents = await dbQuery(
      `SELECT i.*,u.name AS acknowledged_by_name,r.name AS resolved_by_name FROM platform_incidents i LEFT JOIN users u ON u.id=i.acknowledged_by LEFT JOIN users r ON r.id=i.resolved_by ORDER BY i.opened_at DESC LIMIT 100`
    );
    const events = await dbQuery(
      `SELECT e.*,u.name AS actor_name FROM platform_incident_events e LEFT JOIN users u ON u.id=e.actor_user_id WHERE e.incident_id=ANY($1::uuid[]) ORDER BY e.created_at`,
      [incidents.rows.map((row: any) => row.id)]
    ).catch(() => ({ rows: [] }) as any);
    res.json({
      incidents: incidents.rows.map((incident: any) => ({
        ...incident,
        timeline: events.rows.filter((event: any) => event.incident_id === incident.id),
      })),
    });
  } catch (err: any) {
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
...
export async function createIncident(req: Request, res: Response) {
  ...
  if (
    ...
    return res.status(422).json({ error: 'Komponen, severity, dan judul wajib valid.' });
  try {
    const result = await dbTransaction(async (client) => {
      const row = await client.query(
        `INSERT INTO platform_incidents(component,severity,status,title,details) VALUES ($1,$2,'OPEN',$3,$4) RETURNING *`,
        [component.trim(), severity, title.trim(), details || null]
      );
      await client.query(
        `INSERT INTO platform_incident_events(incident_id,event_type,note,actor_user_id) VALUES ($1,'CREATED',$2,$3)`,
        [row.rows[0].id, details || null, req.authActor?.userId]
      );
      await adminAudit(
        client,
        req,
    ...
    logger.error({ err: err.message }, 'createIncident failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function updateIncident(req: Request, res: Response) {
  ...
  if (!['ACKNOWLEDGE', 'RESOLVE', 'REOPEN', 'NOTE'].includes(action))
    return res.status(422).json({ error: 'Aksi insiden tidak valid.' });
  try {
    const result = await dbTransaction(async (client) => {
      const before = await client.query(`SELECT * FROM platform_incidents WHERE id=$1 FOR UPDATE`, [
        req.params.id,
      ]);
      ...
      if (action === 'ACKNOWLEDGE') {
        await client.query(
          `UPDATE platform_incidents SET acknowledged_at=now(),acknowledged_by=$2,version=version+1 WHERE id=$1`,
          [req.params.id, req.authActor?.userId]
        );
      ...
      if (action === 'RESOLVE') {
        await client.query(
          `UPDATE platform_incidents SET status='RESOLVED',resolved_at=now(),resolved_by=$2,version=version+1 WHERE id=$1`,
          [req.params.id, req.authActor?.userId]
        );
        eventType = 'RESOLVED';
      }
      if (action === 'REOPEN') {
        await client.query(
          `UPDATE platform_incidents SET status='OPEN',resolved_at=NULL,resolved_by=NULL,version=version+1 WHERE id=$1`,
          [req.params.id]
        );
        eventType = 'REOPENED';
      }
      await client.query(
        `INSERT INTO platform_incident_events(incident_id,event_type,note,actor_user_id) VALUES ($1,$2,$3,$4)`,
        [req.params.id, eventType, note, req.authActor?.userId]
      );
      const after = await client.query(`SELECT * FROM platform_incidents WHERE id=$1`, [
        req.params.id,
      ]);
      await adminAudit(
        client,
    ...
    logger.error({ err: err.message }, 'updateIncident failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function getAlertSettings(_req: Request, res: Response) {
  try {
    const result = await dbQuery(
      `SELECT value FROM app_settings WHERE key='superadmin_alert_settings' LIMIT 1`
    );
    res.json({
      settings: result.rows[0]?.value || {
        overdueEnabled: true,
        trialDays: 7,
        queueFailureEnabled: true,
        dailyReportHour: 8,
      },
    });
  } catch (err: any) {
    logger.error({ err: err.message }, 'getAlertSettings failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function updateAlertSettings(req: Request, res: Response) {
  ...
  if (
    !Number.isInteger(settings.trialDays) ||
    settings.trialDays < 1 ||
    settings.trialDays > 30 ||
    !Number.isInteger(settings.dailyReportHour) ||
    settings.dailyReportHour < 0 ||
    settings.dailyReportHour > 23
  )
    return res.status(422).json({ error: 'Pengaturan alert tidak valid.' });
  try {
    await dbQuery(
      `INSERT INTO app_settings(key,value,updated_at) VALUES ('superadmin_alert_settings',$1::jsonb,now()) ON CONFLICT(key) DO UPDATE SET value=$1::jsonb,updated_at=now()`,
      [JSON.stringify(settings)]
    );
    res.json({ success: true, settings });
  } catch (err: any) {
    logger.error({ err: err.message }, 'updateAlertSettings failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function listRolePermissions(_req: Request, res: Response) {
  try {
    const result = await dbQuery(
      `SELECT role, array_agg(permission ORDER BY permission) AS permissions FROM superadmin_role_permissions GROUP BY role ORDER BY role`
    );
    res.json({ roles: result.rows });
  } catch (err: any) {
    logger.error({ err: err.message }, 'listRolePermissions failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function updateRolePermissions(req: Request, res: Response) {
  ...
  if (role === 'ROOT_ADMIN' && req.authActor?.superadminRole !== 'ROOT_ADMIN') {
    return res
      .status(403)
      .json({ error: 'Hanya ROOT_ADMIN yang dapat mengubah permission ROOT_ADMIN.' });
  }
  if (permissions.includes('*') && role !== 'ROOT_ADMIN') {
    return res
      .status(422)
      .json({ error: 'Permission wildcard (*) hanya diizinkan untuk ROOT_ADMIN.' });
  }

  try {
    const result = await dbTransaction(async (client) => {
      const before = await client.query(
        `SELECT role, array_agg(permission ORDER BY permission) AS permissions FROM superadmin_role_permissions WHERE role=$1 GROUP BY role`,
        [role]
      );
      ...
      for (const permission of permissions) {
        await client.query(
          `INSERT INTO superadmin_role_permissions(role,permission) VALUES ($1,$2)`,
          [role, permission]
        );
      }
...
export async function assignSuperAdminRole(req: Request, res: Response) {
  ...
  try {
    const validRole = await dbQuery(
      `SELECT 1 FROM superadmin_role_permissions WHERE role=$1 LIMIT 1`,
      [role]
    );
    ...
    const result = await dbTransaction(async (client) => {
      const before = await client.query(
        `SELECT id,name,email,superadmin_role FROM users WHERE id=$1 AND role='SUPER_ADMIN' FOR UPDATE`,
        [userId]
      );
      ...
      const after = await client.query(
        `UPDATE users SET superadmin_role=$2 WHERE id=$1 RETURNING id,name,email,superadmin_role AS "superadminRole"`,
        [userId, role]
      );
      await adminAudit(
        client,
        req,
        'SUPERADMIN_ROLE_ASSIGNED',
        'user',
        userId,
        null,
        before.rows[0],
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const tenant = await client.query(
        `SELECT status, tier, version FROM tenants WHERE id=$1 FOR UPDATE`,
        [tenantId]
      );
      if (!tenant.rows[0]) return { code: 404, error: 'Tenant tidak ditemukan.' };
...
export async function extendSubscription(req: Request, res: Response) {
  ...
  try {
    const result = await dbTransaction(async (client) => {
      ...
      if (!active.rows[0])
        return { code: 404, error: 'Tidak ada langganan aktif yang bisa diperpanjang.' };

      const oldEnd = new Date(active.rows[0].period_end);
      const newEnd = new Date(oldEnd);
      newEnd.setDate(newEnd.getDate() + days);

      await client.query(`UPDATE saas_invoices SET period_end=$2, version=version+1 WHERE id=$1`, [
        active.rows[0].id,
        newEnd,
      ]);

      await client.query(`UPDATE tenants SET trial_ends_at=$2, version=version+1 WHERE id=$1`, [
        tenantId,
        newEnd,
      ]);
    ...
    res.json({
      success: true,
      message: `Langganan diperpanjang ${days} hari.`,
      newPeriodEnd: (result as any).newPeriodEnd,
    });
  } catch (err: any) {
...
export async function getTenantDetail(req: Request, res: Response) {
  try {
    const [tenant, users, invoices, statusHistory, impersonations, audit] = await Promise.all([
      dbQuery(
        `SELECT id,name,subdomain,status,tier,trial_ends_at AS "trialEndsAt",created_at AS "createdAt",settings,branding,version,status_reason AS "statusReason",scheduled_reactivation_at AS "scheduledReactivationAt",storage_used_bytes AS "storageUsedBytes",storage_measured_at AS "storageMeasuredAt" FROM tenants WHERE id=$1`,
        [req.params.id]
      ),
      dbQuery(
      ...
      dbQuery(
        `SELECT id,date,due_date AS "dueDate",amount,tier,status,billing_cycle AS "billingCycle",paid_at AS "paidAt",period_start AS "periodStart",period_end AS "periodEnd",created_at AS "createdAt" FROM saas_invoices WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT 100`,
        [req.params.id]
      ).catch(() => ({ rows: [] }) as any),
      dbQuery(
        `SELECT previous_status AS "previousStatus",next_status AS "nextStatus",category,reason,internal_note AS "internalNote",created_at AS "createdAt" FROM tenant_status_history WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT 50`,
        [req.params.id]
      ),
      dbQuery(
        `SELECT id,reason,ticket_id AS "ticketId",access_mode AS "accessMode",started_at AS "startedAt",expires_at AS "expiresAt",ended_at AS "endedAt" FROM impersonation_sessions WHERE tenant_id=$1 ORDER BY started_at DESC LIMIT 50`,
        [req.params.id]
      ),
      dbQuery(
        `SELECT id,action,outcome,resource_type AS "resourceType",resource_id AS "resourceId",created_at AS "createdAt",metadata FROM superadmin_audit_events WHERE effective_tenant_id=$1 ORDER BY created_at DESC LIMIT 100`,
        [req.params.id]
      ),
    ]);
    if (!tenant.rows[0]) return res.status(404).json({ error: 'Tenant tidak ditemukan.' });
    res.json({
      tenant: redactTenantSettingsSecrets(tenant.rows[0]),
      users: users.rows,
    ...
    logger.error({ err: err.message, tenantId: req.params.id }, 'getTenantDetail failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function listInvitations(req: Request, res: Response) {
  try {
    const result = await dbQuery(
      `SELECT id,tenant_id AS "tenantId",email,name,role,token_prefix AS "tokenPrefix",expires_at AS "expiresAt",accepted_at AS "acceptedAt",revoked_at AS "revokedAt",created_at AS "createdAt" FROM tenant_invitations WHERE tenant_id=$1 ORDER BY created_at DESC`,
      [req.params.id]
    );
    res.json({ invitations: result.rows });
  } catch (err: any) {
    logger.error({ err: err.message, tenantId: req.params.id }, 'listInvitations failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function revokeInvitation(req: Request, res: Response) {
  try {
    const row = await dbQuery(
      `UPDATE tenant_invitations SET revoked_at=now() WHERE id=$1 AND tenant_id=$2 AND accepted_at IS NULL AND revoked_at IS NULL RETURNING id`,
      [req.params.invitationId, req.params.id]
    );
    if (!row.rows[0]) return res.status(404).json({ error: 'Undangan aktif tidak ditemukan.' });
    res.json({ success: true });
  } catch (err: any) {
    logger.error({ err: err.message }, 'revokeInvitation failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function checkTenantAvailability(req: Request, res: Response) {
  ...
  const email = String(req.query.email || '')
    ...
    .toLowerCase();
  if (!subdomain && !email)
    return res.status(422).json({ error: 'Subdomain atau email wajib diisi.' });
  try {
    const [tenant, user, invitation] = await Promise.all([
      subdomain
        ? dbQuery(`SELECT 1 FROM tenants WHERE subdomain=$1 LIMIT 1`, [subdomain])
        : Promise.resolve({ rows: [] }),
      email
        ? dbQuery(`SELECT 1 FROM users WHERE lower(email)=$1 LIMIT 1`, [email])
        : Promise.resolve({ rows: [] }),
      email
        ? dbQuery(
            `SELECT 1 FROM tenant_invitations WHERE lower(email)=$1 AND accepted_at IS NULL AND revoked_at IS NULL AND expires_at>now() LIMIT 1`,
            [email]
          )
        : Promise.resolve({ rows: [] }),
    ]);
    res.json({
      subdomainAvailable: subdomain ? !tenant.rows[0] : null,
      emailAvailable: email ? !user.rows[0] && !invitation.rows[0] : null,
    });
  } catch (err: any) {
    logger.error({ err: err.message }, 'checkTenantAvailability failed');
    res.status(500).json({ error: 'Operasi Super Admin gagal diproses.' });
  }
}
...
export async function permanentDeleteTenant(req: Request, res: Response) {
  ...
  if (force !== true) {
    return res
      .status(400)
      .json({ error: "Konfirmasi 'force: true' diperlukan untuk menghapus permanen tenant." });
  }
  try {
    const result = await dbTransaction(async (client) => {
      const locked = await client.query(
        `SELECT id,name,status,tier,subdomain FROM tenants WHERE id=$1 FOR UPDATE`,
        [req.params.id]
      );
      const tenant = locked.rows[0];
      if (!tenant) return { code: 404, error: 'Tenant tidak ditemukan.' };
...
export async function registerTenant(req: Request, res: Response) {
  ...
  const cleanOwnerEmail = String(ownerEmail || '')
    .trim()
    .toLowerCase();
  ...
  if (
    !cleanName ||
    !cleanOwnerName ||
    !cleanOwnerEmail.includes('@') ||
    !['BASIC', 'PRO', 'ENTERPRISE'].includes(tier)
  ) {
  ...
  const planLimits: Record<
    string,
    { users: number; branches: number; storageMb: number; features: string[] }
  > = {
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const duplicateEmail = await client.query(`SELECT id FROM users WHERE lower(email)=$1`, [
        cleanOwnerEmail,
      ]);
      ...
      const tenant = await client.query(
        ...
        [
          ...
          JSON.stringify({
            baseCurrency: 'IDR',
            limits: planLimits[tier as keyof typeof planLimits] || planLimits['BASIC'],
            authSettings: { requireMfa: false, passwordPolicy: 'medium' },
      ...
      await client.query(
        `INSERT INTO branches (id,tenant_id,name,address,phone,is_active,created_at) VALUES ($1,$2,$3,$4,'',true,now())`,
        [branchId, tenantId, `Cabang Utama ${cleanName}`, `Alamat Utama ${cleanName}`]
      );
      await client.query(
        `INSERT INTO warehouses (id,tenant_id,branch_id,name,location,created_at) VALUES ($1,$2,$3,'Gudang Utama','Lokasi utama',now())`,
        [randomUUID(), tenantId, branchId]
      );
      const coa = [
        ['10100', 'Kas Utama', 'ASSET'],
        ['10200', 'Bank Utama', 'ASSET'],
        ['10300', 'Piutang Pelanggan', 'ASSET'],
        ['10500', 'Persediaan Barang', 'ASSET'],
        ['40100', 'Pendapatan Jasa Servis', 'REVENUE'],
        ['40200', 'Pendapatan Penjualan', 'REVENUE'],
        ['50100', 'HPP Sparepart', 'EXPENSE'],
        ['60100', 'Beban Gaji', 'EXPENSE'],
      ];
      for (const [code, accountName, type] of coa)
        await client.query(
          `INSERT INTO coa_accounts (id,tenant_id,code,name,type,balance) VALUES ($1,$2,$3,$4,$5,0)`,
          [randomUUID(), tenantId, code, accountName, type]
        );
      ...
      await client.query(
        `INSERT INTO billing_notification_outbox (event_key,tenant_id,channel,recipient,payload,status) VALUES ($1,$2,'INTERNAL',$3,$4::jsonb,'PENDING') ON CONFLICT(event_key,channel,recipient) DO NOTHING`,
        [
          `tenant_invitation:${invitation.rows[0].id}`,
          tenantId,
          cleanOwnerEmail,
          JSON.stringify({
      ...
      await adminAudit(
        ...
        null,
        tenant.rows[0],
        { invitationId: invitation.rows[0].id, ownerEmail: cleanOwnerEmail }
      );
      return { tenant: tenant.rows[0], branchId, invitation: invitation.rows[0] };
    });
    if ((result as any).error)
      return res.status((result as any).code).json({ error: (result as any).error });
    const deliveryQueued = await sendInvitationEmail(
      cleanOwnerEmail,
      cleanOwnerName,
      cleanName,
      token,
      (result as any).invitation.expiresAt
    ).catch((err) => {
      logger.warn({ err: err.message }, 'Invitation email delivery failed; retained in outbox');
      return false;
    });
...
export async function createTenantInvitation(req: Request, res: Response) {
  ...
  if (!name?.trim() || !email?.includes('@'))
    return res.status(422).json({ error: 'Nama dan email valid wajib diisi.' });
  const token = randomBytes(32).toString('base64url');
  const tokenHash = createHash('sha256').update(token).digest('hex');
  try {
    const row = await dbQuery(
      ...
      [
        ...
        token.slice(0, 8),
        req.authActor?.userId,
        Math.min(168, Math.max(1, expiresInHours)),
      ]
    );
    ...
    await dbQuery(
      `INSERT INTO billing_notification_outbox (event_key,tenant_id,channel,recipient,payload,status) VALUES ($1,$2,'INTERNAL',$3,$4::jsonb,'PENDING') ON CONFLICT(event_key,channel,recipient) DO NOTHING`,
      [
        `tenant_invitation:${row.rows[0].id}`,
        req.params.id,
        row.rows[0].email,
        JSON.stringify({
          type: 'TENANT_INVITATION',
          ...
          ownerName: row.rows[0].name,
          expiresAt: row.rows[0].expiresAt,
        }),
      ]
    );
...
export async function getTenantOperationalSummary(req: Request, res: Response) {
  try {
    ...
    const [
      ...
      dbQuery(
        `SELECT COUNT(*)::int AS open_tickets, COALESCE(SUM(estimated_cost),0)::numeric AS total_estimated_cost, COUNT(*) FILTER (WHERE warranty_expires_at IS NOT NULL AND warranty_expires_at >= now())::int AS active_warranties FROM service_tickets WHERE tenant_id=$1`,
        [tenantId]
      ).catch(
        () =>
          ({ rows: [{ open_tickets: 0, total_estimated_cost: 0, active_warranties: 0 }] }) as any
      ),
      dbQuery(
        `SELECT COUNT(*)::int AS transaction_count, COALESCE(SUM(grand_total),0)::numeric AS total_grand_total, COUNT(*) FILTER (WHERE void_reference_id IS NOT NULL)::int AS void_count FROM pos_transactions WHERE tenant_id=$1 AND created_at >= date_trunc('day', now())`,
        [tenantId]
      ).catch(
        () => ({ rows: [{ transaction_count: 0, total_grand_total: 0, void_count: 0 }] }) as any
      ),
      dbQuery(
        `SELECT COUNT(*)::int AS total_products, COUNT(*) FILTER (WHERE quantity <= 0)::int AS out_of_stock, COUNT(*) FILTER (WHERE quantity <= reorder_point)::int AS low_stock FROM products WHERE tenant_id=$1`,
        [tenantId]
      ).catch(() => ({ rows: [{ total_products: 0, out_of_stock: 0, low_stock: 0 }] }) as any),
      dbQuery(
        `SELECT COUNT(*)::int AS journal_count, COUNT(*) FILTER (WHERE is_posted = true)::int AS posted_count, COALESCE(SUM(total_debit),0)::numeric AS total_debit, COALESCE(SUM(total_credit),0)::numeric AS total_credit FROM journal_entries WHERE tenant_id=$1 AND created_at >= date_trunc('month', now())`,
        [tenantId]
      ).catch(
        () =>
          ({
            rows: [{ journal_count: 0, posted_count: 0, total_debit: 0, total_credit: 0 }],
          }) as any
      ),
      dbQuery(
        `SELECT COUNT(*)::int AS employee_count, COUNT(*) FILTER (WHERE status = 'ACTIVE')::int AS active_employees FROM employees WHERE tenant_id=$1`,
        [tenantId]
      ).catch(() => ({ rows: [{ employee_count: 0, active_employees: 0 }] }) as any),
      dbQuery(
        `SELECT COUNT(*)::int AS active_deals, COALESCE(SUM(value),0)::numeric AS pipeline_value FROM pipeline_deals WHERE tenant_id=$1 AND stage NOT IN ('WON','LOST')`,
        [tenantId]
      ).catch(() => ({ rows: [{ active_deals: 0, pipeline_value: 0 }] }) as any),
      dbQuery(
        `SELECT COUNT(*)::int AS open_tickets FROM support_tickets WHERE tenant_id=$1 AND status NOT IN ('CLOSED','RESOLVED')`,
        [tenantId]
      ).catch(() => ({ rows: [{ open_tickets: 0 }] }) as any),
      dbQuery(
        `SELECT COUNT(*)::int AS active_workflows, COUNT(*)::int AS executed_today FROM erp_workflows WHERE tenant_id=$1 AND created_at >= date_trunc('day', now())`,
        [tenantId]
      ).catch(() => ({ rows: [{ active_workflows: 0, executed_today: 0 }] }) as any),
    ]);
    ...
    const alerts = [];
    if ((modules.service.open_tickets || 0) > 0)
      alerts.push({
        module: 'SERVICE',
        label: 'Tiket servis terbuka',
        count: modules.service.open_tickets,
      });
    if ((modules.pos.void_count || 0) > 0)
      alerts.push({ module: 'POS', label: 'Void nota hari ini', count: modules.pos.void_count });
    if ((modules.inventory.out_of_stock || 0) > 0)
      alerts.push({
        module: 'INVENTORY',
        label: 'Stok habis',
        count: modules.inventory.out_of_stock,
      });
    if ((modules.accounting.total_debit || 0) !== (modules.accounting.total_credit || 0))
      alerts.push({ module: 'ACCOUNTING', label: 'Jurnal belum balance', count: 1 });
    if ((modules.hr.employee_count || 0) === 0)
      alerts.push({ module: 'HR', label: 'Belum ada data karyawan', count: 1 });
    ...
    logger.error(
      { err: err.message, tenantId: req.params.id },
      'Tenant operational summary failed'
    );
    res.status(500).json({ error: 'Ringkasan operasional tenant gagal dimuat.' });
  }
}

export async function getAggregatedOperationalSummary(req: Request, res: Response) {
  try {
    const result = await dbQuery(`SELECT id FROM tenants ORDER BY id`);
    const tenantIds = result.rows.map((row: any) => row.id);
      ...
      tenantIds.map(async (tenantId: string) => {
        const [
          ...
          dbQuery(
            ...
            () =>
              ({
                rows: [{ open_tickets: 0, total_estimated_cost: 0, active_warranties: 0 }],
              }) as any
          ),
          dbQuery(
            `SELECT COUNT(*)::int AS transaction_count, COALESCE(SUM(grand_total),0)::numeric AS total_grand_total, COUNT(*) FILTER (WHERE void_reference_id IS NOT NULL)::int AS void_count FROM pos_transactions WHERE tenant_id=$1 AND created_at >= date_trunc('day', now())`,
    ...
    const totalModules = summaries.reduce((sum, s) => sum + s.health.moduleCount, 0);
    ...
    res.json({
      ...
      aggregate: {
        totalAlerts,
        totalModules,
        status: totalAlerts === 0 ? 'ok' : 'warning',
      },
    });
  } catch (err: any) {
    logger.error({ err: err.message }, 'Aggregated operational summary failed');
    res.status(500).json({ error: 'Ringkasan operasional agregat gagal dimuat.' });
  }
}
