      ...
      <div className="xl:col-span-7 space-y-6">
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          ...
          <label className="block space-y-1">
            ...
            <input value={branding.logoUrl || ""} onChange={(e) => updateBranding({ logoUrl: e.target.value })} placeholder="https://domain/logo.png" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-500" />
          </label>

          <label className="block space-y-1">
            <span className="text-[10px] font-bold uppercase text-slate-400">Slogan</span>
            <input value={branding.slogan || ""} onChange={(e) => updateBranding({ slogan: e.target.value })} placeholder="Slogan bisnis" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-500" />
          </label>

          <label className="block space-y-1">
            <span className="text-[10px] font-bold uppercase text-slate-400">Font</span>
            <select value={branding.fontFamily || "inter"} onChange={(e) => updateBranding({ fontFamily: e.target.value })} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs bg-white outline-none focus:border-indigo-500">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          ...
          <label className="block space-y-1">
            ...
            <input value={branding.portalHelpTitle || ""} onChange={(e) => updateBranding({ portalHelpTitle: e.target.value })} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-500" />
          </label>

          <label className="block space-y-1">
            <span className="text-[10px] font-bold uppercase text-slate-400">Kontak bantuan portal</span>
            <textarea value={branding.portalContactText || ""} onChange={(e) => updateBranding({ portalContactText: e.target.value })} rows={3} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-500" />
          </label>

          <div className="space-y-1">
            <span className="text-[10px] font-bold uppercase text-slate-400">Custom domain</span>
            <div className="flex gap-2">
              <input value={branding.customDomain || ""} onChange={(e) => updateBranding({ customDomain: e.target.value })} className="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-500" />
          ...
          <label className="flex items-center gap-2 text-xs font-semibold text-slate-700">
            <input type="checkbox" checked={!!branding.whiteLabelEnabled} onChange={(e) => updateBranding({ whiteLabelEnabled: e.target.checked })} className="accent-indigo-600" />
            Aktifkan white-label branding
          </label>
        </div>

        <button type="button" onClick={save} className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-xs font-bold">
