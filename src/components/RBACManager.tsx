...
export const RBACManager: React.FC = () => {
  ...
  const currentTenantUsersCount = users.filter(
    (u) => u.tenantId === currentTenantId && u.role !== UserRole.OWNER
  ).length;
  ...
  // SUPER_ADMIN: tenant + branch selection
  const isSuperAdminNoTenant = currentUser.role === UserRole.SUPER_ADMIN && !currentTenantId;
  ...
  const effectiveBranches = isSuperAdminNoTenant ? tenantBranches : scopedBranches;

  // Fetch branches for SUPER_ADMIN selected tenant
  React.useEffect(() => {
    if (!isSuperAdminNoTenant || !newTenantId) {
      setTenantBranches([]);
      return;
    }
    let cancelled = false;
  ...
  }, [isSuperAdminNoTenant, newTenantId, apiFetch]);

  // Reset branch selection when modal opens or tenant changes
  React.useEffect(() => {
    if (showAddModal && effectiveBranches.length > 0 && newBranchIds.length === 0) {
      setNewBranchIds([effectiveBranches[0].id]);
    }
  }, [showAddModal, effectiveBranches, newBranchIds]);
  ...
  const handleAddUser = (e: React.FormEvent) => {
    ...
    try {
      addUser({
        ...
        email: cleanEmail,
        role: newRole,
        tenantId: newTenantId || currentTenantId,
        branchIds:
    ...
    switch (role) {
      ...
      case UserRole.OWNER:
        return {
          ...
          badge: 'bg-accent-lighter text-accent border-indigo-200',
      ...
      <div className="flex items-center justify-between rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-[11px] text-slate-500">
            ...
            <span className="font-bold uppercase text-accent">{activeTenant?.tier || 'BASIC'}</span>
          </p>
        </div>
              ...
              <form onSubmit={handleAddUser} className="space-y-5 p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="max-h-[100px] overflow-y-auto rounded-xl border border-slate-200 bg-slate-50/50 p-2">
                        {loadingBranches ? (
                          <p className="px-2 py-1 text-[11px] text-slate-400">Memuat cabang...</p>
                        ) : effectiveBranches.length === 0 ? (
                ...
                {isLimitReached ? (
                  <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-[11px] text-rose-700">
                    <strong>Kuota Penuh:</strong> Anda telah mencapai batas maksimal (
                    {currentTenantUsersCount}/{userLimit}) staff. Silakan lakukan upgrade paket.
                  </div>
                ) : (
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-full bg-accent-lighter px-2 py-0.5 text-[10px] font-bold text-accent">
                      {editPerms.length} / {DEFAULT_ROLE_PERMISSIONS.SUPER_ADMIN?.length || 40}{' '}
                      Aktif
                    </span>
                  </div>
              ...
              <div className="max-h-[58vh] overflow-y-auto p-5 space-y-5">
                {permGroups.map((g) => {
                  ...
                  return (
                    <div
                      key={g.cat}
                      className="space-y-2 rounded-xl border border-slate-100 bg-slate-50/30 p-3"
                    >
                      ...
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        {g.perms.map((perm) => {
                          ...
                          const labelText =
                            PERMISSION_LABELS[perm] || perm.split('-').slice(1).join('-') || perm;
                          return (
                            <button
                              key={perm}
