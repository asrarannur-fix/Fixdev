...
const TROUBLESHOOT_TIPS = [
  ...
  },
  {
    ...
    cause: 'Instabilitas pada sirkuit RAM, korosi jalur BIOS, atau kegagalan chip grafis (GPU).',
    steps: [
  ...
  },
  {
    ...
    cause: 'Konektor fisik kotor/longgar, IC Charger rusak, atau sekering (fuse) input putus.',
    steps: [
...
export const KnowledgeBase: React.FC = () => {
  ...
  const categories = ['Semua', 'Smartphone', 'Laptop', 'Console', 'Tablet', 'MacBook'];
      ...
      <div className="bg-slate-900 text-white rounded-3xl p-6 relative overflow-hidden shadow-md">
        ...
        <div className="relative z-10 max-w-2xl space-y-2">
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            Akses diagram boardview sirkuit cetak (PCB), skema kelistrikan resmi, dan gunakan
            panduan diagnosis untuk memecahkan masalah perangkat keras yang kompleks.
          </p>
        </div>
      </div>
            ...
            {filteredGuides.length > 0 ? (
              filteredGuides.map((guide) => (
                ...
                >
                  ...
                  <button
                    onClick={() => showToast(`Memulai unduhan PDF: ${guide.title}`, 'info')}
              ...
              <div className="py-12 text-center text-slate-400 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                ...
                <p className="text-xs font-bold text-slate-600">Skema tidak ditemukan</p>
                <p className="text-[10px] mt-1">Coba gunakan kata kunci pencarian yang berbeda.</p>
              </div>
            )}
          </div>
        </div>
            ...
            {TROUBLESHOOT_TIPS.map((tip, idx) => (
              ...
              >
                <div className="space-y-1.5">
                  <h5 className="font-black text-xs text-indigo-950 leading-snug">{tip.title}</h5>
                ...
                <div className="pt-2 border-t border-indigo-100/50 space-y-1">
                  ...
                  <ul className="space-y-1">
                    {tip.steps.map((st, i) => (
                      <li key={i} className="text-[9.5px] text-slate-700 flex items-start gap-1">
