...
if (process.env.NODE_ENV !== 'test' && process.env.ALLOW_DB_MIGRATIONS !== '1') {
...
const files = fs.readdirSync('migrations').filter((file) => file.endsWith('.sql')).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
try {
  ...
  for (const filename of files) {
    ...
    await client.query(fs.readFileSync(path.join('migrations', filename), 'utf8'));
