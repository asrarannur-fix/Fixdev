...
export default function SaaSSubscription({ readOnlyMode = false, section = "all" }: { readOnlyMode?: boolean; section?: "all" | "billing" | "gateway" }) {
  ...
  const handleSaveManualConfig = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const saved = await readJsonResponse<any>(await apiFetch("/api/billing/manual-payment-config", { method: "PUT", body: JSON.stringify({ ...manualConfig, qrisObjectPath, qrisOriginalName }) }), "Pembayaran manual");
