...
export async function createSupplier(req: Request, res: Response) {
  ...
  if (!parsed.success)
    return res
      .status(422)
      .json({ error: 'Data supplier tidak valid.', details: parsed.error.flatten() });
  try {
...
export async function receiveGoods(req: Request, res: Response) {
  ...
  if (!parsed.success)
    return res
      .status(422)
      .json({ error: 'Data penerimaan barang tidak valid.', details: parsed.error.flatten() });
  const tenantId = req.tenantId!;
  const { warehouseId, items, paymentMethod } = parsed.data;
  const idemKey = parsed.data.idempotencyKey || null;
  try {
    const result = await dbTransaction(async (client) => {
      ...
      if (idemKey) {
        ...
        if (dup.rows[0]) {
          return {
            id: dup.rows[0].id,
            receiptNo: dup.rows[0].receiptNo,
            totalAmount: 0,
            journalId: null,
            duplicate: true,
          };
      ...
      for (const it of items) {
        ...
        if (!prod.rows[0]) {
          const e: any = new Error(`Produk ${it.name} tidak ditemukan.`);
          e.status = 422;
          throw e;
        }
        ...
        await client.query(`UPDATE products SET purchase_cost=$3 WHERE id=$1 AND tenant_id=$2`, [
          it.productId,
          tenantId,
          newCost,
        ]);
      ...
      if (total > 0) {
        ...
        const creditCode =
          paymentMethod === 'CREDIT' ? '20200' : paymentDebitAccountCode(paymentMethod);
        ...
        await client.query(`UPDATE goods_receipts SET journal_entry_id=$2 WHERE id=$1`, [
          receiptId,
          journalId,
        ]);
      }
    ...
    if ((result as any).duplicate) {
      return res
        .status(200)
        .json({ data: result, message: 'Penerimaan duplikat diabaikan (idempotensi).' });
    }
...
export async function paySupplier(req: Request, res: Response) {
  ...
  if (!parsed.success)
    return res
      .status(422)
      .json({ error: 'Data pembayaran tidak valid.', details: parsed.error.flatten() });
  const tenantId = req.tenantId!;
  try {
    const result = await dbTransaction(async (client) => {
      const gr = await client.query(
      ...
      if (!gr.rows[0]) {
        const e: any = new Error('Penerimaan barang tidak ditemukan.');
        e.status = 404;
        throw e;
      }
      if (gr.rows[0].paymentMethod !== 'CREDIT') {
        const e: any = new Error('Penerimaan ini bukan pembelian kredit.');
        e.status = 409;
        throw e;
      }
      const outstanding =
        (Number(gr.rows[0].totalAmount) || 0) - (Number(gr.rows[0].amountPaid) || 0);
      if (parsed.data.amount > outstanding + 0.5) {
        const e: any = new Error(
          `Pembayaran melebihi sisa utang (Rp${Math.round(outstanding).toLocaleString('id-ID')}).`
        );
        e.status = 422;
        throw e;
      }
      ...
      const cashAcctId = await ensureAccount(
        client,
        tenantId,
      ...
      const pay = await client.query(
        ...
        [
          tenantId,
          req.branchId || null,
          parsed.data.goodsReceiptId,
          gr.rows[0].supplierId,
          paymentNo,
          parsed.data.amount,
          parsed.data.method,
          journalId,
      ...
      await client.query(`UPDATE goods_receipts SET amount_paid = amount_paid + $2 WHERE id=$1`, [
        parsed.data.goodsReceiptId,
        parsed.data.amount,
      ]);
      return pay.rows[0];
    ...
    res
      .status(201)
      .json({ data: result, message: 'Pembayaran utang dicatat & jurnal diperbarui.' });
  } catch (e) {
    sendError(res, e);
  }
}
