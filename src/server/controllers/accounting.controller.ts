...
export const updateAccount = async (req: any, res: any) => {
  ...
  try {
    ...
    if (updates.isGroup !== undefined) {
      sets.push(`is_group = $${idx++}`);
      params.push(updates.isGroup);
    }
  ...
  try {
    ...
    const result = await dbTransaction(async (client) => {
      ...
      if (Math.abs(totalDebit - totalCredit) > 0.01) {
        throw Object.assign(
          new Error('Jurnal tidak seimbang. Total debit harus sama dengan total kredit.'),
          {
            status: 422,
          }
        );
      ...
      // Validate every line's account belongs to THIS tenant (prevents posting
      // to another tenant's chart of accounts via a global FK).
      const accountIds = [...new Set(lines.map((l: any) => l.accountId))];
      const accCheck = await client.query(
        `SELECT id FROM coa_accounts WHERE tenant_id = $1 AND id = ANY($2::uuid[])`,
        [tenantId, accountIds]
      );
      if (accCheck.rows.length !== accountIds.length) {
        throw Object.assign(new Error('Salah satu akun tidak ditemukan untuk tenant ini.'), {
          status: 422,
        });
      }
      ...
      if (refNo) {
        ...
        if (refCheck.rows[0])
          throw Object.assign(
            new Error(`Duplikasi transaksi: Reference No ${refNo} sudah terdaftar.`),
            {
              status: 422,
            }
          );
    ...
    res.status(status).json({ error: err?.message || 'Operasi akuntansi gagal diproses.' });
  }
};
...
export const getTrialBalance = async (req: any, res: any) => {
  ...
  try {
    ...
    // Textbook trial balance: total of all posted debits vs all posted credits.
    // Previous per-type "positive balance" approach silently dropped contra
    // balances (accounts with a negative normal balance), hiding real imbalance.
    const totalDebit = accounts.reduce(
      (sum: number, a: any) =>
        sum +
        (a.type === 'ASSET' || a.type === 'EXPENSE'
          ? Math.max(0, Number(a.balance))
          : Math.max(0, -Number(a.balance))),
      0
    );
...
export const getBalanceSheet = async (req: any, res: any) => {
  ...
  try {
    ...
    const assets = allAccounts
      ...
      .map((a: any) => ({ ...a, balance: Math.max(0, Number(a.balance)) }));
    ...
    const revenue = allAccounts
      ...
      .reduce((s: number, a: any) => s + Math.max(0, Number(a.balance)), 0);
    const expense = allAccounts
...
export const getProfitAndLoss = async (req: any, res: any) => {
  ...
  try {
    ...
    if (from) {
      params.push(from);
      dateConditions.push(`je.entry_date >= $${params.length}`);
    }
    if (to) {
      params.push(to);
      dateConditions.push(`je.entry_date <= $${params.length}`);
  ...
  try {
    const result = await dbTransaction(async (client) => {
      ...
      if (!cashAcct.rows[0]) {
        ...
        if (!fallback.rows[0])
          throw new Error(
            'Tidak ada akun aset (kas) untuk transaksi. Buat akun kas terlebih dahulu.'
          );
      ...
      if (!targetAcct.rows[0])
        throw new Error('Akun lawan transaksi tidak ditemukan atau bukan milik tenant Anda.');
      ...
      if (parsed.refNo) {
        ...
        if (refCheck.rows[0])
          throw Object.assign(
            new Error(`Duplikasi transaksi: Reference No ${parsed.refNo} sudah terdaftar.`),
            { status: 422 }
          );
      }
    ...
    await dbQuery(
     ...
     VALUES (gen_random_uuid(), $1, $2, 'CASH_TX', $3)`,
      [
        tenantId,
        userId,
        `${parsed.type}: Rp${parsed.amount.toLocaleString('id-ID')} — ${parsed.description}`,
      ]
    );
