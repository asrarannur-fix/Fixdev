...
const STATUS_COLOR: Record<string, string> = {
  ...
  [ServiceStatus.SEDANG_DIKERJAKAN]:
    'bg-indigo-100 dark:bg-indigo-950/40 text-accent dark:text-accent',
...
const PRIORITY_CONFIG: Record<string, { label: string; dot: string; bg: string }> = {
  URGENT: {
    label: 'URGENT',
    dot: 'bg-rose-500',
    bg: 'bg-rose-50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-400',
  },
  HIGH: {
    label: 'HIGH',
    dot: 'bg-amber-500',
...
const fmtRp = (n?: number) =>
  n != null && Number.isFinite(n) ? 'Rp\u00a0' + n.toLocaleString('id-ID') : '–';
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => (
  <span
    className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full ${STATUS_COLOR[status] ?? 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}
  >
    {STATUS_LABEL[status] ?? status}
  </span>
);

const InfoRow: React.FC<{ label: string; children: React.ReactNode; mono?: boolean }> = ({
  label,
  children,
  mono,
}) => (
  <div>
    <dt className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">
      {label}
    </dt>
    <dd
      className={`text-sm font-semibold text-slate-800 dark:text-slate-200 ${mono ? 'font-mono text-xs tracking-wider' : ''}`}
    >
      {children}
    </dd>
  </div>
...
export const TicketEditorDock: React.FC<{
  ...
  const [parts, setParts] = useState<{ name: string; qty: number; cost: number }[]>(
    ticket?.spareParts ?? []
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3 text-center">
        ...
        <button
          ...
          className="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-xs font-bold rounded-xl cursor-pointer transition-colors hover:opacity-80"
        >
          ← Kembali
        </button>
      </div>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMsg({
        ...
        text:
          status === ServiceStatus.SELESAI
            ? `Tersimpan! Garansi aktif ${wDays} hari.`
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-slate-800 via-indigo-900 to-slate-900 text-white">
        ...
        <div className="relative p-5">
              ...
              <div className="flex items-center gap-2 flex-wrap mb-2">
                ...
                {ticket.priority && (
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${priority.bg}`}>
                    <span
                      className={`inline-block w-1.5 h-1.5 rounded-full ${priority.dot} mr-1 -mb-px`}
                    />
                    {priority.label}
                  </span>
                )}
          ...
          <div className="flex items-center gap-2 pt-3 border-t border-white/10">
            ...
            {qcItems.length > 0 && (
              <span
                className={`text-[10px] font-bold px-2 py-1 rounded-full border ${qcRate >= 80 ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-300' : 'bg-rose-500/20 border-rose-500/30 text-rose-300'}`}
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
              ...
              <dl className="space-y-3">
                ...
                {ticket.serialNumber && (
                  <InfoRow label="Serial Number" mono>
                    {ticket.serialNumber}
                  </InfoRow>
                )}
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:col-span-2">
              ...
              <p className="text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 rounded-xl px-4 py-3 leading-relaxed whitespace-pre-wrap">
                {ticket.customerComplaints || (
                  <span className="text-slate-400 italic">Tidak ada catatan keluhan.</span>
                )}
              </p>
            </div>
              ...
              <dl className="space-y-3">
                <InfoRow label="Estimasi Total">
                  <span className="text-lg font-black text-emerald-600 dark:text-emerald-400">
                    {fmtRp(ticket.estimatedCost)}
                  </span>
                </InfoRow>
                {ticket.diagnosticFee > 0 && (
                  <InfoRow label="Biaya Diagnosa">{fmtRp(ticket.diagnosticFee)}</InfoRow>
                )}
                <InfoRow label="Teknisi">
                  {technicians.find((t) => t.id === ticket.technicianId)?.name ?? (
            ...
            {/* Warranty */}
            <div
              className={`border rounded-2xl p-4 ${hasWarranty ? 'bg-teal-50/50 dark:bg-teal-950/10 border-teal-200 dark:border-teal-800/40' : 'bg-slate-50 dark:bg-slate-800/30 border-dashed border-slate-200 dark:border-slate-700'}`}
            >
              <div className="flex items-center gap-2 mb-3.5 pb-2.5 border-b border-teal-100/60 dark:border-teal-900/30">
                ...
                <h3
                  className={`text-xs font-bold uppercase tracking-wide ${hasWarranty ? 'text-teal-700 dark:text-teal-400' : 'text-slate-500'}`}
                >
                  Garansi Servis
                </h3>
              </div>
              {ticket.warrantyExpiry ? (
                <dl className="space-y-2">
                  ...
                  <div className="pt-1">
                    {hasWarranty ? (
                      <span className="text-[10px] font-black bg-teal-500 text-white px-2.5 py-1 rounded-full">
                        ✓ AKTIF
                      </span>
                    ) : (
                      <span className="text-[10px] font-black bg-rose-500 text-white px-2.5 py-1 rounded-full">
                        KADALUARSA
                      </span>
                    )}
                  </div>
                ...
                <div className="py-4 text-center">
                  ...
                  <p className="text-xs text-slate-400 leading-snug">
                    Garansi belum aktif.
                    <br />
                    Ubah status ke <strong>Selesai</strong> untuk mengaktifkan.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {ticket.photos.map((url: string, i: number) => (
                  <div
                    key={i}
                    className="aspect-video rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800"
                  >
            ...
            {status === ServiceStatus.SELESAI && (
              <div className="bg-emerald-50/60 dark:bg-emerald-950/15 border border-emerald-200 dark:border-emerald-800/40 rounded-xl p-4 space-y-2.5">
                ...
                <input
                  ...
                  className={
                    inputCls +
                    ' border-emerald-200 dark:border-emerald-700 focus:border-accent focus:ring-accent/25'
                  }
                />
                <p className="text-xs text-emerald-600 dark:text-emerald-500">
                  Berlaku hingga:{' '}
                  <strong>
                    {fmtDt(new Date(Date.now() + Number(wDays || 30) * 86_400_000).toISOString())}
                  </strong>
                </p>
              </div>
            )}
            ...
            {parts.length > 0 && (
              <div className="space-y-1.5 mb-4">
                {parts.map((p, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-3 px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800/50 rounded-xl group"
                  >
                    <div className="flex-1 min-w-0">
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-1">
              ...
              {qcItems.length > 0 && (
                <span
                  className={`text-sm font-black px-3 py-1 rounded-full ${qcRate >= 80 ? 'bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400' : 'bg-rose-100 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400'}`}
                >
                  {qcRate}%
                </span>
              )}
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-5">
              Lakukan pengecekan QC sebelum unit diserahkan. Minimal <strong>80%</strong> checklist
              harus PASS.
            </p>
            ...
            {qcItems.length > 0 ? (
              <div className="space-y-1.5 mb-5">
                {qcItems.map((item: any, i: number) => (
                  <div
                    key={i}
                    className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl ${item.checked ? 'bg-emerald-50/60 dark:bg-emerald-950/10' : 'bg-slate-50 dark:bg-slate-800/40'}`}
                  >
                    <div
                      className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${item.checked ? 'bg-emerald-500' : 'bg-rose-400'}`}
                    >
                      {item.checked ? (
                    ...
                    <span
                      className={`text-xs flex-1 leading-snug ${item.checked ? 'text-emerald-700 dark:text-emerald-300' : 'text-slate-600 dark:text-slate-400'}`}
                    >
                      {item.label}
                    </span>
                    <span
                      className={`text-[10px] font-black px-2 py-0.5 rounded-full min-w-[2.5rem] text-center ${item.checked ? 'bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400' : 'bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400'}`}
                    >
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800 px-0.5">
                  <span className="text-xs text-slate-500">
                    {qcPass}/{qcItems.length} item PASS
                  </span>
                  <span
                    className={`text-sm font-black ${qcRate >= 80 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
                  >
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-start gap-4 pl-1">
                ...
                <div className="pt-1.5 flex-1 min-w-0">
                  ...
                  {ticket.notes && (
                    <p className="text-xs text-slate-500 italic bg-slate-50 dark:bg-slate-800 rounded-lg px-3 py-2 mt-2 line-clamp-3">
                      {ticket.notes}
                    </p>
                  )}
                </div>
