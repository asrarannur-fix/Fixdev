...
export default function RolePermissionMatrix({ readOnlyMode = false }: { readOnlyMode?: boolean }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900" id="sa-role-permission-matrix">
      ...
      <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[800px] text-xs"><thead><tr className="border-b border-slate-200 text-slate-500 dark:border-zinc-800"><th className="p-3 text-left">Role</th>{permissions.map((permission) => <th key={permission} className="p-3 text-center font-medium">{permission}</th>)}<th className="p-3 text-center">Simpan</th></tr></thead><tbody>{roles.map((row) => <tr key={row.role} className="border-b border-slate-100 dark:border-zinc-800"><td className="p-3 font-black text-slate-800 dark:text-zinc-200">{row.role}</td>{permissions.map((permission) => { const allowed = row.permissions.includes("*") || row.permissions.includes(permission); return <td key={permission} className="p-3 text-center"><button type="button" disabled={readOnlyMode || row.role === "ROOT_ADMIN"} onClick={() => togglePermission(row.role, permission)} aria-label={`${allowed ? "Cabut" : "Beri"} ${permission} untuk ${row.role}`} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg p-2 disabled:cursor-not-allowed">{allowed ? <CheckCircle2 className="mx-auto h-4 w-4 text-emerald-500" /> : <XCircle className="mx-auto h-4 w-4 text-slate-300 dark:text-zinc-700" />}</button></td>; })}<td className="p-3"><button type="button" disabled={readOnlyMode || row.role === "ROOT_ADMIN" || savingRole === row.role} onClick={() => saveRole(row)} className="min-h-11 rounded-lg bg-accent px-3 py-2 text-[10px] font-bold text-white disabled:opacity-40">{savingRole === row.role ? "Menyimpan…" : "Simpan"}</button></td></tr>)}</tbody></table></div>
      ...
      <div className="mt-6 border-t border-slate-200 pt-5 dark:border-zinc-800">
        ...
        <div className="mt-3 space-y-2">{adminUsers.map((user) => <div key={user.id} className="flex flex-col gap-3 rounded-xl border border-slate-200 p-3 sm:flex-row sm:items-center sm:justify-between dark:border-zinc-800"><div><p className="text-xs font-bold">{user.name || user.email}</p><p className="text-[11px] text-slate-500">{user.email}</p></div><select aria-label={`Role untuk ${user.email}`} value={user.superadminRole || "SUPPORT_ADMIN"} disabled={readOnlyMode || assigningUser === user.id} onChange={async (event) => { const role = event.target.value; setAssigningUser(user.id); try { const result = await readJsonResponse<any>(await apiFetch(`/api/superadmin/users/${user.id}/role`, { method: "PUT", body: JSON.stringify({ role }) }), "Tetapkan role"); setAdminUsers((current) => current.map((item) => item.id === user.id ? { ...item, superadminRole: result.user.superadminRole } : item)); setError(""); } catch (err: any) { setError(err.message); } finally { setAssigningUser(""); } }} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-950">{roles.map((row) => <option key={row.role} value={row.role}>{row.role}</option>)}</select></div>)}</div>
      </div>
    </section>
  );
}
