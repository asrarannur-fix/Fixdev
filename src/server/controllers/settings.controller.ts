...
const templateSchema = z
  .object({
    id: text(100),
    name: text(150),
    category: z.enum(['SERVICE_UPDATE', 'INVOICE_REMINDER', 'PROMOTION', 'CUSTOM']),
...
const schemas = {
  ...
  generalSettings: z
    .object({
      appName: text(150).optional(),
      timezone: text(100).optional(),
      dateFormat: text(30).optional(),
      language: text(10).optional(),
      numberFormat: text(30).optional(),
      maintenanceMode: bool.optional(),
    })
    .strict(),
  serviceSettings: z
    .object({
      defaultDiagnosisFee: money.optional(),
      requireEstimateApproval: bool.optional(),
      allowProceedWithoutApproval: bool.optional(),
      slaHours: z.number().int().min(1).max(720).optional(),
      autoAssignTechnician: bool.optional(),
    })
    .strict(),
  posSettings: z
    .object({
      paymentMethods: z.array(text(50)).max(30).optional(),
      maxDiscount: z.number().min(0).max(100).optional(),
      allowNegativeStock: bool.optional(),
      requireVoidApproval: bool.optional(),
      requireCloseCash: bool.optional(),
    })
    .strict(),
  inventorySettings: z
    .object({
      hppMethod: z.enum(['FIFO', 'LIFO', 'AVG']).optional(),
      defaultWarehouseId: z.string().uuid().optional(),
      requireAdjustmentApproval: bool.optional(),
      enableStockAlert: bool.optional(),
    })
    .strict(),
  accountingSettings: z
    .object({
      defaultCashAccountId: text(100).optional(),
      defaultBankAccountId: text(100).optional(),
      defaultSalesAccountId: text(100).optional(),
      defaultHppAccountId: text(100).optional(),
      defaultInventoryAccountId: text(100).optional(),
      defaultReceivableAccountId: text(100).optional(),
      defaultPayableAccountId: text(100).optional(),
      autoJournalEnabled: bool.optional(),
    })
    .strict(),
  hrmSettings: z
    .object({
      defaultWorkHours: z.number().min(1).max(24).optional(),
      graceLateMinutes: z.number().int().min(0).max(1440).optional(),
      enableOvertime: bool.optional(),
  ...
  portalSettings: z
    .object({
      ...
      enableEstimateApproval: bool.optional(),
      enableInvoiceView: bool.optional(),
      enableWarrantyView: bool.optional(),
      enableTicketTracking: bool.optional(),
      hideInternalNotes: bool.optional(),
      hideProfit: bool.optional(),
    })
    .strict(),
  emailSettings: z
    .object({
      smtpHost: text(253).optional(),
      smtpPort: z.number().int().min(1).max(65535).optional(),
      smtpUser: text(320).optional(),
      smtpPass: secret,
      defaultFromEmail: z.union([z.literal(''), z.string().email().max(320)]).optional(),
      enablePushNotifications: bool.optional(),
      enableRealtimeNotifications: bool.optional(),
    })
    .strict(),
  uploadSettings: z
    .object({
      maxUploadSizeMb: z.number().int().min(1).max(100).optional(),
      allowedFileTypes: text(1000).optional(),
      folderInvoices: text(250).optional(),
      folderServicePhotos: text(250).optional(),
      folderAttendance: text(250).optional(),
      folderCustomerDocs: text(250).optional(),
      fileVisibility: z.enum(['private', 'public']).optional(),
  ...
  themeSettings: z
    .object({
      ...
      sidebarMode: z.enum(['collapsed', 'expanded']).optional(),
  ...
  securitySettings: z
    .object({
      ...
      allowPasswordReuse: bool.optional(),
    })
    .strict(),
  notificationSettings: z
    .object({
      ...
      telegramChatId: text(100).optional(),
    })
    .strict(),
  printConfig: z
    .object({
  ...
  waConfig: z
    .object({
      gateway: text(100).optional(),
      isConnected: bool.optional(),
      credentialsConfigured: bool.optional(),
      apiToken: secret,
      phoneNumber: text(30).optional(),
      sendingMethod: z.enum(['API', 'MANUAL']).optional(),
      syncEstimate: bool.optional(),
      phoneId: text(100).optional(),
      wabaId: text(100).optional(),
      ...
      triggers: z
        .record(
          z.string().max(100),
          z.object({ enabled: bool.optional(), templateId: text(100).optional() }).partial()
        )
        .optional(),
...
const rbacSchema = z
  .object({
    ...
    matrix: z.record(roleSchema, permissionsSchema),
  })
  .strict();
...
const MASK = /^(?:\*+|•+|x+)$/i;
const SECRET_FIELDS: Partial<Record<SettingsDomain, string[]>> = {
  emailSettings: ['smtpPass'],
  notificationSettings: ['telegramBotToken'],
  waConfig: ['apiToken', 'webhookSecret', 'whatsappKey'],
};
export function mergeSettingsSecrets(
  ...
  for (const field of SECRET_FIELDS[domain] || []) {
    const value = incoming[field];
    if (
      value === undefined ||
      (typeof value === 'string' && (!value.trim() || MASK.test(value.trim())))
    ) {
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const current = await client.query(
        `SELECT settings,branding,version FROM tenants WHERE id=$1 FOR UPDATE`,
        [req.tenantId]
      );
      const tenant = current.rows[0];
      if (!tenant || tenant.version !== expectedVersion.data) return null;
      const existing =
        domain === 'branding' ? tenant.branding || {} : tenant.settings?.[domain] || {};
      const value = mergeSettingsSecrets(domain, existing, parsed.data as Record<string, unknown>);
      return client.query(
        domain === 'branding'
          ? `UPDATE tenants SET branding=$3::jsonb,version=version+1 WHERE id=$1 AND version=$2 RETURNING settings,branding,version`
          : `UPDATE tenants SET settings=jsonb_set(COALESCE(settings,'{}'::jsonb),$3::text[],$4::jsonb,true),version=version+1 WHERE id=$1 AND version=$2 RETURNING settings,branding,version`,
        domain === 'branding'
          ? [req.tenantId, expectedVersion.data, JSON.stringify(value)]
          : [req.tenantId, expectedVersion.data, [domain], JSON.stringify(value)]
      );
    });
    if (!result?.rows[0])
      return res
...
export async function updateRbacMatrix(req: Request, res: Response) {
  const body = rbacSchema.safeParse(req.body);
  if (!body.success) return res.status(422).json({ error: 'Matriks RBAC atau versi tidak valid.' });
  try {
    const result = await dbQuery(
      `UPDATE tenants SET rbac_matrix=$3::jsonb,version=version+1 WHERE id=$1 AND version=$2 RETURNING rbac_matrix,version`,
      [req.tenantId, body.data.expectedVersion, JSON.stringify(body.data.matrix)]
    );
    if (!result.rows[0])
      return res
        .status(409)
        .json({ error: 'RBAC telah berubah. Muat ulang halaman.', code: 'VERSION_CONFLICT' });
    res.json(toApiResponse(result.rows[0]));
  } catch (err: any) {
    logger.error({ err: err.message, tenantId: req.tenantId }, 'RBAC matrix update failed');
    res.status(500).json({ error: 'Matriks RBAC gagal disimpan.' });
  }
}
export async function updateUserRbac(req: Request, res: Response) {
  ...
  try {
    const result = await dbQuery(
      `UPDATE users SET role=$3,permissions=$4::text[] WHERE id=$1 AND tenant_id=$2 RETURNING id,role,permissions`,
      [req.params.userId, req.tenantId, body.data.role, body.data.permissions]
    );
    if (!result.rows[0]) return res.status(404).json({ error: 'Pengguna tidak ditemukan.' });
    res.json(toApiResponse(result.rows[0]));
  } catch (err: any) {
    logger.error({ err: err.message, tenantId: req.tenantId }, 'User RBAC update failed');
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const tenantResult = await client.query(
        `SELECT tier,settings FROM tenants WHERE id=$1 FOR UPDATE`,
        [req.tenantId]
      );
      ...
      if (count >= limit)
        return {
          code: 409,
          error: `Kuota cabang penuh (${count}/${limit}).`,
          errorCode: 'BRANCH_QUOTA_EXCEEDED',
        };
      const branch = await client.query(
        `INSERT INTO branches(id,tenant_id,name,address,phone,is_active) VALUES (COALESCE($1::uuid,gen_random_uuid()),$2,$3,$4,$5,$6) RETURNING *`,
        [
          body.data.id || null,
          req.tenantId,
          body.data.name,
    ...
    if ('error' in result)
      return res.status(result.code).json({ error: result.error, code: result.errorCode });
    res.status(201).json(toApiResponse(result.branch));
  } catch (err: any) {
    logger.error({ err: err.message, tenantId: req.tenantId }, 'Branch creation failed');
    res.status(500).json({ error: 'Cabang gagal dibuat.' });
  }
}
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const branchResult = await client.query(
        `SELECT * FROM branches WHERE id=$1 AND tenant_id=$2 FOR UPDATE`,
        [req.params.id, req.tenantId]
      );
      const branch = branchResult.rows[0];
      if (!branch) return { code: 404, error: 'Cabang tidak ditemukan.' };
      if (body.data.isActive === false && branch.is_active !== false) {
        if (req.branchId === req.params.id)
          return { code: 409, error: 'Pindah ke cabang lain sebelum menonaktifkan cabang aktif.' };
        const otherActive = await client.query(
          `SELECT COUNT(*)::int AS count FROM branches WHERE tenant_id=$1 AND id<>$2 AND is_active=TRUE`,
          [req.tenantId, req.params.id]
        );
        if (otherActive.rows[0].count === 0)
          return { code: 409, error: 'Tenant harus memiliki minimal satu cabang aktif.' };
        const activeTickets = await client.query(
          `SELECT COUNT(*)::int AS count FROM service_tickets WHERE tenant_id=$1 AND branch_id=$2 AND status NOT IN ('DIAMBIL','DIBATALKAN','SELESAI','KLAIM_GARANSI')`,
          [req.tenantId, req.params.id]
        );
        if (activeTickets.rows[0].count > 0)
          return {
            code: 409,
            error: `Selesaikan atau pindahkan ${activeTickets.rows[0].count} tiket servis aktif terlebih dahulu.`,
          };
        const openShifts = await client.query(
          `SELECT COUNT(*)::int AS count FROM pos_shifts WHERE tenant_id=$1 AND branch_id=$2 AND status='OPEN'`,
          [req.tenantId, req.params.id]
        );
        if (openShifts.rows[0].count > 0)
          return { code: 409, error: 'Tutup shift terlebih dahulu sebelum menonaktifkan cabang.' };
      }
      const next = {
      ...
      const updated = await client.query(
        `UPDATE branches SET name=$3,address=$4,phone=$5,is_active=$6 WHERE id=$1 AND tenant_id=$2 RETURNING *`,
        [req.params.id, req.tenantId, next.name, next.address, next.phone, next.isActive]
      );
    ...
    logger.error({ err: err.message, tenantId: req.tenantId }, 'Branch update failed');
    res.status(500).json({ error: 'Cabang gagal diperbarui.' });
  }
}
...
export async function deleteBranch(req: Request, res: Response) {
  ...
  try {
    const result = await dbTransaction(async (client) => {
      const tenantResult = await client.query(`SELECT id FROM tenants WHERE id=$1 FOR UPDATE`, [
        req.tenantId,
      ]);
      ...
      if (remainingActive === 0)
        return {
          code: 409,
          error: 'Tidak dapat menonaktifkan cabang terakhir. Minimal satu cabang harus aktif.',
        };
      const activeTicketsResult = await client.query(
        `SELECT COUNT(*)::int AS count FROM service_tickets WHERE tenant_id=$1 AND branch_id=$2 AND status NOT IN ('DIAMBIL','DIBATALKAN','SELESAI','KLAIM_GARANSI')`,
        [req.tenantId, id]
      );
      if (activeTicketsResult.rows[0].count > 0)
        return {
          code: 409,
          error: 'Selesaikan atau pindahkan tiket servis aktif terlebih dahulu.',
        };
      const openShiftsResult = await client.query(
        `SELECT COUNT(*)::int AS count FROM pos_shifts WHERE tenant_id=$1 AND branch_id=$2 AND status = 'OPEN'`,
        [req.tenantId, id]
      );
      if (openShiftsResult.rows[0].count > 0)
        return { code: 409, error: 'Tutup shift terlebih dahulu sebelum menonaktifkan cabang.' };
      const softDelete = await client.query(
        `UPDATE branches SET is_active = FALSE WHERE id=$1 AND tenant_id=$2 RETURNING *`,
        [id, req.tenantId]
      );
    ...
    logger.error({ err: err.message, tenantId: req.tenantId }, 'Branch soft delete failed');
