...
import { InventoryProduct, InventoryTransfer, Warehouse } from '../../../types';
...
interface TransferPanelProps {
  ...
  selectedTrfFromWarehouse: string;
  setSelectedTrfFromWarehouse: (value: string) => void;
  selectedTrfToWarehouse: string;
  setSelectedTrfToWarehouse: (value: string) => void;
  trfNote: string;
  setTrfNote: (value: string) => void;
  bulkInputText: string;
  setBulkInputText: (value: string) => void;
  bulkTrfItems: { productId: string; qty: number }[];
  setBulkTrfItems: React.Dispatch<React.SetStateAction<{ productId: string; qty: number }[]>>;
  trfStatusNote: string;
  setTrfStatusNote: (value: string) => void;
  selectedTrfIdForStepper: string | null;
  setSelectedTrfIdForStepper: (value: string | null) => void;
}
...
export const TransferPanel: React.FC<TransferPanelProps> = ({
  ...
  setSelectedTrfFromWarehouse,
  selectedTrfToWarehouse,
  setSelectedTrfToWarehouse,
  trfNote,
  setTrfNote,
  bulkInputText,
  setBulkInputText,
  ...
  const isAllStockValid =
    ...
    bulkTrfItems.every((item) => {
      const p = tenantProducts.find((prod) => prod.id === item.productId);
      const stock = p?.warehouseStock?.[selectedTrfFromWarehouse] || 0;
      return item.qty > 0 && stock >= item.qty;
    });

  // Get selected transfer for the visual stepper
  const activeTransfer = inventoryTransfers.find((t) => t.id === selectedTrfIdForStepper);

  // Handler to parse and validate bulk input
  const handleParseBulkInput = () => {
    if (!bulkInputText.trim()) {
      showToast('Silakan tempel teks CSV/SKU terlebih dahulu!', 'error');
      return;
    }
    ...
    lines.forEach((line) => {
      ...
      if (isNaN(qty) || qty <= 0) {
        failures.push(`Jumlah tidak valid untuk SKU ${sku}: "${qtyStr}"`);
        failedCount++;
        return;
      }
      ...
      if (!prod) {
        failures.push(`Produk dengan SKU/Barcode "${sku}" tidak ditemukan`);
        failedCount++;
        return;
      }

      // Check stock
      const availableStock = prod.warehouseStock?.[selectedTrfFromWarehouse] || 0;
      ...
      // Add to draft list
      const existingIdx = newItems.findIndex((item) => item.productId === prod.id);
      if (existingIdx !== -1) {
    ...
    if (failedCount > 0) {
      ...
      showToast(`Berhasil menambahkan ${addedCount} produk dari bulk input.`, 'success');
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {proactiveSuggestions.length > 0 && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-l-4 border-amber-500 rounded-r-xl p-5 shadow-xs space-y-3 dark:from-slate-800 dark:to-slate-800 dark:border-amber-600">
          ...
          <p className="text-[11px] text-amber-700 dark:text-amber-400">
            Sistem kami mendeteksi bahwa beberapa cabang mengalami kekurangan stok untuk produk
            tertentu, sementara cabang lain memiliki persediaan berlebih. Ambil tindakan cepat untuk
            menyeimbangkan stok di bawah ini:
          </p>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 leading-normal">
                    ...
                    <strong className="text-slate-700 dark:text-slate-300">
                      {warehouses.find((w) => w.id === sug.destWhId)?.name}
                    </strong>{' '}
                    ({sug.destStock} {sug.product.unit} / min: {sug.minStock})
                  </p>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 leading-normal">
                    Melimpah di{' '}
                    <strong className="text-slate-700 dark:text-slate-300">
                      {warehouses.find((w) => w.id === sug.originWhId)?.name}
                    </strong>{' '}
                ...
                <button
                  onClick={() => {
                    ...
                    const recommendedQty = Math.max(
                      1,
                      Math.min(sug.originStock - sug.minStock, sug.minStock - sug.destStock)
                    );
                    setBulkTrfItems([
                      {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden dark:bg-slate-900 dark:border-slate-800">
            <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 dark:bg-slate-800 dark:border-slate-800 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-xs text-slate-800 dark:text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
                  <Truck className="w-4 h-4 text-accent" /> Permintaan Transfer Multi-Barang Antar
                  Cabang
                </h3>
                <p className="text-[10px] text-slate-400 mt-0.5">
                  Pilih cabang pengirim & penerima, lalu tambahkan barang yang ingin dipindahkan.
                </p>
              </div>
            </div>
            <div className="p-5 space-y-5 text-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSelectedTrfToWarehouse(e.target.value)}
                  >
                    <option value="">-- Pilih Cabang Penerima --</option>
                    {warehouses
                      .filter(
                        (wh) =>
                          wh.tenantId === currentTenantId && wh.id !== selectedTrfFromWarehouse
                      )
                      .map((wh) => (
                        <option key={wh.id} value={wh.id}>
                          {getWarehouseLabel(wh.id)}
              ...
              {selectedTrfFromWarehouse ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="font-bold text-slate-700 dark:text-slate-300">
                      Daftar Produk yang Akan Ditransfer ({bulkTrfItems.length})
                    </h4>
                    <button
                      onClick={() => {
                        // Add first available product or empty row
                        const nonJasaProducts = tenantProducts.filter((p) => p.category !== 'JASA');
                  ...
                  {bulkTrfItems.length === 0 ? (
                    <div className="border border-dashed border-slate-200 rounded-xl p-6 text-center text-slate-400 dark:border-slate-800">
                      Belum ada produk ditambahkan. Gunakan tombol di atas atau paste bulk di
                      sebelah kanan untuk menambahkan.
                    </div>
                  ) : (
                    <div className="border border-slate-200 rounded-xl overflow-x-auto dark:border-slate-800">
                      <table className="w-full min-w-[600px] text-left text-xs">
                        <thead className="bg-slate-50 dark:bg-slate-800 text-slate-400 font-mono text-[9px] uppercase">
                          <tr>
                            <th className="px-3 py-2">Nama Barang</th>
                            <th className="px-3 py-2 text-center" style={{ width: '90px' }}>
                              Qty
                            </th>
                            <th className="px-3 py-2" style={{ width: '160px' }}>
                              Cek Stok Asal
                            </th>
                        ...
                        <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                          {bulkTrfItems.map((item, idx) => {
                            const activeProd = tenantProducts.find((p) => p.id === item.productId);
                            const availableStock =
                              activeProd?.warehouseStock?.[selectedTrfFromWarehouse] || 0;
                            const isStockSufficient = availableStock >= item.qty;
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-3 py-2 text-right">
                                  <button
                                    onClick={() => {
                                      setBulkTrfItems((prev) => prev.filter((_, i) => i !== idx));
                                    }}
                ...
                <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-xl border border-dashed border-slate-200 dark:border-slate-700 text-center text-slate-400">
                  Pilih Gudang Pengirim terlebih dahulu untuk mulai menambahkan barang & memvalidasi
                  stok real-time.
                </div>
              )}
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  onClick={() => {
                    if (!selectedTrfFromWarehouse || !selectedTrfToWarehouse) {
                      showToast('Cabang asal dan tujuan harus diisi!', 'error');
                      return;
                    }
                    if (selectedTrfFromWarehouse === selectedTrfToWarehouse) {
                ...
                >
                  <Truck className="w-4 h-4" /> Kirim Pengajuan Transfer ({bulkTrfItems.length}{' '}
                  Produk)
                </button>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-xs p-5 space-y-4 dark:bg-slate-900 dark:border-slate-800">
            <div>
              <h4 className="font-bold text-xs text-slate-800 dark:text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
                <FileSpreadsheet className="w-4 h-4 text-emerald-600" /> Upload / Paste Data Mutasi
                Bulk
              </h4>
              <p className="text-[10px] text-slate-400 mt-0.5">
                Masukkan baris data SKU produk dan kuantiti secara cepat (cocok copy-paste dari
                Excel).
              </p>
            </div>
            ...
            <div className="space-y-3">
              <label className="block text-[11px] text-slate-500 font-semibold leading-normal">
                ...
                <span className="text-[10px] text-slate-400 font-normal">
                  Satu baris untuk satu produk. Pisahkan dengan koma atau tab.
                </span>
              </label>
              <textarea
                rows={6}
              ...
              <button
                ...
                className={`w-full py-2.5 rounded-xl font-bold uppercase tracking-wider text-[10px] cursor-pointer transition shadow-xs flex items-center justify-center gap-1.5 ${
                  !selectedTrfFromWarehouse
                    ? 'bg-slate-50 border border-slate-200 text-slate-400 cursor-not-allowed dark:bg-slate-800 dark:border-slate-800 dark:text-slate-600'
                    : 'bg-emerald-600 hover:bg-emerald-700 text-white'
                }`}
              >
                <CheckSquare className="w-3.5 h-3.5" /> Parse & Validasi Stok Input
              </button>
            </div>

            <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 space-y-2 text-[10px] text-slate-500">
              <p className="font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400">
                Petunjuk Upload:
              </p>
              <ul className="list-disc pl-4 space-y-1">
                <li>
                  Pilih Gudang Pengirim terlebih dahulu sebelum menekan tombol parse agar sistem
                  dapat memvalidasi stok.
                </li>
                <li>
                  Jika SKU valid dan stok tersedia di gudang pengirim, barang akan ditambahkan ke
                  tabel draft di kiri.
                </li>
                <li>Tinjau ulang daftar item sebelum melakukan konfirmasi akhir.</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-xs p-6 space-y-6 dark:bg-slate-900 dark:border-slate-800 animate-slideUp">
          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4 gap-4">
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-1">
                Dari {getWarehouseLabel(activeTransfer.originWarehouseId)} ke{' '}
                {getWarehouseLabel(activeTransfer.destinationWarehouseId)} • Alasan:{' '}
                {activeTransfer.note}
              </p>
            </div>
            <button
          ...
          <div className="relative py-4">
            ...
            <div className="relative flex justify-between">
              ...
              <div className="flex flex-col items-center text-center space-y-1.5 w-[20%]">
                ...
                >
                  {activeTransfer.status === 'REQUEST_CREATED' ? '1' : '✓'}
                </div>
                ...
                <span className="text-[8px] text-slate-400 font-mono leading-none">
                  {new Date(
                    activeTransfer.history.find((h) => h.status === 'REQUEST_CREATED')?.timestamp ||
                      ''
                  ).toLocaleTimeString()}
                </span>
              </div>
              ...
              <div className="flex flex-col items-center text-center space-y-1.5 w-[20%]">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs border-2 transition z-15 ${
                    activeTransfer.status === 'PACKED'
                      ? 'bg-accent border-accent text-white shadow-md ring-4 ring-indigo-100 dark:ring-indigo-950'
                      : ['SHIPPED', 'RECEIVED'].includes(activeTransfer.status)
                        ? 'bg-emerald-500 border-emerald-500 text-white'
                        : 'bg-white border-slate-200 text-slate-400 dark:bg-slate-900 dark:border-slate-800'
                  }`}
                >
                  {['SHIPPED', 'RECEIVED'].includes(activeTransfer.status) ? '✓' : '2'}
                </div>
                <span className="font-bold text-[10px] text-slate-700 dark:text-slate-300">
                  Telah Dikemas (Packed)
                </span>
                <span className="text-[8px] text-slate-400 font-mono leading-none">
                  {activeTransfer.history.find((h) => h.status === 'PACKED')
                    ? new Date(
                        activeTransfer.history.find((h) => h.status === 'PACKED')!.timestamp
                      ).toLocaleTimeString()
                    : '-'}
                </span>
              </div>
              ...
              <div className="flex flex-col items-center text-center space-y-1.5 w-[20%]">
                ...
                <span className="text-[8px] text-slate-400 font-mono leading-none">
                  {activeTransfer.history.find((h) => h.status === 'SHIPPED')
                    ? new Date(
                        activeTransfer.history.find((h) => h.status === 'SHIPPED')!.timestamp
                      ).toLocaleTimeString()
                    : '-'}
                </span>
              </div>
              ...
              <div className="flex flex-col items-center text-center space-y-1.5 w-[20%]">
                ...
                <span className="text-[8px] text-slate-400 font-mono leading-none">
                  {activeTransfer.history.find((h) => h.status === 'RECEIVED')
                    ? new Date(
                        activeTransfer.history.find((h) => h.status === 'RECEIVED')!.timestamp
                      ).toLocaleTimeString()
                    : '-'}
                </span>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            <div className="space-y-3">
              ...
              <div className="border border-slate-100 dark:border-slate-800 rounded-xl p-4 bg-slate-50/50 dark:bg-slate-800/40 space-y-3.5 max-h-[160px] overflow-y-auto">
                  ...
                  >
                    ...
                    <div className="space-y-0.5">
                      <p className="font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider font-mono text-[9px]">
                        {hist.status} • {new Date(hist.timestamp).toLocaleString('id-ID')}
                      </p>
            ...
            <div className="space-y-3">
              ...
              <div className="border border-slate-100 dark:border-slate-800 rounded-xl p-4 bg-slate-50/50 dark:bg-slate-800/40 space-y-4">
                <div>
                ...
                <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => {
                          updateInventoryTransferStatus(
                            activeTransfer.id,
                            'PACKED',
                            trfStatusNote || 'Barang selesai dipacking, siap diambil kurir'
                          );
                          setTrfStatusNote('');
                        }}
                      ...
                      <button
                        onClick={() => {
                          updateInventoryTransferStatus(
                            activeTransfer.id,
                            'SHIPPED',
                            trfStatusNote || 'Barang dikirim dan keluar dari gudang asal'
                          );
                          setTrfStatusNote('');
                        }}
                    ...
                    <button
                      onClick={() => {
                        updateInventoryTransferStatus(
                          activeTransfer.id,
                          'SHIPPED',
                          trfStatusNote || 'Barang diserahkan ke driver logistik (In Transit)'
                        );
                        setTrfStatusNote('');
                      }}
                      className="px-3.5 py-2 bg-accent hover:bg-accent-hover text-white font-bold text-[10px] uppercase rounded-lg cursor-pointer transition shadow-xs"
                    ...
                    <button
                      onClick={() => {
                        updateInventoryTransferStatus(
                          activeTransfer.id,
                          'RECEIVED',
                          trfStatusNote || 'Barang telah tiba dan masuk kartu stok gudang tujuan'
                        );
                        setTrfStatusNote('');
                      }}
                  ...
                  {activeTransfer.status === 'RECEIVED' && (
                    <p className="text-[11px] text-emerald-600 dark:text-emerald-400 font-bold">
                      ✓ Siklus mutasi logistik barang selesai dikonfirmasi. Stok telah bertambah di
                      cabang penerima.
                    </p>
                  )}
                </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden dark:bg-slate-900 dark:border-slate-800">
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 dark:bg-slate-800 dark:border-slate-800 flex items-center justify-between">
          <div>
            <h3 className="font-bold text-xs text-slate-800 dark:text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
              <History className="w-4 h-4 text-accent" /> Buku Besar & Ledger Mutasi Antar Cabang
            </h3>
            <p className="text-[10px] text-slate-400 mt-0.5">
              Daftar lengkap audit trail seluruh pergerakan barang antar gudang cabang beserta
              status pengiriman aktual.
            </p>
          </div>
        </div>
        ...
        <div className="responsive-table-container overflow-x-auto">
          <table className="w-full min-w-[900px] text-left text-xs">
            <thead className="bg-slate-50 dark:bg-slate-800 text-slate-400 uppercase text-[9px] font-mono">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              {inventoryTransfers.filter((t) => t.tenantId === currentTenantId).length === 0 && (
                <tr>
                  <td
                    colSpan={7}
                    className="text-center py-8 text-slate-400 dark:text-slate-500 font-medium"
                  >
                    Belum ada riwayat transfer logistik antar cabang yang diajukan.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden dark:bg-slate-900 dark:border-slate-800">
          <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 dark:bg-slate-800 dark:border-slate-800 flex items-center justify-between">
            <h3 className="font-bold text-xs text-slate-800 dark:text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
              <Layers className="w-4 h-4 text-emerald-600" /> Distribusi Stok Multi-Cabang Saat Ini
            </h3>
          ...
          <div className="p-4 space-y-4 text-xs max-h-[400px] overflow-y-auto">
            {tenantProducts
              .filter((p) => p.category !== 'JASA')
              .map((p) => {
                const whs = warehouses.filter((w) => w.tenantId === currentTenantId);
                return (
                  <div
                    key={p.id}
                    className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2 dark:bg-slate-800/50 dark:border-slate-800"
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-[9px] font-mono text-slate-400">SKU: {p.sku}</p>
                      </div>
                    ...
                    <div className="divide-y divide-slate-200 text-[10px] bg-white rounded-lg p-2 border border-slate-100 space-y-1.5 dark:bg-slate-900 dark:border-slate-800 dark:divide-slate-800">
                      {whs.map((wh) => {
                        const stockInWh = p.warehouseStock?.[wh.id] || 0;
                        const pct = p.stockQty > 0 ? (stockInWh / p.stockQty) * 100 : 0;
                        return (
                          <div key={wh.id} className="pt-1.5 first:pt-0">
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-xs p-5 space-y-4 dark:bg-slate-900 dark:border-slate-800">
          <h4 className="font-bold text-slate-800 dark:text-slate-200 text-xs uppercase tracking-wider flex items-center gap-1">
            <Truck className="w-4 h-4 text-accent" /> Catatan Alur & Kebijakan Transfer
          </h4>
          <div className="space-y-3.5 text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">
            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl">
              ...
              <p className="mt-1">
                <strong>Request Created</strong> (Diajukan) → <strong>Packed</strong> (Kemas barang)
                → <strong>Shipped</strong> (Driver jalan, stok di cabang pengirim langsung dipotong)
                → <strong>Received</strong> (Barang tiba di tujuan & terkonfirmasi, stok cabang
                penerima bertambah).
              </p>
            </div>
            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl">
              ...
              <p className="mt-1">
                Sistem akan melarang pengiriman jika kuantiti mutasi melebihi sisa stok real-time
                yang terdaftar di lokasi gudang pengirim. Ini mencegah terjadinya pencatatan stok
                negatif.
              </p>
            </div>
            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl">
              ...
              <p className="mt-1">
                Setiap penyesuaian status pada alur pengiriman di atas akan otomatis mengunci nama
                pengguna (operator), melampirkan timestamp lokal presisi, dan mencatat mutasi stok
                ke sistem log audit internal untuk perlindungan aset anti-fraud.
              </p>
            </div>
          </div>
        </div>
