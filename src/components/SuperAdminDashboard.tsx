...
export const SuperAdminDashboard: React.FC<{
    ...
    const initSession = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          if (
            data.code === 'SUPERADMIN_SESSION_EXPIRED' ||
            data.code === 'SUPERADMIN_SESSION_REQUIRED'
          ) {
  ...
  const [configTier, setConfigTier] = useState<SubscriptionTier>(SubscriptionTier.PRO);
  const [configStatus, setConfigStatus] = useState<TenantStatus>(TenantStatus.ACTIVE);
  ...
  const systemStorageCount = tenants.filter(
    (t) =>
      !(t.settings as any)?.storageSettings || (t.settings as any).storageSettings.mode === 'SYSTEM'
  ).length;
  ...
  // Sync billing sub-tab with activeSubTab from App context
  useEffect(() => {
    if (currentTab === 'billing-management' && activeSubTab) {
      setBillingSubTab((prev) => (prev !== activeSubTab ? activeSubTab : prev));
    }
    ...
    <div className="space-y-6" id="super-admin-root">
      ...
      {currentTab === 'billing-management' && (
        <div className="space-y-6">
          ...
          <div className="pt-4">
            {billingSubTab === 'billing-plans' && (
              <BillingPlansManager readOnlyMode={readOnlyMode} />
            )}
