...
const SHORTCUTS: ShortcutItem[] = [
  ...
  { keys: 'G lalu D', label: 'Buka Data Explorer' },
  { keys: '⌘ / Ctrl + ⇧ + M', label: 'Ganti mode navigasi (Sidebar ↔ Horizontal)' },
  { keys: '⌘ / Ctrl + ⇧ + T', label: 'Ganti tema (Terang ↔ Gelap)' },
      ...
      >
        ...
        <div className="space-y-1.5">
          {SHORTCUTS.map((s) => (
            ...
            >
              <span className="text-xs text-slate-600 dark:text-zinc-300">{s.label}</span>
        ...
        <p className="text-[10px] text-slate-400 dark:text-zinc-500 mt-4 leading-relaxed">
          Tekan <kbd className="font-mono">G</kbd> lalu huruf dalam 1 detik untuk berpindah modul
          dengan cepat. Pintasan tidak aktif saat mengetik di input. Gunakan{' '}
          <kbd className="font-mono">⌘/Ctrl + K</kbd> untuk Command Palette dan{' '}
          <kbd className="font-mono">?</kbd> untuk menampilkan daftar pintasan ini kapan saja.
        </p>
      </div>
    </div>,
    document.body
