    ...
    <div className="w-full max-w-6xl animate-fadeIn space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
        ...
        <button
          ...
          className="bg-accent hover:bg-accent-hover text-white font-extrabold text-xs px-4 py-2 rounded-xl flex items-center gap-1.5 cursor-pointer shadow-sm transition-all"
        >
        ...
        <div className="bg-slate-50 border border-indigo-100 rounded-2xl p-5 animate-fadeIn space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-200">
            <h4 className="font-extrabold text-xs text-indigo-950 uppercase tracking-wider flex items-center gap-1.5">
              <Sparkles className="w-4 h-4 text-accent animate-pulse" /> Buat Alur Kerja Otomatis Baru
            </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <label className="block">
              ...
              <input type="text" value={wfName} onChange={(e) => setWfName(e.target.value)} placeholder="Contoh: Pengingat WhatsApp Invoice Jatuh Tempo" className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl outline-none focus:border-accent text-xs font-medium" />
            </label>
            <label className="block">
              <span className="block text-[10px] font-mono text-slate-500 uppercase mb-1">Pemicu Kejadian (Trigger Event)</span>
              <select value={wfTriggerType} onChange={(e) => setWfTriggerType(e.target.value as any)} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl outline-none focus:border-accent text-xs font-semibold text-slate-800">
            ...
            <label className="block">
              ...
              <input type="text" value={wfTriggerCondition} onChange={(e) => setWfTriggerCondition(e.target.value)} placeholder="Contoh: > 30, < 5, all" className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl outline-none focus:border-accent text-xs font-mono font-bold" />
            </label>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <label className="block">
              ...
              <select value={wfActionType} onChange={(e) => setWfActionType(e.target.value as any)} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl outline-none focus:border-accent text-xs font-semibold text-slate-800">
            ...
            <label className="md:col-span-2 block">
              ...
              <textarea value={wfActionPayload} onChange={(e) => setWfActionPayload(e.target.value)} rows={2} placeholder="Isi konten notifikasi otomatis atau entri log..." className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl outline-none focus:border-accent text-xs font-medium font-sans" />
            </label>
          </div>
          ...
          <div className="flex justify-end gap-2.5 pt-1">
            ...
            <button
              ...
              className="px-4 py-2 bg-accent hover:bg-accent-hover text-white font-black rounded-xl transition-all cursor-pointer text-xs shadow-sm"
            >
              Aktifkan Alur Kerja
            </button>
          </div>
              ...
              <div key={w.id} className={`border rounded-2xl p-5 transition-all relative overflow-hidden flex flex-col justify-between ${isWfActive ? "bg-white border-slate-200 shadow-sm hover:border-indigo-200" : "bg-slate-50/70 border-slate-200 opacity-75"}`}>
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-50/80 border border-slate-100 rounded-xl p-3 space-y-1.5 text-xs">
                    <div className="flex items-center gap-1 text-[10px] font-mono text-accent uppercase font-black">
                ...
                <div className="flex items-center justify-between border-t border-slate-100 pt-3.5 mt-4 text-[10px] text-slate-500">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={async () => { try { await executeWorkflow(w.id); showToast(`Pemicu alur "${w.name}" tersimpan.`, "success"); } catch (error: any) { showToast(error?.message || "Gagal menjalankan alur.", "error"); } }} disabled={!isWfActive} className={`px-3 py-1.5 rounded-lg font-bold flex items-center gap-1 transition-all text-[11px] cursor-pointer ${isWfActive ? "bg-accent-lighter hover:bg-indigo-100 text-accent border border-indigo-100" : "bg-slate-100 text-slate-400 cursor-not-allowed"}`}>
                      <Play className="w-3 h-3 text-accent fill-indigo-600" /> Uji Alur
                    </button>
