...
export const AuditRecovery: React.FC<AuditRecoveryProps> = ({
  auditLogs,
  handleExportBackup,
  handleImportBackup,
  readOnlyMode = false,
  ...
  useEffect(() => {
    const timer = window.setTimeout(loadOperations, 250);
  ...
  const displayedAudit = useMemo(
    () =>
      auditItems.filter(
        (log) =>
          !auditSearch || JSON.stringify(log).toLowerCase().includes(auditSearch.toLowerCase())
      ),
  ...
  const exportAudit = (format: 'json' | 'csv') => {
    const content =
      format === 'json'
        ? JSON.stringify(displayedAudit, null, 2)
        : [
            'timestamp,actor,tenant,action,outcome,resource,correlationId',
            ...displayedAudit.map((log) =>
              [
                log.created_at || log.timestamp,
                log.actor_name || log.userName,
    ...
    anchor.href = url;
    anchor.download = `superadmin-audit-${new Date().toISOString().slice(0, 10)}.${format}`;
    anchor.click();
    URL.revokeObjectURL(url);
  };

  const recordSnapshot = async () => {
    const response = await apiFetch('/api/superadmin/backups', {
      method: 'POST',
      headers: { 'X-SuperAdmin-Mode': readOnlyMode ? 'read-only' : 'edit' },
      body: JSON.stringify({
        mode: 'SNAPSHOT',
        fileName: `application-snapshot-${new Date().toISOString().slice(0, 10)}.json`,
        schemaVersion: 1,
        summary: { source: 'superadmin-console' },
      }),
    });
  ...
  return (
    <div className="space-y-6" id="sa-bottom-infrastructure">
      <nav className="flex flex-wrap gap-2" aria-label="Keamanan dan recovery">
        {(
          [
            ['audit', 'Audit'],
          ...
          <button
            ...
            className={`rounded-xl px-4 py-2 text-xs font-bold ${activeSection === id ? 'bg-accent text-white' : 'border border-slate-200 bg-white text-slate-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300'}`}
          >
            {label}
          </button>
        ))}
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <h3 className="flex items-center gap-2 text-sm font-black text-slate-900 dark:text-white">
              <Database className="h-4 w-4" /> Snapshot Aplikasi & Pemulihan
            </h3>
            <p className="mt-2 text-xs leading-relaxed text-slate-500 dark:text-zinc-400">
              Snapshot JSON hanya untuk ekspor data aplikasi. Restore database belum tersedia dan
              harus dilakukan melalui prosedur terkelola.
            </p>
            <button
              type="button"
              disabled={readOnlyMode}
              onClick={recordSnapshot}
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-accent py-2.5 text-xs font-bold text-white disabled:opacity-40"
            >
              <Download className="h-4 w-4" /> Catat & unduh snapshot
            </button>
            <p className="mt-3 rounded-xl border border-dashed border-slate-300 p-3 text-center text-xs text-slate-500 dark:border-zinc-700">
              Restore snapshot dinonaktifkan sampai endpoint restore database transaksional
              tersedia.
            </p>
          </article>
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-2 dark:border-zinc-800 dark:bg-zinc-900">
            <div className="flex items-center justify-between">
              <h3 className="flex items-center gap-2 text-sm font-black text-slate-900 dark:text-white">
                <ArchiveRestore className="h-4 w-4" /> Riwayat Snapshot
              </h3>
              <button
                type="button"
                onClick={loadOperations}
                className="rounded-lg border border-slate-200 p-2 dark:border-zinc-700"
                ...
                backupJobs.map((job) => (
                  ...
                  >
                    <span>
                      <b>{job.job_type}</b>
                      <br />
                      <span className="text-slate-500">{job.file_name || job.source}</span>
                    </span>
      ...
      {activeSection === 'audit' && (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4 dark:border-zinc-800">
            <div>
              <h3 className="flex items-center gap-2 text-sm font-black text-slate-900 dark:text-white">
                <ShieldAlert className="h-4 w-4 text-rose-500" /> Audit Super Admin
              </h3>
              <p className="mt-1 text-xs text-slate-500">
                Actor, tenant, outcome, resource, IP, correlation ID, dan perubahan state.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="rounded-xl bg-slate-900 px-3 py-2 text-xs font-bold text-white dark:bg-zinc-100 dark:text-zinc-900"
              >
                CSV
              </button>
              <button
                ...
                >
                  <summary className="cursor-pointer list-none">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <span>
                        <b>{log.action}</b> · {log.actor_name || log.userName || 'System'}
                        <br />
                        <span className="text-slate-500">
                          {log.tenant_name || log.effective_tenant_id || 'Platform'} ·{' '}
                          {log.resource_type || log.category}
                        </span>
                      </span>
                      <span className="text-right font-bold">
                        {log.outcome || log.riskLevel || 'SUCCESS'}
                        <br />
                        <span className="font-normal text-slate-400">
                  ...
                  <pre className="mt-3 overflow-x-auto rounded-lg bg-slate-50 p-3 text-[11px] dark:bg-zinc-950">
                    {JSON.stringify(
                      {
                        correlationId: log.correlation_id,
                        clientIp: log.client_ip || log.ipAddress,
