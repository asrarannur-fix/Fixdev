...
export function DataTable<T>({
  ...
  const filtered = useMemo(() => {
    ...
    if (globalQuery.trim() && searchable) {
      ...
      const keys = searchKeys ?? columns.map((c) => c.accessor).filter((k): k is keyof T => !!k);
      result = result.filter((row) =>
        keys.some((k) =>
          String((row as any)[k] ?? '')
            .toLowerCase()
    ...
    return result;
  }, [data, globalQuery, columnFilters, searchable, searchKeys, columns, serverSide]);
  ...
  const pageCount = Math.max(
    1,
    Math.ceil((serverSide ? (totalRows ?? data.length) : sorted.length) / pageSize)
  );
  const current = Math.min(page, pageCount - 1);
  const paged = serverSide ? data : sorted.slice(current * pageSize, current * pageSize + pageSize);
    ...
    >
        ...
        <div className="flex flex-wrap gap-2 px-3 py-2 border-b border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-950/50">
          {filterCols.map((col) => (
            <div key={col.key} className="flex flex-col gap-0.5 min-w-[100px]">
              ...
              <input
                value={String(
                  columnFilters[col.key as keyof DataTableState['columnFilters']] ?? ''
                )}
            ...
            {paged.length === 0 ? (
              <tr>
                <td colSpan={columns.length} className="p-8 text-center">
                  {emptyState ?? (
                    <span className="text-slate-400 dark:text-zinc-500">Tidak ada data</span>
                  )}
                </td>
              </tr>
            ) : (
                ...
                >
                  {columns.map((col) => (
                    ...
                    >
                      {col.render
                        ? col.render(row)
                        : String(col.accessor ? ((row as any)[col.accessor] ?? '') : '')}
                    </td>
                  ))}
                </tr>
              ))
      ...
      {pageCount > 1 && (
        <div className="flex items-center justify-between px-4 py-2 border-t border-slate-200 dark:border-zinc-800 text-[11px] text-slate-500 dark:text-zinc-400">
          <span>
            {serverSide ? (totalRows ?? data.length) : sorted.length} baris · hal {current + 1}/
            {pageCount}
          </span>
          <div className="flex items-center gap-1">
            <button
              onClick={() => emitState({ ...tableState, page: Math.max(0, current - 1) })}
              disabled={current === 0}
            ...
            <button
              onClick={() =>
                emitState({ ...tableState, page: Math.min(pageCount - 1, current + 1) })
              }
