...
export const InventoryStockPanel: React.FC<any> = (props) => {
  const {
    ...
    editProdMinStock,
    editProdName,
    editProdPurchaseCost,
    editProdSellPrice,
    editProdSku,
    editProdStorageLocId,
    editProdWarehouseStock,
    expandedProductIds,
    getBranchStock,
    isAddProductOpen,
    isEditProductOpen,
    ...
    setEditProdSku,
    setEditProdStockQty,
    setEditProdStorageLocId,
    setEditProdWarehouseStock,
    ...
    <div className="space-y-4">
        ...
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              Kelola data logistik, harga beli (HPP), harga jual, dan status reorder stok.
            </p>
          </div>
              ...
              {tenantProducts.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3.5">
                    ...
                    {p.storageLocationId &&
                      (() => {
                        const loc = getStorageLocations(currentTenantId).find(
                          (l) => l.id === p.storageLocationId
                        );
                        return loc ? (
                        ...
                        {branches
                          ...
                          .map((b) => {
                            const branchWhIds = warehouses
                              ...
                              .map((w) => w.id);
                            const sum = branchWhIds.reduce(
                              (s, whId) => s + (Number(p.warehouseStock?.[whId]) || 0),
                              0
                            );
                            return (
                              <div key={b.id} className="flex justify-between items-center gap-2">
                                <span className="text-[9px] font-semibold text-slate-500 truncate w-1/2">
                                  {b.name}
                                </span>
                  ...
                  <td className="px-4 py-3.5 text-right">
                    {p.category !== 'JASA' && getBranchStock(p) <= p.minStock ? (
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <button
                      onClick={() => {
                        ...
                        setEditProdSellPrice((p.sellPrice ?? 0).toString());
                        setEditProdPurchaseCost((p.purchaseCost ?? 0).toString());
      ...
      {isAddProductOpen &&
        createPortal(
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div
              className="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 animate-fadeIn overflow-y-auto my-auto"
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-900 pb-3">
                <h3 className="font-extrabold text-sm text-slate-800 dark:text-slate-100 uppercase tracking-wide flex items-center gap-2">
                  <PlusCircle className="w-5 h-5 text-accent animate-pulse" /> Tambah Barang Baru
                </h3>
                <button
              ...
              <div className="grid grid-cols-2 gap-3.5 text-xs">
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">SKU</label>
                  <input
                    type="text"
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl"
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Barcode</label>
                  <input
                    type="text"
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setAddProdCategory(e.target.value as any)}
                  >
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Satuan</label>
                  <input
                    type="text"
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl"
                    value={addProdUnit}
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Harga Jual</label>
                  <input
                    type="number"
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-mono"
                    placeholder="Rp"
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Stok Awal</label>
                  <input
                    type="number"
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-mono"
                    placeholder="Jumlah"
                ...
                <div className="col-span-2 grid grid-cols-2 gap-3.5">
                  <div>
                    <label className="block text-slate-500 font-semibold mb-1">Cabang</label>
                    <select
                      value={addProdBranchId}
                      onChange={(e) => {
                        setAddProdBranchId(e.target.value);
                        setAddProdWarehouseId('');
                      }}
                  ...
                  <div>
                    <label className="block text-slate-500 font-semibold mb-1">Gudang Tujuan</label>
                    <select
                    ...
                    >
                      ...
                      {(warehouses || [])
                        .filter(
                          (w) => w.tenantId === currentTenantId && w.branchId === addProdBranchId
                        )
                        .map((w) => (
                ...
                <div className="col-span-2">
                  ...
                  >
                    ...
                    {getStorageLocations(currentTenantId)
                      .filter((l) => l.branchId === currentBranchId && l.type === 'SPAREPART')
                      .map((l) => (
                ...
                <button
                  onClick={() => {
                    ...
                    const warehouseStockObj = addProdWarehouseId
                      ...
                      : {
                          [tenantWhs[0]?.id || warehouses[0]?.id || '']:
                            Number(addProdStockQty) || 0,
                        };
                    const totalStock = Object.values(warehouseStockObj).reduce(
                      (sum, q) => sum + (q || 0),
                      0
                    );
                    ...
                    const cleanBarcode = (
                      addProdBarcode.trim() || Date.now().toString().slice(-12)
                    ).toUpperCase();
      ...
      {isEditProductOpen &&
        ...
        createPortal(
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div
              className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 animate-fadeIn overflow-y-auto my-auto"
              style={{ maxHeight: '90vh' }}
            >
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-900 pb-3">
                <h3 className="font-extrabold text-sm text-slate-800 dark:text-slate-100 uppercase tracking-wide flex items-center gap-2">
                  <Pencil className="w-4 h-4 text-accent" /> Ubah Detail & Harga Barang
                </h3>
                <button
              ...
              <div className="space-y-3.5 text-xs">
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Nama Barang</label>
                  <input
                    type="text"
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl"
                    value={editProdName}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditProdPurchaseCost(e.target.value)}
                    />
                  </div>
                ...
                <div>
                  <label className="block text-slate-500 font-semibold mb-1">Stok per Gudang</label>
                        ...
                        return (
                          <div key={w.id} className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setEditProdWarehouseStock((prev) => ({
                                  ...prev,
                                  [w.id]: e.target.value,
                                }))
                              }
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      value={Object.values(editProdWarehouseStock as Record<string, number>).reduce(
                        (s, v) => s + (Number(v) || 0),
                        0
                      )}
                      readOnly
