...
export async function notifyTelegramManualPayment(request: {
  id: string;
  version: number;
  invoice_id: string;
  ...
  try {
    const configured = await dbQuery(
      `SELECT value FROM app_settings WHERE key='platform_telegram_manual_payment'`
    );
    config = configured.rows[0]?.value || {};
  } catch (error: any) {
    logger.warn(
      { err: error.message },
      'Platform Telegram config unavailable; using environment fallback'
    );
  }
  ...
  try {
    admins =
      (config.adminMap as Record<string, string> | undefined) ||
      (JSON.parse(process.env.PLATFORM_TELEGRAM_ADMIN_MAP || '{}') as Record<string, string>);
  } catch {
  ...
  const text = [
    'Pembayaran manual menunggu verifikasi',
    `Invoice: ${request.invoice_id}`,
    `Nominal: Rp${Number(request.amount).toLocaleString('id-ID')}`,
    `Metode: ${request.method === 'MANUAL_QRIS' ? 'QRIS manual' : 'Transfer bank'}`,
    `Pengirim: ${request.payer_name}`,
  ].join('\n');
  const reply_markup = {
    inline_keyboard: [
      [{ text: 'Setujui', callback_data: `mp:a:${request.id}:${request.version}` }],
    ],
  };
  await Promise.allSettled(
    Object.keys(admins).map(async (chatId) => {
...
async function readManualConfig(): Promise<ManualPaymentConfig> {
  const result = await dbQuery(`SELECT value FROM app_settings WHERE key=$1 LIMIT 1`, [
    MANUAL_CONFIG_KEY,
  ]);
  ...
  try {
    ...
    if (config.manualQrisEnabled && config.qrisObjectPath) {
      try {
        ...
        logger.warn(
          { path: config.qrisObjectPath, err: fileErr.message },
          'QRIS image file missing, returning config without image'
        );
      }
    }
...
export async function createManualQrisUpload(req: Request, res: Response) {
  ...
  if (
    !fileName ||
    !ALLOWED_QRIS_TYPES.has(contentType) ||
    !Number.isInteger(sizeBytes) ||
    sizeBytes < 1 ||
    sizeBytes > MAX_QRIS_BYTES
  ) {
  ...
  try {
    ...
    await dbTransaction(async (client) =>
      platformAudit(
        client,
        req,
        'MANUAL_PAYMENT_QRIS_UPLOAD_CREATED',
        'manual_payment_qris',
        objectPath,
        null,
        { objectPath, fileName }
    ...
    res.json({
      objectPath,
      signedUploadUrl: `/api/billing/manual-payment-config/qris-upload/${path.basename(objectPath)}`,
      expiresIn: 60,
    });
  } catch (err: any) {
...
export async function uploadManualQris(req: Request, res: Response) {
  ...
  if (
    !/^qris-[0-9a-f-]+\.(jpg|png)$/.test(fileName) ||
    !Buffer.isBuffer(req.body) ||
    req.body.length < 1 ||
    req.body.length > MAX_QRIS_BYTES
  ) {
    return res.status(422).json({ error: 'File QRIS tidak valid.' });
  }
  const contentType = String(req.headers['content-type'] || '').split(';')[0];
  if (!ALLOWED_QRIS_TYPES.has(contentType) || !matchesImageSignature(req.body, contentType))
    return res.status(422).json({ error: 'Isi file QRIS tidak sesuai format JPG/PNG.' });
  try {
...
export async function updateManualPaymentConfig(req: Request, res: Response) {
  const body = req.body || {};
  if (typeof body.bankTransferEnabled !== 'boolean' || typeof body.manualQrisEnabled !== 'boolean')
    return res.status(422).json({ error: 'Flag metode pembayaran harus boolean.' });
  const config: ManualPaymentConfig = {
    ...
    qrisOriginalName: String(body.qrisOriginalName || '')
      .trim()
      .slice(0, 255),
  };
  if (
    config.bankTransferEnabled &&
    (!config.bankName || !config.accountNumber || !config.accountHolder)
  ) {
    return res.status(422).json({ error: 'Bank, nomor rekening, dan nama pemilik wajib diisi.' });
  }
  if (
    config.manualQrisEnabled &&
    !/^platform\/manual-payment\/qris-[0-9a-f-]+\.(jpg|png)$/.test(config.qrisObjectPath)
  ) {
  ...
  try {
    ...
    await dbTransaction(async (client) => {
      const before = await client.query(`SELECT value FROM app_settings WHERE key=$1`, [
        MANUAL_CONFIG_KEY,
      ]);
      await client.query(
...
async function platformAudit(
  ...
  await client.query(
    `INSERT INTO superadmin_audit_events(actor_user_id,actor_role,effective_tenant_id,action,resource_type,resource_id,outcome,client_ip,before_state,after_state,metadata) VALUES($1,$2,NULL,$3,$4,$5,'SUCCESS',$6,$7::jsonb,$8::jsonb,'{}'::jsonb)`,
    [
...
export async function createManualProofUpload(req: Request, res: Response) {
  ...
  if (
    ...
    return res
      .status(422)
      .json({ error: 'Bukti harus JPG, PNG, atau PDF dengan ukuran maksimal 5 MB.' });
  }

  const invoice = await dbQuery(
    `SELECT id, tenant_id, amount, status FROM saas_invoices WHERE id = $1 AND tenant_id = $2`,
  ...
  if (invoice.rows[0].status === 'PAID')
    return res.status(409).json({ error: 'Invoice sudah lunas.' });
  ...
  try {
    ...
    if (req.body && (req.body as any).fileBuffer !== undefined) {
      ...
      if (
        ...
        fileBuffer.length < 1 ||
        fileBuffer.length > MAX_PROOF_BYTES ||
        fileBuffer.length !== sizeBytes
      ) {
        return res.status(422).json({ error: 'Isi bukti pembayaran tidak valid.' });
      }
      if (
        (contentType === 'application/pdf' && !matchesPdfSignature(fileBuffer)) ||
        (contentType !== 'application/pdf' && !matchesImageSignature(fileBuffer, contentType))
      ) {
    ...
    return res.json({
      uploadId,
      objectPath,
      signedUploadUrl: `/api/billing/invoices/${invoiceId}/manual-payments/uploads/${uploadId}.${extensionFor(contentType)}`,
      fileName,
      contentType,
      sizeBytes,
    });
...
export async function uploadManualProof(req: Request, res: Response) {
  ...
  const contentType = String(req.headers['content-type'] || '').split(';')[0];
  if (
    !/^[0-9a-f-]+\.(jpg|png|pdf)$/.test(fileName || '') ||
    !ALLOWED_PROOF_TYPES.has(contentType) ||
    !Buffer.isBuffer(req.body) ||
    req.body.length < 1 ||
    req.body.length > MAX_PROOF_BYTES
  ) {
    return res.status(422).json({ error: 'Bukti pembayaran tidak valid.' });
  }
  if (
    (contentType === 'application/pdf' && !matchesPdfSignature(req.body)) ||
    (contentType !== 'application/pdf' && !matchesImageSignature(req.body, contentType))
  ) {
    return res.status(422).json({ error: 'Isi bukti pembayaran tidak sesuai format.' });
  }
  const invoice = await dbQuery(`SELECT id FROM saas_invoices WHERE id=$1 AND tenant_id=$2`, [
    invoiceId,
    req.tenantId,
  ]);
  if (!invoice.rows[0]) return res.status(404).json({ error: 'Invoice tidak ditemukan.' });
...
export async function submitManualPayment(req: Request, res: Response) {
  ...
  if (
    !['BANK_TRANSFER', 'MANUAL_QRIS'].includes(method) ||
    !Number.isFinite(paidAtTime) ||
    paidAtTime > Date.now() + 5 * 60_000 ||
    !payerName?.trim() ||
    payerName.trim().length > 120 ||
    !referenceNumber?.trim() ||
    referenceNumber.trim().length > 120 ||
  ...
  if (
    (method === 'BANK_TRANSFER' && !manualConfig.bankTransferEnabled) ||
    (method === 'MANUAL_QRIS' && !manualConfig.manualQrisEnabled)
  ) {
  ...
  try {
    ...
    const result = await dbTransaction(async (client) => {
      const locked = await client.query(
        `SELECT id, tenant_id, amount, status FROM saas_invoices WHERE id = $1 AND tenant_id = $2 FOR UPDATE`,
        [invoiceId, req.tenantId]
      );
      ...
      const inserted = await client.query(
        ...
        [
          ...
          sizeBytes,
          req.authActor?.userId,
          req.authActor?.role === 'SUPER_ADMIN' ? req.authActor.userId : null,
          supersedesId || null,
        ]
      );
      await client.query(
        `UPDATE saas_invoices SET status = 'PENDING_VERIFICATION', version = version + 1 WHERE id = $1`,
        [invoiceId]
      );
      await audit(client, req, 'MANUAL_PAYMENT_SUBMITTED', inserted.rows[0].id, invoice.tenant_id, {
        method,
        invoiceId,
      });
    ...
    try {
      await dbTransaction((client) =>
        notify(
          client,
          `manual_submitted:${request.id}`,
          req.tenantId!,
          'SUPER_ADMIN',
          'Pembayaran menunggu verifikasi',
          `Bukti pembayaran invoice ${invoiceId} telah dikirim.`,
          request.id
        )
      );
    } catch (err: any) {
      logger.error(
        { err: err.message, requestId: request.id },
        'Manual payment notification failed'
      );
    }
    try {
      await notifyTelegramManualPayment(request);
    } catch (err: any) {
      logger.error(
        { err: err.message, requestId: request.id },
        'Telegram manual payment notification failed'
      );
    }
    return res.status(201).json({ success: true, ...(result as any) });
  } catch (err: any) {
    if (err.code === 'ENOENT')
      return res.status(422).json({ error: 'Bukti pembayaran belum diunggah.' });
    if (err.code === '23505')
      return res
        .status(409)
        .json({ error: 'Invoice ini sudah memiliki pengajuan yang sedang ditinjau.' });
  ...
  try {
    ...
    const result = await dbQuery(
      `SELECT m.*, i.tier, i.billing_cycle, t.name AS tenant_name
       ...
       ORDER BY m.submitted_at DESC LIMIT 100`,
      params
    );
...
export async function getManualProofUrl(req: Request, res: Response) {
  ...
  const result = await dbQuery(
    `SELECT proof_object_path FROM manual_payment_requests WHERE id = $1${scope}`,
    params
  );
...
export async function streamManualProof(req: Request, res: Response) {
  ...
  const result = await dbQuery(
    `SELECT proof_object_path,proof_content_type,proof_original_name FROM manual_payment_requests WHERE id=$1${scope}`,
    params
  );
  ...
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `inline; filename="${String(proof.proof_original_name || 'proof').replace(/["\\]/g, '')}"`
    );
...
export async function reviewManualPaymentByActor(
  paymentId: string,
  expectedVersion: number,
  decision: 'APPROVED' | 'REJECTED',
  actor: { userId: string; role: string },
  reason = 'Ditolak melalui Telegram'
) {
  const result = await dbTransaction(async (client) => {
    const locked = await client.query(
      `SELECT m.*, i.tier, i.billing_cycle, i.status AS invoice_status, i.plan_snapshot
       FROM manual_payment_requests m JOIN saas_invoices i ON i.id = m.invoice_id
        WHERE m.id = $1 FOR UPDATE OF m, i`,
      [paymentId]
    );
    const payment = locked.rows[0];
    if (!payment) return { code: 404, error: 'Pengajuan tidak ditemukan.' };
    if (payment.status !== 'SUBMITTED' || payment.version !== expectedVersion)
      return { code: 409, error: 'Pengajuan sudah berubah. Muat ulang data.' };
    if (payment.invoice_status !== 'PENDING_VERIFICATION')
      return { code: 409, error: 'Status invoice tidak lagi menunggu verifikasi.' };

    if (decision === 'REJECTED') {
      await client.query(
        `UPDATE manual_payment_requests SET status='REJECTED', rejection_reason=$2, reviewed_by=$3, reviewed_at=now(), version=version+1 WHERE id=$1`,
        [payment.id, reason, actor.userId]
      );
      await client.query(
        `UPDATE saas_invoices SET status = CASE WHEN due_date < CURRENT_DATE THEN 'OVERDUE' ELSE 'UNPAID' END, version=version+1 WHERE id=$1 AND status='PENDING_VERIFICATION'`,
        [payment.invoice_id]
      );
    } else {
      if (payment.invoice_status === 'PAID')
        return { code: 409, error: 'Invoice sudah diselesaikan oleh transaksi lain.' };
      const periodEnd = new Date();
      if (payment.billing_cycle === 'yearly') periodEnd.setFullYear(periodEnd.getFullYear() + 1);
      else periodEnd.setMonth(periodEnd.getMonth() + 1);
      await client.query(
        `UPDATE manual_payment_requests SET status='APPROVED', reviewed_by=$2, reviewed_at=now(), version=version+1 WHERE id=$1`,
        [payment.id, actor.userId]
      );
      await client.query(
        `INSERT INTO billing_transactions (invoice_id,tenant_id,channel,method,amount,status,provider_transaction_id) VALUES ($1,$2,'MANUAL',$3,$4,'SETTLEMENT',$5)`,
        [payment.invoice_id, payment.tenant_id, payment.method, payment.amount, payment.id]
      );
      await client.query(
        `UPDATE saas_invoices SET status='PAID', paid_at=now(), period_start=now(), period_end=$2, version=version+1 WHERE id=$1`,
        [payment.invoice_id, periodEnd]
      );
      await client.query(
        `UPDATE tenants SET status='ACTIVE', tier=$2, trial_ends_at=$3, settings=COALESCE(settings,'{}'::jsonb) || jsonb_build_object('limits', COALESCE($4::jsonb,'{}'::jsonb)) WHERE id=$1`,
        [
          payment.tenant_id,
          payment.tier,
          periodEnd,
          JSON.stringify(payment.plan_snapshot?.limits || {}),
        ]
      );
    }
    await client.query(
      `INSERT INTO billing_audit_events (actor_user_id,actor_role,effective_tenant_id,action,resource_type,resource_id,outcome,metadata) VALUES ($1,$2,$3,$4,'manual_payment',$5,'SUCCESS',$6::jsonb)`,
      [
        actor.userId,
        actor.role,
        payment.tenant_id,
        `MANUAL_PAYMENT_${decision}`,
        payment.id,
        JSON.stringify({
    ...
    await client.query(
      `INSERT INTO superadmin_audit_events(actor_user_id,actor_role,effective_tenant_id,action,resource_type,resource_id,outcome,before_state,after_state,metadata) VALUES($1,$2,NULL,$3,'manual_payment',$4,'SUCCESS',$5::jsonb,$6::jsonb,$7::jsonb)`,
      [
        actor.userId,
        actor.role,
        `MANUAL_PAYMENT_${decision}`,
        payment.id,
        JSON.stringify({ status: payment.status, invoiceStatus: payment.invoice_status }),
    ...
    await notify(
      client,
      `manual_${decision.toLowerCase()}:${payment.id}`,
      payment.tenant_id,
      null,
      decision === 'APPROVED' ? 'Pembayaran disetujui' : 'Pembayaran ditolak',
      decision === 'APPROVED'
        ? `Invoice ${payment.invoice_id} telah lunas.`
        : `Pengajuan invoice ${payment.invoice_id} ditolak: ${reason}`,
      payment.id
    );
...
async function reviewManualPayment(req: Request, res: Response, decision: 'APPROVED' | 'REJECTED') {
  ...
  if (!Number.isInteger(expectedVersion))
    return res.status(422).json({ error: 'expectedVersion wajib diisi.' });
  if (decision === 'REJECTED' && !reason)
    return res.status(422).json({ error: 'Alasan penolakan wajib diisi.' });
  const result = await reviewManualPaymentByActor(
    req.params.id,
    expectedVersion,
    decision,
    { userId: req.authActor!.userId, role: req.authActor!.role },
    reason
  );
...
export const approveManualPayment = (req: Request, res: Response) =>
  reviewManualPayment(req, res, 'APPROVED');
export const rejectManualPayment = (req: Request, res: Response) =>
  reviewManualPayment(req, res, 'REJECTED');
