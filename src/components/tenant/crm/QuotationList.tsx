    ...
    <div className="space-y-4">
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          ...
          <button
            onClick={() => onEdit('')}
            className="bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1"
          >
        ...
        <div className="px-5 py-3 border-b border-slate-100 flex gap-3">
          ...
          <select
            ...
            className="px-3 py-1.5 border border-slate-200 rounded-lg text-xs outline-none cursor-pointer"
          >
                ...
                filtered.map((q) => (
                  <tr key={q.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 font-mono font-bold text-emerald-600">
                      Rp {q.total.toLocaleString()}
                    </td>
                    <td className="px-4 py-3 text-slate-500">{q.validUntil}</td>
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 rounded text-[9px] font-bold font-mono ${statusColor(q.status)}`}
                      >
                        {q.status}
                      </span>
                    </td>
