...
export async function authProfileHandler(req: Request, res: Response) {
  try {
    ...
    if (!rows.length) {
      return res
        .status(404)
        .json({ error: 'User profile not found. Contact admin to create a tenant account.' });
    }
    const user = rows[0];
...
export async function authPasswordUpdateHandler(req: Request, res: Response) {
  ...
  if (!currentPassword || !newPassword || String(newPassword).length < 8) {
    return res
      .status(422)
      .json({ error: 'Current password and new password (min 8 chars) are required.' });
  }

  try {
    const account = await withDb(async (c) => {
      const result = await c.query(
        `SELECT u.password_hash,t.settings->'securitySettings' AS policy FROM users u LEFT JOIN tenants t ON t.id=u.tenant_id WHERE u.id = $1 LIMIT 1`,
        [userId]
      );
    ...
    if (
      ...
      !(await bcrypt.compare(String(currentPassword), account.password_hash))
    ) {
...
export async function adminResetPasswordHandler(req: Request, res: Response) {
  ...
  if (!email || !password || String(password).length < 8) {
    return res
      .status(422)
      .json({ success: false, message: 'email and password (min 8 chars) are required.' });
  }
  try {
  ...
  try {
    ...
    await client.query(
      ...
      [
        ...
        JSON.stringify({
          baseCurrency: 'IDR',
          taxSettings: { taxRate: 11, taxEnabled: true, taxInclusive: false },
          authSettings: { requireMfa: false, passwordPolicy: 'medium' },
        }),
        JSON.stringify({
          primaryColor: themeColor || '#4f46e5',
          accentColor: '#6366f1',
          portalHelpTitle: `Pusat Bantuan ${shopName}`,
        }),
      ]
    );
    await client.query(
    ...
    const coaDefaults = [
      ['10100', 'Kas Utama', 'ASSET', 0],
      ['10200', 'Bank Utama', 'ASSET', 0],
      ['10300', 'Piutang Pelanggan', 'ASSET', 0],
      ['10500', 'Persediaan Barang', 'ASSET', 0],
      ['40100', 'Pendapatan Jasa Servis', 'REVENUE', 0],
      ['40200', 'Pendapatan Penjualan Sparepart', 'REVENUE', 0],
      ['50100', 'HPP Sparepart', 'EXPENSE', 0],
      ['60100', 'Beban Gaji Staf', 'EXPENSE', 0],
    ];
...
export async function upgradeTrialHandler(req: Request, res: Response) {
  ...
  const normalizedCycle = billingCycle === 'annually' ? 'yearly' : billingCycle || 'monthly';

  if (!tenantId || !tier) {
    return res.status(400).json({
      success: false,
      message: 'Missing required: tenantId, tier',
    });
  }
  ...
  if (!['monthly', 'yearly'].includes(normalizedCycle)) {
    return res
      .status(400)
      .json({ success: false, message: 'Invalid billing cycle. Must be monthly or yearly.' });
  }
  ...
  try {
    ...
    const amount = normalizedCycle === 'yearly' ? plan.priceYearly : plan.priceMonthly;
    ...
    await client.query(
      ...
      [invoiceId, tenantId, dateStr, dueDateStr, amount, tier, normalizedCycle]
    );
...
export async function extendTrialHandler(req: Request, res: Response) {
  ...
  if (!Number.isInteger(requestedDays) || requestedDays < 1 || requestedDays > maxDays) {
    return res
      .status(422)
      .json({ success: false, message: `days must be an integer between 1 and ${maxDays}.` });
  }
