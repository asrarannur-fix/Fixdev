...
describe('service list query contract', () => {
  ...
  it('keeps an already-registered photo on duplicate upload', () => {
    expect(controller).toContain("if (error.code !== 'EEXIST') await storage.delete(objectPath).catch(() => undefined);");
    expect(controller).toContain('reason: \'duplicate_file\'');
  });
