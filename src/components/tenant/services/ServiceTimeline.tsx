...
export const ServiceTimeline: React.FC<ServiceTimelineProps> = ({ entries = [] }) => (
  <section className="relative overflow-hidden rounded-2xl border border-white/40 p-3.5 shadow-md dark:border-zinc-800/40">
    ...
    <h4 className="relative flex items-center gap-1.5 font-mono text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">
      ...
      {entries.length ? (
        [...entries].reverse().map((entry, index) => (
          <div key={entry.id || `${entry.timestamp}-${index}`} className="relative">
            ...
            <p className="text-xs text-slate-400">Oleh: {entry.operator || 'Sistem'}</p>
          </div>
        ))
      ) : (
        <p className="text-xs text-slate-400 italic">Belum ada catatan perjalanan.</p>
      )}
    </div>
  </section>
);
