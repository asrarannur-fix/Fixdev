...
test("browser dan QZ memakai dokumen print berbasis konfigurasi yang sama", () => {
  ...
  assert.match(source, /data: htmlDoc/);
  assert.match(source, /doc\.write\(createPrintDocument\(title, html, printConfig, qrPayload\)\)/);
  assert.match(source, /if \(printConfig\?\.printMode === 'qz'\)/);
  assert.match(source, /const qzResult = await qzPrint/);
...
test("pratinjau Settings memakai renderer dokumen print yang sama", () => {
  ...
  assert.match(settingsSource, /createPrintDocument,[\s\S]*from '..\/..\/utils\/printJob'/);
