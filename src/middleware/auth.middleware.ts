...
function timingSafeEqual(expected: string, provided: string): boolean {
  ...
  return (
    ...
    cryptoTimingSafeEqual(expectedBuffer, providedBuffer)
  );
}
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        profile.tenant_id !== req.hostTenant.id
      ) {
        return res.status(403).json({ error: 'Access to this tenant is forbidden.' });
      }
      if (req.hostTenant && req.authActor.role === 'SUPER_ADMIN') {
        logger.info(
          { userId: req.authActor.userId, hostTenantId: req.hostTenant.id },
          'Super Admin accessing tenant host'
        );
      }
    } catch (dbErr: any) {
 ...
 */
export const requireTenantScope = async (req: Request, res: Response, next: NextFunction) => {
...
export const requireSettingsDomain =
  (domainParam = 'domain') =>
  (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      ...
      actor.permissions.includes(`settings:${domain}`)
    )
      return next();
    logger.warn(
      { userId: actor.userId, role: actor.role, domain, path: req.path },
      'Settings domain authorization denied'
    );
 ...
 */
export const requireTenantOrSuperAdminPermission =
  (permission: string, allowPlatformScope = false) =>
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!requestedTenant && !allowPlatformScope)
      return res.status(400).json({ error: 'tenantId is required.' });
 ...
 */
export const enforceSuperAdminWriteMode = (req: Request, res: Response, next: NextFunction) => {
  if (
    req.authActor?.role === 'SUPER_ADMIN' &&
...
export const requireSuperAdminPermission =
  (permission: string) => async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.authActor.superadminRole) {
      try {
        ...
        logger.error(
          { err: err.message, role: req.authActor.superadminRole },
          'Could not resolve Super Admin permissions'
        );
        return res.status(503).json({ error: 'Layanan izin Super Admin tidak tersedia.' });
      }
    }
...
export const requireFeature =
  (feature: string) => async (req: Request, res: Response, next: NextFunction) => {
