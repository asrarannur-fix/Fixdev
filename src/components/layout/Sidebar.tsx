...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const tenantBranches = useMemo(() => {
    ...
    if (
      ...
      (currentUser as any).branchIds.length > 0
    ) {
  ...
  const userPermissions = useMemo(() => {
    ...
    if (currentTenant?.rbacMatrix && currentTenant.rbacMatrix[currentUser.role]) {
  ...
  const allowedTenantModules = useMemo(() => {
    return tenantModules.filter(
      (mod) =>
        ...
        userPermissions.some((perm) => perm === mod.id || perm.startsWith(mod.id + '-'))
    );
  }, [tenantModules, userPermissions]);

  // Keep premium modules visible so users understand which package unlocks them.
  const visibleTenantModules = useMemo(() => allowedTenantModules, [allowedTenantModules]);
  ...
  const isSubmenuAllowed = (modId: string, subId: string) => {
    ...
    if (modId === 'settings') {
      ...
      if (subId === 'whatsapp' && !tenantFeatures.includes('WHATSAPP')) return false;
      if (
        subId === 'notifications' &&
        ...
        return false;
      if (subId === 'workflows' && !tenantFeatures.includes('MARKETPLACE')) return false;
      if (subId === 'developer-api' && !tenantFeatures.includes('SECURITY')) return false;
    }
  ...
  const commonMenus = [{ id: 'customer-portal', label: 'Portal Pelanggan', icon: HelpCircle }];
          ...
          <div className="p-5 border-b border-slate-200/40 dark:border-zinc-900 flex items-center justify-between bg-white dark:bg-zinc-900/20 shrink-0">
            <div className="flex items-center gap-3 min-w-0">
              <div className="relative group shrink-0">
                <WhiteLabelGate>
                  <div
                    className="absolute -inset-1 rounded-xl opacity-75 blur-xs group-hover:opacity-100 transition duration-300 animate-pulse"
                    style={{ background: activeTenant?.branding?.primaryColor || 'var(--accent)' }}
                  />
                </WhiteLabelGate>
              ...
              <div className="min-w-0 transition-opacity duration-300 lg:opacity-0 lg:group-hover:opacity-100 lg:group-focus-within:opacity-100">
                <div className="flex items-center gap-1">
                  <h3
                    className="font-syne font-black text-[13px] tracking-tight truncate uppercase leading-tight"
                    style={{ color: activeTenant?.branding?.primaryColor || '#1e293b' }}
                  >
                            ...
                            return (
                              <div key={mod.id} className="relative">
                                ...
                                <button
                                  ...
                                  aria-label={
                                    locked
                                      ? `${mod.label}, memerlukan paket ${requiredTier}`
                                      : `${mod.label}${isModActive ? ', terbuka' : ', buka menu'}`
                                  }
                                ...
                                {isModActive &&
                                  mod.submenus &&
                                  Array.isArray(mod.submenus) &&
                                  mod.submenus.length > 0 && (
                                    <div
                                      id={`sidebar-${mod.id}-submenus`}
                                      className="ml-7 mt-1 space-y-0.5 border-l-2 border-indigo-200 dark:border-indigo-900/40 pl-2.5 transition-opacity duration-300 lg:opacity-0 lg:group-hover:opacity-100 lg:group-focus-within:opacity-100"
                                    >
                                      {mod.submenus
                                        ...
                                        .map((sub) => {
                                          ...
                                          return (
                                            ...
                                            >
                                              <SubIcon
                                                className={`w-3.5 h-3.5 shrink-0 ${isSubActive ? 'text-accent dark:text-accent' : 'text-slate-400'}`}
                                              />
          ...
          <div className="lg:hidden border-t border-slate-200/40 dark:border-zinc-900 p-3 space-y-2.5 bg-white dark:bg-zinc-900/20 shrink-0">
            ...
            <div className="flex items-center gap-3 bg-slate-100/60 dark:bg-slate-900/50 border border-slate-200/60 dark:border-zinc-800 rounded-xl px-3 py-2">
              ...
              <button
                ...
                className="p-2 rounded-lg hover:bg-white dark:hover:bg-slate-800 text-slate-400 cursor-pointer"
              ...
              <button
                onClick={logoutUser}
                className="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-red-500 cursor-pointer"
                title="Keluar"
                aria-label="Keluar"
              >
          ...
          <div className="px-3 pb-3.5 border-t border-slate-200/40 dark:border-zinc-900">
            {commonMenus.map((menu) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-2xl flex items-center justify-center shrink-0 ${isActive ? 'bg-white/10 text-white' : 'bg-slate-100 text-slate-600 dark:bg-zinc-900 dark:text-slate-300'}`}
                  >
                ...
                <div className="bg-indigo-500/10 border border-accent/20 rounded-2xl p-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-accent/80 dark:text-accent/80 mt-0.5 leading-relaxed">
                      ...
                      <strong className="uppercase">{activeTenant?.tier || 'BASIC'}</strong>.
                    </p>
                  </div>
                </div>
              </div>
