...
export const LoginPage: React.FC<LoginPageProps> = ({ onBack }) => {
  ...
  return (
    <main
      className="relative flex min-h-screen flex-col bg-slate-50 text-slate-900"
      id="login-page"
    >
      ...
      <header className="relative z-10 mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
        <button
          onClick={onBack}
          className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-accent"
        >
          <ArrowLeft className="h-4 w-4" /> Kembali
        </button>
        <div className="flex items-center gap-2 font-black">
          <span
            className="grid h-8 w-8 place-items-center overflow-hidden rounded-xl text-white font-bold text-sm shadow-md transition-all duration-200"
            style={{ backgroundColor: activeTenant?.branding?.primaryColor || '#4f46e5' }}
          >
            {activeTenant?.branding?.logoUrl && !logoFailed ? (
              <img
        ...
        <aside className="hidden lg:block">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-accent/10">
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-accent">
              <Building className="h-4 w-4" /> Multi-cabang
            </div>
            <h2 className="mt-3 text-2xl font-black leading-tight tracking-tight">
              Satu akun, seluruh toko terkelola.
            </h2>
            <p className="mt-2 text-xs leading-5 text-slate-600">
              Masuk untuk melihat servis, kasir, stok, dan laporan toko Anda. Data tiap cabang
              terpisah dan aman.
            </p>
            <ul className="mt-5 grid gap-2 text-xs font-bold text-slate-700">
              {[
                ['Cabang terisolasi', 'Data toko tidak tercampur.'],
                ['Akses per peran', 'Owner, Admin, Kasir, Teknisi.'],
                ['Pantauan cepat', 'Ringkasan operasional real-time.'],
              ].map(([t, d]) => (
                <li
                  key={t}
                ...
                >
                  ...
                  <span>
                    ...
                    <span className="block text-[10px] font-semibold text-slate-500">{d}</span>
                  </span>
                </li>
              ))}
            </ul>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-accent/10 sm:p-7">
            <div className="flex p-1 rounded-xl bg-slate-100">
              <button
                ...
                className={`flex-1 rounded-lg py-2.5 text-xs font-black transition ${activeTab === 'manual' ? 'bg-white text-accent shadow-sm' : 'text-slate-500'}`}
              >
              ...
              <button
                ...
                className={`flex-1 rounded-lg py-2.5 text-xs font-black transition ${activeTab === 'register' ? 'bg-white text-accent shadow-sm' : 'text-slate-500'}`}
              >
            ...
            {errorMsg && (
              <div className="mt-3 rounded-xl border border-rose-200 bg-rose-50 p-3 text-[11px] font-medium text-rose-700">
              ...
              {registrationSuccess ? (
                ...
                >
                  ...
                  <div className="space-y-1">
                    <h3 className="text-lg font-black">Toko berhasil dibuat</h3>
                    <p className="text-[11px] text-slate-500">
                      Lingkungan kerja eksklusif sudah siap.
                    </p>
                  </div>
                  <div className="space-y-1 rounded-xl border border-slate-100 bg-slate-50 p-3 text-left text-[11px]">
                    <p className="flex justify-between">
                      ...
                      <span className="font-bold text-slate-800">
                        {registrationSuccess.tenant.name}
                      </span>
                    </p>
                    <p className="flex justify-between">
                      <span className="text-slate-500">Cabang</span>
                      <span className="font-bold text-slate-800">
                        {registrationSuccess.branch.name}
                      </span>
                    </p>
                    <p className="flex justify-between">
                      <span className="text-slate-500">Email</span>
                      <span className="font-bold text-slate-800">
                        {registrationSuccess.ownerEmail}
                      </span>
                    </p>
                  </div>
                  <button
                    ...
                    disabled={loading}
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-accent py-3 text-xs font-black text-white hover:bg-accent-hover disabled:opacity-50"
                  >
                    {loading ? (
                      'Masuk...'
                    ) : (
                ...
                >
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="block">
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold outline-none focus:border-accent focus:ring-2 focus:ring-accent/20"
                      />
                    </label>
                    <label className="block">
                      <span className="mb-1 block text-[10px] font-bold uppercase text-slate-500">
                      ...
                      <select
                        ...
                        className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold outline-none focus:border-accent focus:ring-2 focus:ring-accent/20"
                      >
                        <option>IT & Servis Komputer</option>
                        <option>Servis Smartphone & Gadget</option>
                        <option>Servis Elektronik Umum</option>
                    ...
                    <div className="flex gap-2">
                      {themePresets.map((theme) => (
                        <button
                          ...
                          className={`h-8 w-8 rounded-xl transition ${selectedTheme === theme.color ? 'ring-2 ring-slate-900 ring-offset-2' : ''}`}
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        Buat toko <ArrowRight className="h-4 w-4" />
                      </>
                    )}
                  </button>
                  <p className="text-center text-[10px] font-semibold text-slate-500">
                    Sistem menyiapkan cabang, gudang, dan akun otomatis. Data cabang tetap terpisah.
                  </p>
                </motion.form>
              ) : (
                <motion.form
