...
async function ensureServiceTicketColumns() {
  await withDb(async (client) => {
    ...
    for (const column of columns) {
      await client.query(
        `ALTER TABLE service_tickets ADD COLUMN IF NOT EXISTS ${column.name} ${column.definition}`
      );
    }
  });
}

async function syncProductWarehouseStock(
  tenantId: string,
  productId: string,
  ...
  await withDb(async (client) => {
    for (const [warehouseId, rawQuantity] of entries) {
      ...
      if (!warehouseId || !Number.isFinite(quantity) || quantity < 0) {
        throw Object.assign(new Error('Stok gudang harus berupa angka non-negatif.'), {
          status: 422,
        });
      }
...
export async function moduleRecordsGetHandler(req: Request, res: Response) {
  ...
  try {
    const rows = await withDb((c) =>
      c
        .query(`select * from module_records where tenant_id = $1 and deleted_at is null`, [
          tenantId,
        ])
        .then((r: any) => r.rows)
    );
...
export async function moduleRecordsPostHandler(req: Request, res: Response) {
  ...
  if (!tenantId || !module || !recordId)
    return res.status(422).json({ error: 'tenantId, module, recordId required' });
  try {
...
export async function dataSyncHandler(req: Request, res: Response) {
  ...
  if (!table || !action || !data)
    return res.status(422).json({ error: 'table, action, data required' });
  if (!ALLOWED_TABLES.has(table))
    return res.status(403).json({ error: 'Table not allowed for direct sync' });
  ...
  if (table === 'tenants') {
    return res
      .status(403)
      .json({ error: 'Gunakan endpoint pengaturan khusus untuk memperbarui tenant.' });
  }
  ...
  try {
    ...
    const safePayload =
      table === 'tenants'
        ? {
            id: tenantId,
            ...(sanitizedPayload.settings !== undefined
              ? { settings: sanitizedPayload.settings }
              : {}),
            ...(sanitizedPayload.branding !== undefined
              ? { branding: sanitizedPayload.branding }
              : {}),
          }
        : sanitizedPayload;
    ...
    const payload = table === 'tenants' ? safePayload : { ...safePayload, tenant_id: tenantId };
    ...
    if (action === 'insert') {
      ...
      if (table === 'journal_entries' && data.lines?.length) {
        for (const ln of data.lines.map((line: any) => ({
          id: line.id || `fallback-${Date.now()}-${Date.now().toString(36)}`,
          ...
          await withDb((c) =>
            c.query(
              `INSERT INTO journal_lines (${lCols.join(',')}) SELECT ${lVals.join(',')} WHERE EXISTS (SELECT 1 FROM journal_entries WHERE id = $${lCols.length + 1} AND tenant_id = $${lCols.length + 2}) ON CONFLICT (id) DO NOTHING`,
              [...Object.values(ln), data.id, tenantId]
            )
          );
        }
      }
      if (table === 'products' && data.warehouseStock)
        await syncProductWarehouseStock(tenantId, String(data.id), data.warehouseStock);
      ...
      if (table === 'products' && data.warehouseStock)
        await syncProductWarehouseStock(tenantId, String(idVal), data.warehouseStock);
