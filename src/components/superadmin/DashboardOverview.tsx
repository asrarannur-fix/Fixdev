...
export const DashboardOverview: React.FC<DashboardOverviewProps> = ({
  ...
  const metrics = overview?.metrics || {
    ...
    trialExpiring: tenants.filter((tenant) => tenant.status === "TRIAL" && tenant.trialEndsAt && new Date(tenant.trialEndsAt).getTime() <= Date.now() + 7 * 86400000).length,
    unreadNotifications: 0,
  };

  const trialRows = useMemo(() => tenants
    ...
    .sort((a, b) => a.days - b.days).slice(0, 5), [tenants]);
      ...
      <section aria-labelledby="kpi-heading">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {cards.map(({ label, value, meta, icon: Icon, tab, filter }) => (
            <button key={label} type="button" onClick={() => onNavigate?.(tab, filter)} className="group rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:border-accent/50 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-indigo-700">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900" aria-labelledby="attention-heading">
        ...
        <div className="mt-4 space-y-2">
          {overview?.actions.length ? overview.actions.map((action) => (
            <button key={action.id} type="button" onClick={() => onNavigate?.(action.targetTab, action.targetFilter)} className="flex w-full items-center justify-between rounded-xl border border-slate-200 px-4 py-3 text-left hover:bg-slate-50 dark:border-zinc-800 dark:hover:bg-zinc-950">
