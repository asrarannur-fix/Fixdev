...
export default function SaaSSubscription({
  ...
  section?: 'all' | 'billing' | 'gateway';
}) {
  const { currentUser, currentTenantId, tenants, updateTenant, apiFetch } = useSaaS();
  ...
  const estimatedMrr = tenants.reduce(
    (sum, tenant) => sum + (planPrices[tenant.tier || SubscriptionTier.BASIC] || 0),
    0
  );
  ...
  const [selectedConfigTier, setSelectedConfigTier] = useState<SubscriptionTier>(
  ...
  const [manualMethod, setManualMethod] = useState<'BANK_TRANSFER' | 'MANUAL_QRIS'>(
    'BANK_TRANSFER'
  );
  ...
  const filteredInvoices = invoices.filter((item) => {
    ...
    if (
      query &&
      ![item.id, item.tier, item.status].some((value) =>
        String(value || '')
          .toLowerCase()
          .includes(query)
      )
    ...
    if (invoiceDateFrom && invoiceTime < new Date(`${invoiceDateFrom}T00:00:00`).getTime())
      return false;
    if (invoiceDateTo && invoiceTime > new Date(`${invoiceDateTo}T23:59:59`).getTime())
      return false;
  ...
  const handleSubmitManualPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const uploadResponse = await apiFetch(
        `/api/billing/invoices/${manualInvoice.id}/manual-payments/upload-url`,
        {
          method: 'POST',
      ...
      const submitResponse = await apiFetch(
        `/api/billing/invoices/${manualInvoice.id}/manual-payments`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-Tenant-ID': selectedTenantId },
          body: JSON.stringify({
  ...
  const handleSavePlanConfig = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const safePosTransactions = Math.max(
        0,
        Math.floor(Number(configPlanMaxPosTransactions) || 0)
      );
      ...
      if (res.ok) {
        setPlanSuccess('Konfigurasi Paket Langganan berhasil diperbarui secara permanen!');
  ...
  const handleSaveManualConfig = async (e: React.FormEvent) => {
    e.preventDefault();
    if (readOnlyMode)
      return showToast('Aktifkan Edit Mode untuk mengubah pembayaran manual.', 'error');
    ...
    try {
      ...
      if (manualQrisFile) {
        if (
          !['image/jpeg', 'image/png'].includes(manualQrisFile.type) ||
          manualQrisFile.size > 2 * 1024 * 1024
        )
          throw new Error('QRIS harus JPG atau PNG maksimal 2 MB.');
        const upload = await readJsonResponse<any>(
          await apiFetch('/api/billing/manual-payment-config/qris-upload', {
            method: 'POST',
            body: JSON.stringify({
              fileName: manualQrisFile.name,
              contentType: manualQrisFile.type,
              sizeBytes: manualQrisFile.size,
            }),
          }),
          'Upload QRIS'
        );
        const storageResponse = await apiFetch(upload.signedUploadUrl, {
          method: 'PUT',
          headers: { 'Content-Type': manualQrisFile.type },
          body: manualQrisFile,
        });
      ...
      const saved = await readJsonResponse<any>(
        await apiFetch('/api/billing/manual-payment-config', {
          method: 'PUT',
          body: JSON.stringify({
            bankTransferEnabled: manualConfig.bankTransferEnabled,
            manualQrisEnabled: manualConfig.manualQrisEnabled,
            bankName: manualConfig.bankName,
            accountNumber: manualConfig.accountNumber,
            accountHolder: manualConfig.accountHolder,
      ...
      const refreshed = await readJsonResponse<any>(
        await apiFetch('/api/billing/manual-payment-config'),
        'Pembayaran manual'
      );
      ...
      showToast(err.message || 'Konfigurasi pembayaran manual gagal disimpan.', 'error');
    } finally {
      setManualConfigSaving(false);
    }
  };
  ...
  const getTierIcon = (tier: SubscriptionTier) => {
    ...
    if (tier === SubscriptionTier.PRO) return <CreditCard className="w-5 h-5 text-teal-500" />;
              ...
              <div className="space-y-2.5">
                {[1, 2, 3].map((j) => (
                  <div key={j} className="flex items-center gap-2.5">
                    ...
                    <div
                      className="h-3 bg-slate-200 dark:bg-zinc-800 rounded flex-1"
                      style={{ width: `${90 - j * 10}%` }}
                    />
                  </div>
                ))}
  ...
  return (
    <div
      className={`${isSuperAdmin ? `billing-view-${simpleView}` : 'billing-view-owner'} space-y-6 max-w-7xl mx-auto animate-fadeIn`}
    ...
    >
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div className="bg-[radial-gradient(circle_at_top_right,_rgba(99,102,241,0.24),_transparent_42%),linear-gradient(135deg,#020617,#172554_55%,#312e81)] px-5 py-6 text-white sm:px-8 sm:py-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              {isSuperAdmin && (
                <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-indigo-100">
                  <ShieldCheck className="h-3.5 w-3.5" /> Billing Control Plane
                </span>
              )}
              <h2 className="mt-4 text-2xl font-black tracking-tight sm:text-3xl">
                {isSuperAdmin
                  ? 'Pendapatan langganan dalam satu ruang kerja'
            ...
            <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
              {isSuperAdmin &&
                [
                  ...
                  { label: 'Invoice', value: invoices.length },
                  {
                    label: 'Perlu review',
                    value: manualRequests.filter((item) => item.status === 'SUBMITTED').length,
                  },
                  { label: 'Overdue', value: overdueInvoices },
                ].map((item) => (
                  <div
                    key={item.label}
                    className="min-w-24 rounded-2xl border border-white/10 bg-white/10 p-3 backdrop-blur"
                  >
        ...
        {isSuperAdmin && (
          <nav
            className="grid grid-cols-3 gap-2 border-t border-white/5 bg-slate-950 p-3"
          ...
          >
            {(
              ...
              <button
                key={view}
                type="button"
                onClick={() => setSimpleView(view)}
                aria-pressed={simpleView === view}
                className={`rounded-xl px-3 py-2.5 text-xs font-bold transition ${simpleView === view ? 'bg-white text-slate-950 shadow' : 'border border-slate-700 text-slate-300 hover:border-indigo-400 hover:text-white'}`}
              >
                {label}
              </button>
            ))}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed">
                {billingError ||
                  'Paket fallback lokal sedang digunakan karena API plan tidak tersedia.'}
              </p>
            </div>
            <button
              type="button"
      ...
      {isSuperAdmin && (
        <div
          id="billing-context"
          className="scroll-mt-24 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4"
        >
          <div>
            <h3 className="font-extrabold text-sm text-slate-800 dark:text-zinc-200 uppercase tracking-wider flex items-center gap-2">
              <Building className="w-5 h-5 text-accent" /> Pilih Tenant untuk Dikelola (SaaS
              Billing)
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Sebagai Super Admin, Anda dapat memantau billing, mengubah paket, membuat tagihan QRIS
              manual, atau melihat histori invoice tenant mana pun.
            </p>
          </div>
          <select
            ...
            {tenants.length === 0 ? (
              ...
              tenants.map((t) => (
                <option key={t.id} value={t.id}>
                  {t.name || 'Tenant tanpa nama'} (Tenant ID: {t.subdomain || t.id}) — Paket:{' '}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4" id="sa-billing-reconciliation">
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm">
            ...
            <p className="text-xl font-black text-slate-900 dark:text-white mt-1">
              Rp {estimatedMrr.toLocaleString()}
            </p>
          </div>
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm">
            <p className="text-[10px] uppercase font-black text-slate-400 tracking-wider">
          ...
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm">
            ...
            <p className="text-[11px] font-bold text-slate-700 dark:text-zinc-300 mt-2">
              BASIC {planDistribution[SubscriptionTier.BASIC] || 0} · PRO{' '}
              {planDistribution[SubscriptionTier.PRO] || 0} · ENT{' '}
              {planDistribution[SubscriptionTier.ENTERPRISE] || 0}
            </p>
          </div>
        </div>
      )}
      ...
      <div
        id="billing-tenant-summary"
        className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-indigo-900/50 rounded-xl md:rounded-2xl p-4 md:p-6 text-white shadow-lg relative overflow-hidden"
      >
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="bg-indigo-500/20 text-indigo-300 text-[9px] md:text-[10px] font-extrabold uppercase px-2 md:px-2.5 py-1 rounded-full border border-accent/20 tracking-wider flex items-center gap-1 md:gap-1.5">
                <ShieldCheck className="w-3 h-3 md:w-3.5 md:h-3.5" /> SECURE INTEGRATED QRIS BILLING
              </span>
            </div>
            <h2 className="text-lg md:text-xl font-extrabold tracking-tight">
              {isSuperAdmin ? 'Status Paket Langganan Tenant' : 'Status Langganan Anda'}
            </h2>
            <p className="text-[10px] md:text-xs text-slate-400 mt-1 max-w-2xl">
              {isSuperAdmin
                ? 'Kelola plan langganan ERP, batasan kapasitas (users/cabang), histori struk invoice, dan sistem auto-debit QRIS.'
                : 'Lihat paket aktif, batas kapasitas, dan status langganan toko Anda.'}
            </p>
          </div>
              ...
              <p className="text-[9px] md:text-[10px] text-slate-400 mt-0.5">
                ...
                <span className="font-mono text-white font-bold">
                  {activeTenant?.trialEndsAt
                    ? new Date(activeTenant.trialEndsAt).toLocaleDateString('id-ID', {
                        day: 'numeric',
                        month: 'long',
                        year: 'numeric',
                      })
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mt-4 md:mt-6 pt-4 md:pt-6 border-t border-white/10 text-xs">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-center mb-1 text-[11px] md:text-xs">
                <span className="text-slate-400 font-bold truncate">Kapasitas Pengguna</span>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-center mb-1 text-[11px] md:text-xs">
                <span className="text-slate-400 font-bold truncate">Cabang / Warehouses</span>
                <span className="text-white font-mono font-bold text-[10px] md:text-[11px] ml-2">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-center mb-1 text-[11px] md:text-xs">
                <span className="text-slate-400 font-bold truncate">Cloud Storage Owner</span>
                <span className="text-white font-mono font-bold text-[10px] md:text-[11px] ml-2">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Sesuaikan paket yang tepat untuk menunjang skala operasional reparasi gadget Anda.
            </p>
          </div>
                ...
                >
                  ...
                  {p.tier === 'PRO' && (
                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-teal-600 text-white text-[9px] font-extrabold uppercase px-3 py-1 rounded-full tracking-wider shadow-sm flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3 text-green-500" /> Pilihan Terpopuler
                    </span>
                  )}
                    ...
                    <div className="my-4 md:my-5">
                      ...
                      {billingCycle === 'yearly' && (
                        <p className="text-[11px] md:text-xs text-emerald-600 dark:text-emerald-400 font-bold mt-1">
                          Hemat {formatRupiah(p.priceMonthly * 12 - p.priceYearly)} dibandingkan
                          bayar bulanan
                        </p>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4 bg-slate-50 dark:bg-zinc-950 p-2.5 md:p-3 rounded-xl border border-slate-100 dark:border-zinc-800 text-[11px] md:text-xs font-medium text-slate-700 dark:text-slate-300">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          Tiket Servis:{' '}
                          <strong className="text-slate-900 dark:text-white">
                            Maks {p.limits.maxServiceTickets}
                          </strong>
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          Transaksi POS:{' '}
                          <strong className="text-slate-900 dark:text-white">
                            Maks {p.limits.maxPosTransactions}
                          </strong>
                        </span>
                      </div>
                  ...
                  <button
                    ...
                    disabled={
                      readOnlyMode ||
                      !activeTenant ||
                      (isCurrent && activeTenant?.status === 'ACTIVE' && billingCycle === 'monthly')
                    }
                  ...
                  >
                    {activeTenant?.status === 'TRIAL'
                      ...
                      : isCurrent && activeTenant?.status === 'ACTIVE'
                        ? 'Paket Sedang Aktif'
                        : `Upgrade ke ${p.tier}`}{' '}
                    {activeTenant?.status !== 'TRIAL' && !isCurrent && (
      ...
      {isSuperAdmin && (
        <div
          ...
          className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm grid grid-cols-1 lg:grid-cols-3 gap-6"
        >
          <div className="lg:col-span-1 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mt-1">
                Untuk mengamankan operasional tenant tanpa interupsi, modul pemrosesan aman kami
                menjalankan pemindaian otomatis (cron-job) secara berkala.
              </p>
            </div>
            <div className="bg-slate-50 dark:bg-zinc-950 border border-slate-100 dark:border-zinc-800 rounded-xl p-3 md:p-4 text-xs md:text-sm text-slate-700 dark:text-slate-300 space-y-2 leading-relaxed">
              <p className="font-bold text-slate-900 dark:text-zinc-100 flex items-center gap-1.5 text-sm">
                <ShieldCheck className="w-4 h-4 text-accent" /> Kriteria Auto-Renewal:
              </p>
            ...
            {isSuperAdmin && (
              ...
              >
                {cronLoading ? (
                  ...
                  <>
                    <Terminal className="w-3.5 h-3.5 md:w-4 md:h-4 text-indigo-400" /> Jalankan Cron
                    Job
                  </>
                )}
              </button>
          ...
          <div className="lg:col-span-2 bg-slate-950 rounded-2xl p-4 border border-slate-800 flex flex-col justify-between min-h-[220px]">
            <div className="flex justify-between items-center pb-2 border-b border-slate-900 text-[10px] font-mono text-slate-500">
              <span className="flex items-center gap-1.5">
                <span className="w-2 h-2 bg-emerald-500 rounded-full animate-ping" /> SECURE CRON
            ...
            <div className="flex-1 font-mono text-[10px] text-indigo-300 space-y-1.5 py-3 overflow-y-auto max-h-[160px] custom-scrollbar leading-relaxed">
              {cronLogs.length === 0 ? (
                <p className="text-slate-600 italic">
                  Klik tombol "Jalankan Cron Job" untuk memantau sistem pemrosesan background
                  otomatis yang memeriksa tagihan & melakukan auto-renewal...
                </p>
              ) : (
      ...
      {isSuperAdmin && (
        ...
        >
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    Modifikasi tarif harga, batasan kuota cabang, limit staff, dan modul yang
                    diaktifkan secara global per tier paket langganan.
                  </p>
                </div>
              </div>
            </div>
          ...
          <form onSubmit={handleSavePlanConfig} className="space-y-6">
            <fieldset
              disabled={readOnlyMode || usingFallbackPlans}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConfigPlanPriceMonthly(Number(e.target.value))}
                    className="w-full text-xs px-3.5 py-2.5 border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950 rounded-xl font-medium text-slate-800 dark:text-zinc-200 outline-none focus:border-accent transition-colors"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConfigPlanPriceYearly(Number(e.target.value))}
                    className="w-full text-xs px-3.5 py-2.5 border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950 rounded-xl font-medium text-slate-800 dark:text-zinc-200 outline-none focus:border-accent transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50/50 dark:bg-zinc-950/50 p-4 rounded-2xl border border-slate-100 dark:border-zinc-800">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConfigPlanUserLimit(Number(e.target.value))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConfigPlanBranchLimit(Number(e.target.value))}
                    className="w-full text-xs px-3.5 py-2.5 border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-xl font-bold text-slate-800 dark:text-zinc-200 outline-none focus:border-accent transition-colors"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConfigPlanStorageLimitMb(Number(e.target.value))}
                    className="w-full text-xs px-3.5 py-2.5 border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-xl font-bold text-slate-800 dark:text-zinc-200 outline-none focus:border-accent transition-colors"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConfigPlanMaxServiceTickets(Number(e.target.value))}
                    className="w-full text-xs px-3.5 py-2.5 border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-xl font-bold text-slate-800 dark:text-zinc-200 outline-none focus:border-accent transition-colors"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConfigPlanMaxPosTransactions(Number(e.target.value))}
                    className="w-full text-xs px-3.5 py-2.5 border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-xl font-bold text-slate-800 dark:text-zinc-200 outline-none focus:border-accent transition-colors"
                  />
                </div>
              </div>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setConfigPlanFeatures([...configPlanFeatures, feature.id]);
                              } else {
                                setConfigPlanFeatures(
                                  configPlanFeatures.filter((f) => f !== feature.id)
                                );
                              }
                            }}
                            className="rounded text-accent focus:ring-accent border-slate-300 dark:border-zinc-700 w-3.5 h-3.5 cursor-pointer"
                          />
                          <span className="text-xs font-bold leading-none">{feature.label}</span>
                        </div>
              ...
              <div>
                ...
                <span className="text-[9px] text-slate-400 dark:text-zinc-500 mt-1 block">
                  Tuliskan fasilitas pemasaran yang akan ditampilkan sebagai daftar bullet point
                  pada kartu paket di atas (Gunakan Enter untuk baris baru).
                </span>
              </div>
      ...
      {isSuperAdmin && (
        <form
          ...
          id="billing-config"
        >
          <fieldset disabled={readOnlyMode} className="contents disabled:opacity-60">
            <div className="border-b border-slate-100 pb-4 dark:border-zinc-800">
              ...
              <p className="mt-1 text-xs text-slate-500">
                Atur rekening tujuan, QRIS statis merchant, dan instruksi yang dilihat tenant.
              </p>
            </div>
            <div className="mt-5 grid gap-5 lg:grid-cols-2">
              <section className="space-y-3 rounded-2xl border border-slate-200 p-4 dark:border-zinc-800">
                <label className="flex items-center justify-between text-xs font-bold">
                  <span>Aktifkan transfer bank</span>
                  <input
                    type="checkbox"
                    checked={manualConfig.bankTransferEnabled}
                    onChange={(e) =>
                      setManualConfig({ ...manualConfig, bankTransferEnabled: e.target.checked })
                    }
                  />
                </label>
                <label className="block text-xs font-bold">
                  Nama bank
                  <input
                    value={manualConfig.bankName}
                    onChange={(e) => setManualConfig({ ...manualConfig, bankName: e.target.value })}
                ...
                <label className="block text-xs font-bold">
                  Nomor rekening
                  <input
                    value={manualConfig.accountNumber}
                    onChange={(e) =>
                      setManualConfig({ ...manualConfig, accountNumber: e.target.value })
                    }
                    className="mt-1 w-full rounded-xl border border-slate-200 p-2.5 dark:border-zinc-700 dark:bg-zinc-950"
                  />
                </label>
                <label className="block text-xs font-bold">
                  ...
                  <input
                    value={manualConfig.accountHolder}
                    onChange={(e) =>
                      setManualConfig({ ...manualConfig, accountHolder: e.target.value })
                    }
                    className="mt-1 w-full rounded-xl border border-slate-200 p-2.5 dark:border-zinc-700 dark:bg-zinc-950"
                  />
                </label>
              ...
              <section className="space-y-3 rounded-2xl border border-slate-200 p-4 dark:border-zinc-800">
                <label className="flex items-center justify-between text-xs font-bold">
                  <span>Aktifkan QRIS statis</span>
                  <input
                    type="checkbox"
                    checked={manualConfig.manualQrisEnabled}
                    onChange={(e) =>
                      setManualConfig({ ...manualConfig, manualQrisEnabled: e.target.checked })
                    }
                  />
                </label>
                {manualConfig.qrisImageUrl && (
                  <img
                    ...
                    alt="QRIS statis merchant"
                    className="mx-auto h-44 w-44 rounded-xl border bg-white object-contain p-2"
                  />
                )}
                <label
                  htmlFor="manual-qris-upload"
                  className="inline-flex min-h-11 w-full cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-indigo-300 bg-indigo-50 px-4 py-3 text-xs font-black text-indigo-700 transition hover:border-indigo-500 hover:bg-indigo-100 dark:border-indigo-800 dark:bg-indigo-950/30 dark:text-indigo-300"
                >
                  <QrCode className="h-4 w-4" /> Pilih Gambar QRIS
                </label>
                <input
                  id="manual-qris-upload"
                  type="file"
                  accept="image/jpeg,image/png"
                  onChange={(e) => setManualQrisFile(e.target.files?.[0] || null)}
                  className="sr-only"
                />
                <p className="text-center text-[11px] text-slate-500">
                  {manualQrisFile?.name ||
                    manualConfig.qrisOriginalName ||
            ...
            <label className="mt-4 block text-xs font-bold">
              ...
              <textarea
                rows={3}
                value={manualConfig.instructions}
                onChange={(e) => setManualConfig({ ...manualConfig, instructions: e.target.value })}
                placeholder="Contoh: Cantumkan nomor invoice pada berita transfer."
                className="mt-1 w-full rounded-xl border border-slate-200 p-3 dark:border-zinc-700 dark:bg-zinc-950"
              />
            </label>
            <button
              type="submit"
              disabled={readOnlyMode || manualConfigSaving}
              className="mt-4 rounded-xl bg-accent px-5 py-2.5 text-xs font-bold text-white disabled:opacity-40"
            >
              {manualConfigSaving ? 'Menyimpan…' : 'Simpan Pembayaran Manual'}
            </button>
          </fieldset>
        </form>
      )}
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    Hubungkan sistem SaaS ERP ke Midtrans untuk memproses transaksi QRIS riil secara
                    otomatis (Option B).
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-3 bg-slate-50 dark:bg-zinc-950 border border-slate-100 dark:border-zinc-800 rounded-2xl p-4 flex justify-between items-center">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                      Jika diaktifkan, pembuatan tagihan baru akan langsung menembak API Midtrans
                      asli untuk menerbitkan QRIS resmi.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <option value="production">Production (Real / Live Payment)</option>
                  </select>
                </div>
                ...
                <div className="md:col-span-3 space-y-2">
                  ...
                  <p className="text-[10px] text-slate-400 leading-relaxed">
                    * Server Key dienkripsi dan disimpan dengan aman secara server-side. Jangan
                    pernah membagikan kunci ini ke siapa pun. Kosongkan jika tidak ingin mengubah
                    Server Key lama.
                  </p>
                </div>
              </div>
          ...
          <div className="bg-slate-50 dark:bg-zinc-950 rounded-2xl p-4 border border-slate-100 dark:border-zinc-800 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
              <div className="space-y-1.5">
                ...
                <p>
                  Ketika penyewa memindai QRIS dan membayar, dana akan langsung masuk ke{' '}
                  <strong>virtual wallet/rekening penampung Midtrans milik Super Admin</strong>.
              ...
              <div className="space-y-1.5">
                ...
                <p>
                  Super Admin dapat mencairkan saldo tersebut ke rekening bank asli terdaftar (BRI,
                  Mandiri, BCA, dll) kapan pun secara real-time via dashboard Midtrans, atau
                  mengatur jadwal otomatis (auto-disbursement) setiap hari kerja.
                </p>
              </div>
            </div>
          </div>
      ...
      >
        <div className="px-5 py-4 border-b border-slate-100 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Daftar transaksi, rincian pembayaran, dan status tagihan perpanjangan paket ERP.
            </p>
          </div>
        ...
        <div className="border-b border-slate-100 bg-white px-5 py-3 dark:border-zinc-800 dark:bg-zinc-900">
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              placeholder="Cari ID invoice, paket, status..."
              className="min-w-56 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-800 outline-none focus:border-accent/60 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-200"
            />
            <select
            ...
            <select
              aria-label="Filter siklus invoice"
              value={invoiceCycleFilter}
              onChange={(event) => setInvoiceCycleFilter(event.target.value)}
              className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-950"
            >
            ...
            <input
              ...
              onChange={(event) => setInvoiceMinAmount(event.target.value)}
              placeholder="Min nominal"
              className="w-32 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-950"
            />
            <input
              ...
              onChange={(event) => setInvoiceMaxAmount(event.target.value)}
              placeholder="Maks nominal"
              className="w-32 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-950"
            />
            <button
              type="button"
              onClick={() => {
                ...
                setInvoiceCycleFilter('');
                setInvoiceDateFrom('');
                setInvoiceDateTo('');
                setInvoiceMinAmount('');
                setInvoiceMaxAmount('');
              }}
              className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600 dark:border-zinc-700 dark:text-zinc-300"
            >
              Reset filter
            </button>
          </div>
        ...
        {filteredInvoices.length === 0 ? (
          <div className="text-center py-12 text-slate-400 dark:text-slate-500">
            ...
            <p className="text-xs font-bold">Belum ada riwayat tagihan terdaftar.</p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <span className="bg-slate-100 dark:bg-zinc-800 text-slate-800 dark:text-zinc-200 px-2 py-0.5 rounded font-bold font-mono text-[9px]">
                        {inv.tier} ({inv.billingCycle === 'yearly' ? 'Thn' : 'Bln'})
                      </span>
                    </td>
        ...
        createPortal(
          <div className="fixed inset-0 z-[10000] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
            <form
              ...
              className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-5 md:p-6 shadow-2xl space-y-4"
            >
              ...
              <div
                ...
                role="radiogroup"
                aria-label="Metode pembayaran manual"
              >
                {[
                  ...(manualConfig.bankTransferEnabled ? ['BANK_TRANSFER' as const] : []),
                  ...(manualConfig.manualQrisEnabled ? ['MANUAL_QRIS' as const] : []),
                ].map((method) => (
                  <button
                    key={method}
                    type="button"
                    onClick={() => setManualMethod(method)}
                    className={`rounded-xl border p-3 text-xs font-bold ${manualMethod === method ? 'border-accent bg-accent-lighter text-accent dark:bg-indigo-950/30 dark:text-indigo-300' : 'border-slate-200 dark:border-zinc-700'}`}
                  >
              ...
              {manualConfig.bankTransferEnabled || manualConfig.manualQrisEnabled ? (
                <>
                    ...
                    {manualMethod === 'BANK_TRANSFER' ? (
                      <div>
                        <p>
                          Transfer ke{' '}
                          <b>
                            {manualConfig.bankName} · {manualConfig.accountNumber}
                          </b>
                        </p>
                        <p>
                          a.n. <b>{manualConfig.accountHolder}</b>
                        </p>
                      </div>
                    ) : (
                      <div className="text-center">
                        {manualConfig.qrisImageUrl ? (
                          <img
                            src={manualConfig.qrisImageUrl}
                            alt="QRIS pembayaran"
                            className="mx-auto mb-2 h-56 w-56 rounded-xl bg-white object-contain p-2"
                          />
                        ) : (
                          <p>Gambar QRIS belum tersedia.</p>
                        )}
                      </div>
                    )}
                    {manualConfig.instructions && (
                      <p className="mt-2 whitespace-pre-wrap border-t border-slate-200 pt-2 dark:border-zinc-800">
                        {manualConfig.instructions}
                      </p>
                    )}
                  </div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-zinc-300">
                    ...
                    <input
                      ...
                      required
                      className="mt-1 w-full rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-950 p-2.5"
                    />
                  </label>
                  <label className="block text-xs font-bold text-slate-700 dark:text-zinc-300">
                ...
                <p className="rounded-xl bg-amber-50 p-4 text-xs text-amber-800">
                  Super Admin belum mengaktifkan metode pembayaran manual.
                </p>
              )}
            </form>
          </div>,
      ...
      {isSuperAdmin && (
        <section
          id="billing-review"
          className="scroll-mt-24 rounded-2xl border border-amber-200 dark:border-amber-900/50 bg-amber-50/60 dark:bg-amber-950/10 p-4 md:p-5"
        >
            ...
            {manualRequests.filter((request) => request.status === 'SUBMITTED').length === 0 ? (
              <div className="rounded-xl bg-white/40 dark:bg-zinc-900/40 p-8 text-center border border-dashed border-amber-200 dark:border-amber-900/30">
                ...
                <p className="text-xs font-bold text-slate-500">
                  Antrean kosong. Semua pembayaran manual sudah diproses.
                </p>
              </div>
            ) : (
              manualRequests
                ...
                .map((request) => (
                  <div
                    key={request.id}
                    className="flex flex-col md:flex-row md:items-center justify-between gap-3 rounded-xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-3"
                  >
                    <div className="text-xs">
                      <p className="font-black">
                        {request.tenant_name} · {request.invoice_id}
                      </p>
                      <p className="text-slate-500 mt-1">
                        {request.method === 'BANK_TRANSFER' ? 'Transfer Bank' : 'QRIS Manual'} ·{' '}
                        {formatRupiah(Number(request.amount))} · Ref {request.reference_number}
                      </p>
                      <p
                        className={`mt-1 text-[10px] font-bold ${Date.now() - new Date(request.submitted_at).getTime() > 24 * 60 * 60 * 1000 ? 'text-rose-600' : 'text-amber-600'}`}
                      >
                        Menunggu{' '}
                        {Math.max(
                          0,
                          Math.floor(
                            (Date.now() - new Date(request.submitted_at).getTime()) / 3600000
                          )
                        )}{' '}
                        jam · SLA 24 jam
                      </p>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={async () => {
                          try {
                            const result = await readJsonResponse<any>(
                              await apiFetch(
                                `/api/billing/manual-payments/${request.id}/proof-url`
                              ),
                              'Bukti pembayaran'
                            );
                            const proofResponse = await apiFetch(result.fileUrl);
                            if (!proofResponse.ok)
                              throw new Error('Bukti pembayaran gagal dimuat.');
                            const proofUrl = URL.createObjectURL(await proofResponse.blob());
                      ...
                      <button
                        onClick={() => handleReviewManual(request, 'approve')}
                        className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white shadow-sm"
                      >
                        Setujui
                      </button>
                      <button
                        onClick={() => handleReviewManual(request, 'reject')}
                        className="rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-bold text-white shadow-sm"
                      >
                        Tolak
                      </button>
                    </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl md:rounded-3xl w-full max-w-md p-4 md:p-6 text-slate-100 shadow-2xl relative animate-fadeIn max-h-[90vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="space-y-2 text-[10px] md:text-[11px] text-slate-400 leading-relaxed mb-4 md:mb-6">
                ...
                <ol className="list-decimal list-inside space-y-1 pl-1">
                  ...
                  <li>Konfirmasikan nominal transfer dan lakukan pembayaran aman.</li>
                </ol>
              </div>
              ...
              <div className="space-y-2 md:space-y-2.5">
                <div className="rounded-xl border border-accent/30 bg-indigo-500/10 p-3 text-xs text-indigo-100">
                  Pembayaran diterima oleh Midtrans dan akan berubah menjadi LUNAS hanya setelah
                  webhook terverifikasi. Tidak ada tombol konfirmasi manual pada alur ini.
                </div>
                ...
                <div className="border-t border-b border-slate-100 dark:border-zinc-800 py-3 space-y-2 md:space-y-2.5">
                  ...
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <span className="font-bold text-accent dark:text-accent text-right">
                      ...
                      {detailModalInvoice.billingCycle === 'yearly' ? 'Per Tahun' : 'Per Bulan'})
                    </span>
                  </div>
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <span className="font-mono text-slate-900 dark:text-zinc-100">
                      {(detailModalInvoice.autoRenew ?? true) ? 'AKTIF' : 'NONAKTIF'}
                    </span>
                  </div>
                ...
                >
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      {
                        label: detailModalInvoice.qrisData
                          ? 'QRIS gateway diterbitkan'
                          : 'Metode pembayaran manual dipilih',
                        date: detailModalInvoice.date,
                        done: true,
                      },
                      {
                        label:
                          detailModalInvoice.status === 'PENDING_VERIFICATION'
                            ? 'Bukti menunggu verifikasi'
                            : 'Pembayaran diproses',
                        date:
                          detailModalInvoice.status === 'PENDING_VERIFICATION' ? 'Saat ini' : '-',
                        done: ['PENDING_VERIFICATION', 'PAID'].includes(detailModalInvoice.status),
                      },
                      {
                        label: 'Settlement dikonfirmasi',
                        date:
                          detailModalInvoice.status === 'PAID' ? 'Terverifikasi' : 'Belum terjadi',
                        done: detailModalInvoice.status === 'PAID',
                      },
                    ].map((event, index) => (
                      <div key={event.label} className="flex gap-3">
                        <span
                          className={`mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full text-[9px] font-black ${event.done ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-500 dark:bg-zinc-700'}`}
                        >
                          {index + 1}
                        </span>
                        <span>
                          <b className="block text-xs text-slate-800 dark:text-zinc-200">
                            {event.label}
                          </b>
                  ...
                  <div className="flex-1">
                    <p className="font-bold">
                      {detailModalInvoice.status === 'PAID'
                        ...
                        : `STATUS: ${detailModalInvoice.status}`}
                    </p>
                    <p className="text-slate-500 dark:text-slate-400 mt-0.5">
                      Transaksi ID: {detailModalInvoice.id.replace('saas-inv-', 'TX-')}
                    </p>
                  </div>
                </div>
              </div>
