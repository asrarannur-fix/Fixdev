...
export const AppSettingsPanel: React.FC<Props> = ({ currentTenantId, tenantObj, updateTenant }) => {
  ...
  const [maintenanceMode, setMaintenanceMode] = useState(
    s.generalSettings?.maintenanceMode ?? false
  );
  const [numberFormat, setNumberFormat] = useState(s.generalSettings?.numberFormat || 'ID');

  // Customer Portal
  const [enableStatusCheck, setEnableStatusCheck] = useState(
    s.portalSettings?.enableStatusCheck ?? true
  );
  const [enableEstApprove, setEnableEstApprove] = useState(
    s.portalSettings?.enableEstimateApproval ?? true
  );
  const [enableInvoiceView, setEnableInvoiceView] = useState(
    s.portalSettings?.enableInvoiceView ?? true
  );
  const [enableWarrantyView, setEnableWarrantyView] = useState(
    s.portalSettings?.enableWarrantyView ?? true
  );
  const [enableTicketTracking, setEnableTicketTracking] = useState(
    s.portalSettings?.enableTicketTracking ?? true
  );
  const [hideInternalNotes, setHideInternalNotes] = useState(
    s.portalSettings?.hideInternalNotes ?? true
  );
  ...
  const [enableRealtime, setEnableRealtime] = useState(
    s.emailSettings?.enableRealtimeNotifications ?? true
  );

  // File Upload
  const [maxUploadMb, setMaxUploadMb] = useState(s.uploadSettings?.maxUploadSizeMb ?? 10);
  const [allowedTypes, setAllowedTypes] = useState(
    s.uploadSettings?.allowedFileTypes || '.jpg,.png,.pdf,.doc,.xlsx'
  );
  ...
  const [fileVisibility, setFileVisibility] = useState(
    s.uploadSettings?.fileVisibility || 'private'
  );
  const [folderInvoices, setFolderInvoices] = useState(s.uploadSettings?.folderInvoices || '');
  const [folderServicePhotos, setFolderServicePhotos] = useState(
    s.uploadSettings?.folderServicePhotos || ''
  );
  const [folderAttendance, setFolderAttendance] = useState(
    s.uploadSettings?.folderAttendance || ''
  );
  const [folderCustomerDocs, setFolderCustomerDocs] = useState(
    s.uploadSettings?.folderCustomerDocs || ''
  );

  // Theme
  const [primaryColor, setPrimaryColor] = useState(
    s.themeSettings?.primaryColor || tenantObj?.branding?.primaryColor || '#3b82f6'
  );
  const [secondaryColor, setSecondaryColor] = useState(
    s.themeSettings?.secondaryColor || tenantObj?.branding?.secondaryColor || '#10b981'
  );
  ...
  const [layoutDensity, setLayoutDensity] = useState(
    s.themeSettings?.layoutDensity || 'comfortable'
  );
  ...
  const handleSave = async () => {
    ...
    if (cleanFromEmail && !emailRegex.test(cleanFromEmail)) {
      showToast('Format email SMTP tidak valid.', 'error');
      return;
    }
    ...
    if (!colorRegex.test(cleanPrimary) || !colorRegex.test(cleanSecondary)) {
      showToast(
        'Format kode warna primer/sekunder harus berupa kode hex valid (cth: #3b82f6).',
        'error'
      );
      return;
    }
        ...
        activeSection === 'general'
          ...
          : activeSection === 'portal'
            ? {
                portalSettings: {
                  enableStatusCheck,
                  enableEstimateApproval: enableEstApprove,
                  enableInvoiceView,
                  enableWarrantyView,
                  enableTicketTracking,
                  hideInternalNotes,
                  hideProfit,
                },
              }
            : activeSection === 'email'
              ? {
                  emailSettings: {
                    ...
                    ...(smtpPass ? { smtpPass } : {}),
                    defaultFromEmail: cleanFromEmail,
                    enablePushNotifications: enablePush,
                    enableRealtimeNotifications: enableRealtime,
                  },
                }
              : activeSection === 'file'
                ? {
                    uploadSettings: {
                      maxUploadSizeMb: clamp(maxUploadMb, 1, 100),
                      allowedFileTypes: allowedTypes.trim(),
                      retentionDays: clamp(retentionDays, 30, 3650),
                      fileVisibility,
                      folderInvoices: folderInvoices.trim(),
                      folderServicePhotos: folderServicePhotos.trim(),
                      folderAttendance: folderAttendance.trim(),
                      folderCustomerDocs: folderCustomerDocs.trim(),
                    },
                  }
                : {
                    themeSettings: {
  ...
  const Toggle = ({ val, onToggle }: { val: boolean; onToggle: () => void }) => (
    ...
    >
      <span
        className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-transform ${val ? 'translate-x-5' : ''}`}
      />
    </button>
  );
  ...
  const sectionDefaults = {
    general: {
      ...
      numberFormat: s.generalSettings?.numberFormat || 'ID',
      maintenanceMode: s.generalSettings?.maintenanceMode ?? false,
    },
    portal: {
      enableStatusCheck: s.portalSettings?.enableStatusCheck ?? true,
      enableEstApprove: s.portalSettings?.enableEstimateApproval ?? true,
      enableInvoiceView: s.portalSettings?.enableInvoiceView ?? true,
      enableWarrantyView: s.portalSettings?.enableWarrantyView ?? true,
      enableTicketTracking: s.portalSettings?.enableTicketTracking ?? true,
      hideInternalNotes: s.portalSettings?.hideInternalNotes ?? true,
      hideProfit: s.portalSettings?.hideProfit ?? true,
    },
    email: {
    ...
    file: {
      maxUploadMb: s.uploadSettings?.maxUploadSizeMb ?? 10,
      allowedTypes: s.uploadSettings?.allowedFileTypes || '.jpg,.png,.pdf,.doc,.xlsx',
      retentionDays: s.uploadSettings?.retentionDays ?? 365,
      fileVisibility: s.uploadSettings?.fileVisibility || 'private',
      folderInvoices: s.uploadSettings?.folderInvoices || '',
      folderServicePhotos: s.uploadSettings?.folderServicePhotos || '',
      folderAttendance: s.uploadSettings?.folderAttendance || '',
      folderCustomerDocs: s.uploadSettings?.folderCustomerDocs || '',
    },
    theme: {
  ...
  const handleResetSection = (section: AppSectionKey) => {
    ...
    switch (section) {
      ...
      case 'file':
        ...
        setFolderInvoices(defaults.folderInvoices);
        setFolderServicePhotos(defaults.folderServicePhotos);
        setFolderAttendance(defaults.folderAttendance);
        setFolderCustomerDocs(defaults.folderCustomerDocs);
        break;
      case 'theme':
            ...
            <Section title="Pengaturan Umum" icon={Settings}>
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-accent"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-accent bg-white"
                  >
              ...
              <div className="space-y-1">
                <Label text="Format Angka" />
                <select
                  value={numberFormat}
                  onChange={(e) => setNumberFormat(e.target.value)}
                  className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-accent bg-white"
                >
                  <option value="ID">ID (1.234.567,89)</option>
                  <option value="US">US (1,234,567.89)</option>
                  <option value="EU">EU (1 234 567,89)</option>
                </select>
              </div>
              <div className="space-y-1">
                <Label text="Bahasa UI" />
                <select
                  value={language}
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <Toggle
                  val={maintenanceMode}
                  onToggle={() => toggle(maintenanceMode, setMaintenanceMode)}
                />
              </div>
            </Section>
          </div>
        ...
        return (
          <Section title="Customer Portal" icon={Shield}>
            <p className="text-[10px] text-slate-500">
              Atur fitur yang tersedia untuk pelanggan di Customer Portal.
            </p>
            {[
      ...
      case 'email':
        return (
          <Section title="Email & Push Notification" icon={Mail}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs font-mono border border-slate-200 rounded-lg focus:ring-1 focus:ring-accent"
                />
              </div>
              <div className="space-y-1">
            ...
            <div className="space-y-1">
              <Label text="Default From Email" />
              <input
                type="email"
                value={defaultFrom}
                onChange={(e) => setDefaultFrom(e.target.value)}
                placeholder="noreply@namatoko.com"
                className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-accent"
              />
            </div>
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <Toggle val={enablePush} onToggle={() => toggle(enablePush, setEnablePush)} />
            </div>
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
              <span className="text-[10px] font-bold text-slate-600 uppercase">
              ...
              <Toggle
                val={enableRealtime}
                onToggle={() => toggle(enableRealtime, setEnableRealtime)}
              />
            </div>
          </Section>
        );
      case 'file':
        return (
          <Section title="Storage & File Upload" icon={Upload}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <Label text="Ukuran Upload Maks (MB)" />
                <input
                  type="number"
                  min={1}
                  max={100}
            ...
            <div className="space-y-1">
              <Label text="File Types yang Diizinkan" />
              <input
                type="text"
                value={allowedTypes}
              ...
              <p className="text-[9px] text-slate-400">
                Pisahkan dengan koma. Contoh: .jpg,.png,.pdf,.xlsx
              </p>
            </div>
            <div className="space-y-1">
              <Label text="Visibilitas File" />
              <select
            ...
            <div className="space-y-1">
              <Label text="Folder Invoice" />
              <input
                type="text"
                value={folderInvoices}
                onChange={(e) => setFolderInvoices(e.target.value)}
                placeholder="contoh: /uploads/invoices"
                className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-accent"
              />
            </div>
            <div className="space-y-1">
              <Label text="Folder Foto Servis" />
              <input
                type="text"
                value={folderServicePhotos}
                onChange={(e) => setFolderServicePhotos(e.target.value)}
                placeholder="contoh: /uploads/service-photos"
                className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-accent"
              />
            </div>
            <div className="space-y-1">
              <Label text="Folder Absensi" />
              <input
                type="text"
                value={folderAttendance}
                onChange={(e) => setFolderAttendance(e.target.value)}
                placeholder="contoh: /uploads/attendance"
                className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-accent"
              />
            </div>
            <div className="space-y-1">
              <Label text="Folder Dokumen Pelanggan" />
              <input
                type="text"
                value={folderCustomerDocs}
                onChange={(e) => setFolderCustomerDocs(e.target.value)}
                placeholder="contoh: /uploads/customer-docs"
                className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg focus:ring-1 focus:ring-accent"
              />
            </div>
          </Section>
      ...
      case 'theme':
        return (
          <Section title="Tampilan & Tema" icon={Palette}>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      applyTenantBranding(
                        { primaryColor: e.target.value, secondaryColor },
                        tenantObj?.name
                      );
                    }}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      applyTenantBranding(
                        { primaryColor, secondaryColor: e.target.value },
                        tenantObj?.name
                      );
                    }}
                    className="w-10 h-10 rounded-lg cursor-pointer"
            ...
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
              <span className="text-[10px] font-bold text-slate-600 uppercase">Dark Mode</span>
              <Toggle val={darkMode} onToggle={() => toggle(darkMode, setDarkMode)} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <Label text="Sidebar Mode" />
                <select
                  value={sidebarMode}
  ...
  return (
    <div className="space-y-6 animate-fadeIn dark:text-zinc-300 dark:[&_.bg-white]:bg-zinc-950 dark:[&_.bg-slate-50]:bg-zinc-900 dark:[&_.border-slate-100]:border-zinc-800 dark:[&_.border-slate-200]:border-zinc-800 dark:[&_.text-slate-900]:text-zinc-100 dark:[&_.text-slate-800]:text-zinc-100 dark:[&_.text-slate-700]:text-zinc-200 dark:[&_.text-slate-600]:text-zinc-300 dark:[&_input]:bg-zinc-950 dark:[&_input]:text-zinc-100 dark:[&_select]:bg-zinc-950 dark:[&_select]:text-zinc-100 dark:[&_.hover\:bg-slate-100:hover]:bg-zinc-800">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[10px] text-slate-500 mt-1">
            Umum, Customer Portal, Email, Storage, dan Tema tampilan.
          </p>
        </div>
        ...
        <aside className="space-y-3 rounded-3xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-slate-500">
              Pilih area yang ingin diubah, lalu simpan atau reset bagian ini.
            </p>
          </div>
          <div className="space-y-2">
            {sections.map((sec) => {
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm animate-fadeIn">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-[10px] text-slate-500 mt-1">
                {sections.find((item) => item.id === activeSection)?.label} tersedia dalam mode
                konfigurasi cepat.
              </p>
            </div>
            <button
