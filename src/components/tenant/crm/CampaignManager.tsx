...
export const CampaignManager: React.FC = () => {
  ...
  const getTargetCount = (segment: string) => {
    switch (segment) {
      ...
      case 'VIP':
        return tenantCustomers.filter((c) => (c.loyaltyPoints || 0) > 500).length;
      case 'CORPORATE':
        return tenantCustomers.filter((c) => c.segment === 'CORPORATE').length;
      case 'COLD':
        return tenantCustomers.filter((c) => {
          if (!c.lastServiceDate) return true;
          const months =
            (Date.now() - new Date(c.lastServiceDate).getTime()) / (1000 * 60 * 60 * 24 * 30);
          return months > 6;
        }).length;
      case 'ACTIVE':
  ...
  const handleSend = (campaign: Campaign) => {
    ...
    setTimeout(() => {
      ...
      showToast(
        `Kampanye "${campaign.name}" berhasil dikirim ke ${delivered} pelanggan`,
        'success'
      );
    }, 1500);
  ...
  const statusIcon = (s: string) => {
    switch (s) {
      ...
      case 'SENDING':
        return <Send className="w-3 h-3 text-amber-400 animate-pulse" />;
      case 'SENT':
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          ...
          <button
            ...
            className="bg-purple-600 hover:bg-purple-700 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1"
          >
          ...
          <div className="p-5 border-b border-slate-100 bg-slate-50/50 space-y-3 text-xs">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                >
                  ...
                  <option value="COLD">Pasif ({getTargetCount('COLD')})</option>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Tulis pesan kampanye di sini..."
            ...
            <div className="flex justify-end gap-2">
              <button
                ...
                className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-bold px-3 py-1.5 rounded-lg"
              >
                Batal
              </button>
              <button
                onClick={handleCreate}
                className="bg-purple-600 hover:bg-purple-700 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg"
              >
                Buat Kampanye
              </button>
            </div>
                  ...
                  <tr key={c.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-[10px] text-slate-400">
                        {c.couponCode ? `Kupon: ${c.couponCode}` : 'Tanpa kupon'}
                      </p>
                    </td>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded text-[9px] font-bold font-mono bg-slate-100 text-slate-700">
                        {c.channel}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right flex gap-1 justify-end">
                      {c.status === 'DRAFT' && (
                        <button
                          ...
                          className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-[10px] font-bold px-2 py-1 rounded flex items-center gap-1"
                        >
          ...
          >
            ...
            <div className="text-xs space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-bold">{selectedCampaign.channel}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Target:</span>
                <span className="font-bold">{selectedCampaign.targetCount} pelanggan</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Status:</span>
                <span className="font-bold">{selectedCampaign.status}</span>
              </div>
              {selectedCampaign.couponCode && (
                <div className="flex justify-between">
                  <span className="text-slate-500">Kupon:</span>
                  <span className="font-bold">{selectedCampaign.couponCode}</span>
                </div>
              )}
              ...
              {selectedCampaign.status === 'SENT' && (
                <div className="pt-2 border-t border-slate-100 grid grid-cols-3 gap-2 text-center">
                  ...
                  <div className="bg-blue-50 rounded-lg p-2">
                    <p className="text-lg font-bold text-blue-600">{selectedCampaign.readCount}</p>
                    <p className="text-[10px] text-slate-400">Dibaca</p>
                  </div>
                  <div className="bg-red-50 rounded-lg p-2">
                    <p className="text-lg font-bold text-red-600">{selectedCampaign.failedCount}</p>
